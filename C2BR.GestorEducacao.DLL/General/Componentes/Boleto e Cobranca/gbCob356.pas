...
published
  ...
  function CalcularDigitoNossoNumero(ATitulo: TgbTitulo) : string; {Calcula o dígito do NossoNumero, conforme critérios
  definidos por cada banco}
  procedure FormatarBoleto(ATitulo: TgbTitulo; var AAgenciaCodigoCedente, ANossoNumero,
                           ACarteira, AEspecieDocumento: string); {Define o formato como alguns valores serão apresentados noboleto }
  {$IFNDEF VER120}
...
begin
  ...
  ADigitoNossoNumero := Modulo10(ANossoNumero + ACodigoAgencia +
  ANumeroConta);
...
{
A primeira parte do código de barras será calculada
automaticamente.
Ela é composta por:
Código do banco (3 posições)
Código da moeda = 9 (1 posição)
Dígito do código de barras (1 posição) - Será calculado e
incluído pelo componente
Fator de vencimento (4 posições) - Obrigatório a partir de
03/07/2000
Valor do documento (10 posições) - Sem vírgula decimal e com
ZEROS à esquerda

A segunda parte do código de barras é um campo livre, que varia
de acordo
com o banco. Esse campo livre será calculado por esta função (que
você deverá
alterar de acordo com as informações fornecidas pelo banco).
}

{Segunda parte do código de barras - Campo livre - Varia de acordo
com o banco}
  ...
  Result := ACodigoAgencia + ANumeroConta + ADigitoConta +
  ANossoNumero;
end;

procedure TgbBanco356.FormatarBoleto(ATitulo: TgbTitulo; var
          AAgenciaCodigoCedente, ANossoNumero, ACarteira, AEspecieDocumento: string);
begin
  with ATitulo do
  begin
    AAgenciaCodigoCedente := Formatar
    (Cedente.ContaBancaria.CodigoAgencia,4,false,'0') + '/' + Formatar
    (Cedente.ContaBancaria.NumeroConta,7,false,'0') + '-' +
    DigitoNossoNumero;
    ANossoNumero := Formatar(NossoNumero,13,false,'0');
    ACarteira := Formatar(Carteira,2,false,'0');
    AEspecieDocumento := 'RC';
  end;
end;
    ...
    with ACobranca do begin
      ...
      for NumeroRegistro := 1 to (Retorno.Count - 2) do begin
        ...
        with ATitulo do begin
          ...
          SeuNumero := '';
          NossoNumero := Copy(Retorno.Strings[NumeroRegistro],45,13);
