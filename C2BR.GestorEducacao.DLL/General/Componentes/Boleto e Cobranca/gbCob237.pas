...
uses
   classes, SysUtils, gbCobranca, Variants
   {$IFDEF VER140}
      , MaskUtils, contnrs
   {$ELSE}
      {$IFDEF VER130}
...
begin
   ...
   begin
      ...
      Remessa.Add('01REMESSA01'+Formatar('COBRANCA',15)+Formatar(Titulos[NumeroRegistro].RegistroBanco,20,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.Nome,30,true,' ')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Banco.Codigo,3,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Banco.Nome,15,true,' ')+FormatDateTime('ddmmyy',DataArquivo)+Formatar('',8)+'MX'+Formatar(IntToStr(NumeroArquivo),7,false,'0')+Formatar('',277)+'000001');
      ...
      begin
         ...
         Registro := Registro + '2';  // Posição 106
         ...
         if (Titulos[NumeroRegistro].DataProtesto <> null) and (Titulos[NumeroRegistro].DataProtesto > Titulos[NumeroRegistro].DataVencimento) then  // Protesto
            ...
            if Titulos[NumeroRegistro].TipoOcorrencia = toRemessaOutrasOcorrencias then
