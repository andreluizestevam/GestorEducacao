...
uses
   Windows, classes, SysUtils, Graphics, ExtCtrls, Contnrs, MaskUtils, idSMTP,
   IdMessage, Variants, gbBoleto7, PDF, QRPDFFilter;
...
type
   ...
   private
      ...
      fRegistroBanco, {Número que identifica a empresa no banco para transferência do arquivo remessa}
      ...
      fDataRecebimento, {Data em que o título foi pago pelo sacado}
   ...
   published
      ...
      property RegistroBanco : string read fRegistroBanco write fRegistroBanco;
...
uses gbCob001, gbCob021, gbCob038, gbCob104, gbCob151, gbCob237, gbCob244, gbCob275, gbCob291, gbCob341, gbCob347, gbCob353, gbCob389, gbCob399, gbCob409, gbCob422, gbCob479;
...
begin
   ...
   if Resto then
      ...
      //if (Digito > 9) then  Desabilitado por causa do BB que usa o resto 10
      //   Digito := 0;
      Result := IntToStr(Digito);
   end
end;
...
begin
   ...
   RegistroBanco := ATitulo.RegistroBanco;
   ...
   DataRecebimento := ATitulo.DataRecebimento;
...
begin
   ...
   ACodigoBarras := ACodigoBanco + ACodigoMoeda + AFatorVencimento + AValorDocumento + Trim(ACampoLivre);
   ADigitoCodigoBarras := Modulo11(ACodigoBarras,9);
   if (ADigitoCodigoBarras = '0') or (ADigitoCodigoBarras = '10') or (ADigitoCodigoBarras = '11') then
...
begin
   ...
   begin
      ...
      txtSacadoRuaNumeroComplemento.Caption := AnsiUpperCase(Sacado.Endereco.Rua + ', ' + Sacado.Endereco.Numero + ' ' + Sacado.Endereco.Complemento + ' - ' + Sacado.Endereco.Bairro);
      txtSacadoCEPBairroCidadeEstado.Caption := AnsiUpperCase(FormatarComMascara('00.000-000;0; ',Sacado.Endereco.CEP) + ' - ' + Sacado.Endereco.Cidade + ' - ' + Sacado.Endereco.Estado);//+'    ' + Sacado.Endereco.Matricula);
      ...
      txtSacadoRuaNumeroComplemento2.Caption := AnsiUpperCase(Sacado.Endereco.Rua + ', ' + Sacado.Endereco.Numero + ' ' + Sacado.Endereco.Complemento + ' - ' + Sacado.Endereco.Bairro);
      txtSacadoCEPBairroCidadeEstado2.Caption := AnsiUpperCase(FormatarComMascara('00.000-000;0; ',Sacado.Endereco.CEP) + ' - ' + Sacado.Endereco.Cidade + ' - ' + Sacado.Endereco.Estado);//+'    ' + Sacado.Endereco.Matricula);
      ...
      txtSacadoRuaNumeroComplemento3.Caption := AnsiUpperCase(Sacado.Endereco.Rua + ', ' + Sacado.Endereco.Numero + ' ' + Sacado.Endereco.Complemento + ' - ' + Sacado.Endereco.Bairro);
      txtSacadoCEPBairroCidadeEstado3.Caption := AnsiUpperCase(FormatarComMascara('00.000-000;0; ',Sacado.Endereco.CEP) + ' - ' + Sacado.Endereco.Cidade + ' - ' + Sacado.Endereco.Estado);// +'    ' + Sacado.Endereco.Matricula);
   ...
   TRY
      ...
      with Mail do begin
         ...
         if Sacado.Endereco.EMail <> '' then
            Sender.Address := Sacado.Endereco.EMail
         else
            Raise Exception.Create('O e-mail do sacado não foi informado');

         Subject := Assunto;
         Body.Assign(Mensagem);
         TidAttachment.Create(Mail.MessageParts, TFileName(NomeArquivo));
      end;
      SMTP.Host := Host;
...
procedure TgbTitulo.GerarPDF;
var
   ABoleto : TBoleto;
   AExportFilter : TQRPDFExportFilter;
begin
   ABoleto := TBoleto.Create(nil);
   TRY
      AExportFilter := TQRPDFExportFilter.Create(ArqNome);
      PrepararBoleto(ABoleto);
      ABoleto.Prepare;
      ABOleto.QRPrinter.ExportToFilter(AExportFilter);
   finally
      AExportFilter.Free;
      ABoleto.Free;
   END;
...
begin
   ...
   TRY
      PrepararBoleto(ABoleto);
      ABoleto.Prepare;
      ABoleto.QRPrinter.Save(ArqNome);
      ABoleto.Free;
   EXCEPT
      ABoleto.Free;
      Raise;
      ...
      begin
         ...
         begin
            ...
            if DataRecebimento <> 0 then
               Add('Data recebimento : ' + DateToStr(DataRecebimento));
