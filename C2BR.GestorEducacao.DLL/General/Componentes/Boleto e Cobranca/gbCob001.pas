...
uses
   classes, SysUtils, gbCobranca, Variants, MaskUtils, contnrs, DateUtils
   {$IFDEF VER130}
       , Mask, contnrs
   {$ELSE}
       , Mask
...
function TgbBanco001.GetNomeBanco : string;
begin
   Result := NomeBanco;
end;

function TgbBanco001.CalcularDigitoNossoNumero(ATitulo: TgbTitulo) : string;
var
   ACarteira,
   ANossoNumero,
...
begin
   ...
   if StrToInt(ADigitoNossoNumero) = 11 then ADigitoNossoNumero := '0';
...
function TgbBanco001.GetCampoLivreCodigoBarra(ATitulo: TgbTitulo) : string;
var
   AAnoAtual,
   ACarteira,
   ANossoNumero,
...
begin
   ...
   begin
      ...
      ACodigoCedente     := Formatar(Cedente.ContaBancaria.NumeroConta,8,false,'0');
...
procedure TgbBanco001.FormatarBoleto(ATitulo: TgbTitulo; var AAgenciaCodigoCedente, ANossoNumero, ACarteira, AEspecieDocumento: string);
begin
   with ATitulo do
   begin
      AAgenciaCodigoCedente := Formatar(Cedente.ContaBancaria.CodigoAgencia,4,false,'0') + '-' + Cedente.ContaBancaria.DigitoAgencia + '/' + Formatar(Cedente.ContaBancaria.NumeroConta,8,false,'0') + '-' + Cedente.ContaBancaria.DigitoConta;
...
function TgbBanco001.GerarRemessaCNAB240(var ACobranca: TgbCobranca; var Remessa: TStringList) : boolean;
var
...
begin
   ...
   begin
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,9,false,'0'); //33 a 41 - Código do convênio no banco
      Registro := Registro + '0014'; //42 a 45 - Código do produto
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Carteira,2,false,'0'); //46 a 47 - Código da carteira
      Registro := Registro + '019'; //48 a 50 - Código da variação
      Registro := Registro + '  '; //51 a 52 - Brancos
      ...
      Registro := Registro + '   '; // 223 a 225 - 'CSP (Cobrança Sem Papel)'
      Registro := Registro + '000'; // 226 a 228 - Uso exclusivo de Vans
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,9,false,'0'); //32 a 42 - Código do convênio no banco
      Registro := Registro + '0014'; //43 a 46 - Código do produto
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Carteira,2,false,'0'); //47 a 48 - Código da carteira
      Registro := Registro + '019'; //49 a 51 - Código da variação
      Registro := Registro + '  '; //52 a 53 - Brancos
      ...
      Registro := Registro + '00000000'; //200 a 207 - Data do crédito - Só para arquivo retorno
      Registro := Registro + Formatar('',11); //208 a 218 - Uso exclusivo FEBRABAN/CNAB
      Registro := Registro + Formatar(Titulos[NumeroRegistro].RegistroBanco,6,false,'0'); //219 a 224 - Número do convênio Lider
      Registro := Registro + Formatar('',16); //225 a 240 - Uso exclusivo FEBRABAN/CNAB
      ...
      begin
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero+Titulos[NumeroRegistro].DigitoNossoNumero,12,True,'0'); //38 a 49 - Nosso número - identificação do título no banco
         Registro := Registro + '        '; //50 a 57 - Brancos
         Registro := Registro + '7'; //58 - Cobrança Especial carteira 17
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].SeuNumero,15); //63 a 77 - Número que identifica o título na empresa
         ...
         Registro := Registro + '0000000000'; //230 a 239 - Uso exclusivo FEBRABAN/CNAB
         ...
         Registro := Formatar(CodigoBanco,3,false,'0'); // 1 a 3 - Código do banco
         Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); // 4 a 7 - Número do lote
         Registro := Registro + '3'; // 8 a 8 - Tipo do registro: Registro detalhe
         Registro := Registro + Formatar(IntToStr(2*NumeroRegistro+2),5,false,'0'); // 9 a 13 - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
         Registro := Registro + 'Q'; // 14 a 14 - Código do segmento do registro detalhe
         Registro := Registro + ' '; // 15 a 15 - Uso exclusivo FEBRABAN/CNAB: Branco
         case Titulos[NumeroRegistro].TipoOcorrencia of // 16 a 17 - Código de movimento}
            toRemessaRegistrar                 : Registro := Registro + '01';
            toRemessaBaixar                    : Registro := Registro + '02';
            toRemessaConcederAbatimento        : Registro := Registro + '04';
         ...
         {Dados do sacado}
         Registro := Registro + Formatar(ASacadoTipoInscricao,1,false,'0'); // 18 a 18
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.NumeroCPFCGC,15,false,'0'); // 19 a 33
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Nome,40); // 34 a 73
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Endereco.Rua+' '+Titulos[NumeroRegistro].Sacado.Endereco.Numero+' '+Titulos[NumeroRegistro].Sacado.Endereco.Complemento,40); // 74 a 113
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Endereco.Bairro,15); // 114 a 128
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Endereco.CEP,8,true,'0'); // 129 a 136
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Endereco.Cidade,15,true); // 137 a 151
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Endereco.Estado,2,false); // 152 a 153
         {Dados do sacador/avalista}
         Registro := Registro + '0'; // 154 a 154 - Tipo de inscrição: Não informado
         Registro := Registro + Formatar('',15,false,'0'); // 155 a 169 - Número de inscrição
         Registro := Registro + Formatar('',40); // 170 a 209 - Nome do sacador/avalista

         Registro := Registro + '000'; // 210 a 212 - Uso exclusivo FEBRABAN/CNAB
         Registro := Registro + Formatar('',20); // 213 a 232 - Uso exclusivo FEBRABAN/CNAB
         Registro := Registro + Formatar('',8); // 233 a 240 - Uso exclusivo FEBRABAN/CNAB
      ...
      Registro := Formatar(CodigoBanco,3,false,'0'); {1-3  Código do banco}
      Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); {4-7 Número do lote}
      Registro := Registro + '5'; {8-8 Tipo do registro: Registro trailer do lote}
      Registro := Registro + Formatar('',9); {9-17 Uso exclusivo FEBRABAN/CNAB}
      ...
      Registro := Registro + Formatar(IntToStr(Remessa.Count),6,false,'0'); {18-23}
      {Totalização da cobrança simples - Só é usado no arquivo retorno}
      Registro := Registro + Formatar('',6,false,'0'); {24-29  Quantidade títulos em cobrança}
      Registro := Registro + Formatar('',17,false,'0'); {30-46  Valor dos títulos em carteiras}
      {Uso exclusivo FEBRABAN/CNAB}
      Registro := Registro + Formatar('',23,false,'0'); {47-69  Uso exclusivo FEBRABAN/CNAB}
      {Totalização da cobrança caucionada - Só é usado no arquivo retorno}
      Registro := Registro + Formatar('',6,false,'0'); {70-75  Quantidade títulos em cobrança}
      Registro := Registro + Formatar('',17,false,'0'); {76-92  Valor dos títulos em carteiras}
      {Uso exclusivo FEBRABAN/CNAB}
      Registro := Registro + Formatar('',23,false,'0'); {93-115  Uso exclusivo FEBRABAN/CNAB}
      Registro := Registro + Formatar('',125); {116- Uso exclusivo FEBRABAN/CNAB}
...
function TgbBanco001.GerarRemessaCNAB400(var ACobranca: TgbCobranca; var Remessa: TStringList) : boolean;
var
...
function  TgbBanco001.GerarRemessa(var ACobranca: TgbCobranca; var Remessa: TStringList) : boolean;
begin
...
function TgbBanco001.LerRetornoCNAB240(var ACobranca: TgbCobranca; Retorno: TStringList) : boolean;
var
   ACodigoBanco,
   ANomeCedente,
   ATipoInscricao,
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  if Copy(Retorno.Strings[NumeroRegistro],146,8) <> '00000000' then
...
function TgbBanco001.LerRetornoCNAB400(var ACobranca: TgbCobranca; Retorno: TStringList) : boolean;
var
   ACodigoBanco,
   ANomeCedente,
   ATipoInscricao,
...
function TgbBanco001.LerRetorno(var ACobranca: TgbCobranca; Retorno: TStringList) : boolean;
var
   ACodigoBanco,
   ANomeCedente,
...
function TgbBanco001.VerificaOcorrenciaOriginal(sOcorrenciaOriginal: String): String;
begin
...
function TgbBanco001.VerificaMotivoRejeicaoComando(sMotivoRejeicaoComando: String): String;
begin
