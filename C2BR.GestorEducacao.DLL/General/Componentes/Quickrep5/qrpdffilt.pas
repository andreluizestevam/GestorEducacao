...
begin
  ...
  if ir.ItemType=IT_TEXT then begin
...
begin
  ...
  while sFrom.Position<sFrom.Size do begin
    ...
    if Length(Buffer)>0 then begin
      sTo.Write(Buffer[1], Length(Buffer));
    end;
    Buffer:='';
  end;
  if Length(LastBuf)>0 then begin
...
var
  ...
  function BToHex(B: Byte) : ShortString;
...
begin
  ...
  while (WasRead>0) do begin
    lineout:='';
    for I := 0 to WasRead-1 do lineout:=lineout+BToHex(CC[I]);
      ...
      begin
        ...
        if widthstr<>'' then begin
          ...
          try
            ...
            if widlist.Count<256 then begin
            ...
            for K := 0 to 255 do if K<widlist.Count then PDFFont.Metrics[K]:=StrToInt(widlist[K]);
          finally
            widlist.Free;
          end;
        end
        ...
        begin
{$ifdef CW32}
           //GetCharWidth32 is unsupported in Win9X according to Microsoft!!!
          ...
          SetLength(PDFFont.Metrics, 256);
          for K := 0 to 255 do PDFFont.Metrics[K]:=Widths[K];
{$else}
      ...
      begin
        ...
        if K<>0 then begin
          ...
          for I := 1 to 255 do begin
            ...
            Found:=False;
            for K := 0 to High(UniGlyphs) do begin
              if charCodes[I]=UniGlyphs[K].ID then begin
            ...
            if not Found then begin
              if charCodes[I]<256 then encoding:=encoding+'/space' else
    ...
    begin
      ...
      try
        ...
        for K := 0 to ntabs-1 do begin
          ...
          // four 4-byte fields
          tabTag:=Chr(Buff[Offset])+Chr(Buff[Offset+1])+Chr(Buff[Offset+2])+Chr(Buff[Offset+
                                 3]);
...
{$ifdef PAGE_CUT}
      // restrict pages
      FPageNumber := PDFPagesToPrint;
      for K := FPDF.pagetextfiles.Count-1 downto PDFPagesToPrint do FPDF.pagetextfiles.Delete(k);
      for K := FPDF.pagegraphicsfiles.Count-1 downto PDFPagesToPrint do FPDF.pagegraphicsfiles.Delete(k);
{$endif}
      // Root catalog
      ...
      CurrObject:=3;
      SaveOffset;
      for J := 0 to FPDF.pagetextfiles.Count-1 do begin
...
GetDIBits( (GControl as TQRImage).canvas.handle,
 ...
 begin
    ...
    begin
      str2.Clear;
      LZWEncode(Str1, Str2);
      //RunLengthEncodeFilter( Str1, str2);
      Ascii85EncodeFilter(Str2, Str1);
      PDFItem.ImageData:=StreamToString(Str1);
      PDFItem.ImageFilter:='/ASCII85Decode /LZWDecode';
      //PDFItem.ImageFilter:='/ASCII85Decode /RunLengthDecode';
    end
    else
    begin
      //      AsciiHexEncodeFilter(str1, str2);
      Ascii85EncodeFilter(Str1, Str2);
      PDFItem.ImageData:=StreamToString(Str2);
