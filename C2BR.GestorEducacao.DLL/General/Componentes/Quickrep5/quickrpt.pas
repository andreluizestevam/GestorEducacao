{ :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  ::                                                         ::
  :: Copyright (c) 2007/8     QBS Software                   ::
  :: All Rights Reserved                                     ::
  ::                                                         ::
...
type
  ...
  // QR5
  TCBPrintOrder = ( cboAfterParent, cboBeforeParent, cboBeforeAndAfter );// childband print order
...
uses
  QRCtrls, CommDlg, QRPrnSu , qrframelines, qrxdocument;
    ...
    begin
      ...
      begin
        ...
        if ParentReport is TQuickRep and
          DSOK and  (TQuickRep(ParentReport).DataSet = DataSet) and (RecCount <> 0) and
             (ParentReport.QRPrinter<>nil) then
            ParentReport.QRPrinter.Progress := (Longint(DetailNumber) * 100) div RecCount
        else
        begin
            ParentReport.QRPrinter.Progress := ParentReport.QRPrinter.Progress + 2;
            if ParentReport.QRPrinter.Progress > 100 then ParentReport.QRPrinter.Progress := 5;
        end;
      ...
      ParentReport.QRPrinter.Progress := 100;
...
begin
  ...
  try
    ...
    begin
        ...
        QRPrinter.Client := nil;//Fixes crash on print direct from preview
    CreateReport(false);
  finally
    QRPrinter.Free;
    QRPrinter := OldQRPrinter;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        UserPrintersettings.PrinterIndex := aDialog.Printer.PrinterIndex;
        UserPrintersettings.FirstPage := aDialog.FromPage;
        UserPrintersettings.LastPage := aDialog.ToPage;
...
begin
  ...
  try
    ...
    begin
      ...
      CompositePrinterIndex := aDialog.Printer.PrinterIndex;
      case PrintRange of
        prAllPages,
        prSelection : begin
                       PrinterSettings.FirstPage := 0;
