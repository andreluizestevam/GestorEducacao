...
implementation
uses OleCtrls, {jpeg,} QRXMLSFilt, QRXBrowser, lzw{$ifndef USE_DLL}, pdfobjs{$endif};
var
...
begin
      ...
      numpages := GetPagecount;
      InitLib( self);
      SetPageparams( FPagewidth*mmToPoints, FPageheight*mmToPoints, 30.0, 0.0, 0.0);
      // this forces the PDF code to write to this and NOT open a file.
      // if it's a file, the PDF code will close it.
      SetOutputStream( pdfstream );
      if FCompressImages then
          SetFiltCompression( 1 )
      else
          SetFiltCompression( 0 );
      pdfitem.Xpos := 0.0;
      pdfitem.Ypos := 0.0;
...
// Width="152.66" Height="2.38" Color="#C0C0C0" BC="#FFFFFF" FT="Out" >Graphic item</Item>
   try
...
procedure CaptureImageBytes( srcmap : TBitmap; outstream : TStream; CompressionOn : boolean);
    var
        k, j : longint;
        b : byte;
        pcolor : TColor;
...
begin
   ...
   try
    try
     ...
     tempstream := Tfilestream.Create( imgfilename, fmCreate);
