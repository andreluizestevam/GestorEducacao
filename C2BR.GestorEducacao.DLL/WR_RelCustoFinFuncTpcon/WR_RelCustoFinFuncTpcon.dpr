library WR_RelCustoFinFuncTpcon;
...
uses
  ...
  U_FrmRelCustoFinFuncTpcon in 'Relatorios\U_FrmRelCustoFinFuncTpcon.pas' {FrmRelCustoFinFuncTpcon};

// Controle Administrativo > Controle de Funcionários
// Relatório: Emissão de Custo Financeiro (Salário) de Funcionários (Por Tipo de Contrato)
// STATUS: OK 
function DLLRelCustoFinFuncTpcon(strIdentFunc, strPathReportGenerate, strReportName, strParamRel, strP_CO_EMP, strP_CO_TPCON, strP_FLA_PROFESSOR, strP_CO_SEXO_COL, strP_tp_def: PChar): Integer; export; cdecl;
var
  intReturn: Integer;
  Relatorio: TFrmRelCustoFinFuncTpcon;
...
begin
  ...
  Try
    Try
      ...
      SqlString := ' select f.no_fun,C.CO_TPCON,c.CO_MAT_COL,c.NO_COL,c.NU_CPF_COL,c.DT_INIC_ATIV_COL,c.CO_TPCAL,'+
               ...
               '                   END), '+
               ' D.CO_SIGLA_DEPTO, G.NO_INST, TC.NO_TPCON, EMP.NO_FANTAS_EMP, EMP.SIGLA '+
              { ' 	(SELECT SUM(C.VL_SALAR_COL) '+
               '	 FROM TB03_COLABOR C '+
               '	 WHERE C.CO_TPCON = ' + IntToStr(codContrato[cbContrato.ItemIndex]) +
               '	) TOTALSALARIO '+  }
               ' from TB03_COLABOR c '+
               ...
               ' WHERE C.CO_TPCON = TC.CO_TPCON ';

        if strP_CO_TPCON <> 'T' then
        begin
          SqlString := SqlString + 'AND C.CO_TPCON = ' + strP_CO_TPCON;
        end;
        ...
        SqlString := SqlString + ' GROUP BY TC.NO_TPCON,C.CO_TPCON,EMP.NO_FANTAS_EMP,EMP.SIGLA,c.CO_MAT_COL,c.NO_COL,c.NU_CPF_COL,c.DT_INIC_ATIV_COL,c.CO_TPCAL,'+
                 ...
                 'order by TC.NO_TPCON,C.NO_COL ';

      // Cria uma instância do Relatório.
      Relatorio := TFrmRelCustoFinFuncTpcon.Create(Nil);
