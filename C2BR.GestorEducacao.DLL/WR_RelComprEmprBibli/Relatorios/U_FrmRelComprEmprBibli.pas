...
type
  TFrmRelComprEmprBibli = class(TFrmRelTemplate)
    QRBand1: TQRBand;
    QRLabel8: TQRLabel;
...
var
  FrmRelComprEmprBibli: TFrmRelComprEmprBibli;
...
procedure TFrmRelComprEmprBibli.PageHeaderBand1BeforePrint(
...
begin
  ...
  if not QryRelatorio.FieldByName('DE_OBS_EMP_BIBLIOT').IsNull then
    qrlObsEmprestimo.Caption := QryRelatorio.FieldByName('DE_OBS_EMP_BIBLIOT').AsString
  else
    qrlObsEmprestimo.Caption := '-';
  ...
  begin
    ...
    begin
      ...
      QRLDesUsu.Caption := descUsu;

      QRLAssUsuario.Caption := FieldByName('NOME').AsString;

      if QryRelatorio.FieldByName('TP_USU_BIB').AsString = 'O' then
        QRLDocAss.Caption := FormatMaskText('000.000.000-00;0',FieldByName('numDoc').AsString)
      else if QryRelatorio.FieldByName('TP_USU_BIB').AsString = 'A' then
        QRLDocAss.Caption := 'Mat.: ' + FormatMaskText('00.000.000000;0',FieldByName('numDoc').AsString)
      else
        QRLDocAss.Caption := 'Mat.: ' + FormatMaskText('99.999-9;0',FieldByName('numDoc').AsString);
...
procedure TFrmRelComprEmprBibli.DetailBand1BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  inherited;
...
procedure TFrmRelComprEmprBibli.QuickRep1BeforePrint(
...
initialization
  ...
  RegisterClasses([TFrmRelComprEmprBibli]);
