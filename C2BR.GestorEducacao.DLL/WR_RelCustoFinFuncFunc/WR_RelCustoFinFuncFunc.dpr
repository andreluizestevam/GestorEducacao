library WR_RelCustoFinFuncFunc;
...
uses
  ...
  U_FrmRelCustoFinFuncFunc in 'Relatorios\U_FrmRelCustoFinFuncFunc.pas' {FrmRelCustoFinFuncFunc};

// Controle Administrativo > Controle de Funcionários
// Relatório: Emissão de Custo Financeiro (Salário) de Funcionários (Por Função)
// STATUS: OK
function DLLRelCustoFinFuncFunc(strIdentFunc, strPathReportGenerate, strReportName, strParamRel, strP_CO_EMP, strP_CO_FUN, strP_FLA_PROFESSOR, strP_CO_SEXO_COL, strP_tp_def: PChar): Integer; export; cdecl;
var
  intReturn: Integer;
  Relatorio: TFrmRelCustoFinFuncFunc;
...
begin
  ...
  Try
    Try
      ...
      SqlString := ' select f.no_fun,c.co_fun,c.CO_MAT_COL,c.NO_COL,c.NU_CPF_COL,c.DT_INIC_ATIV_COL,c.CO_TPCAL,'+
               ...
               '	 FROM TB03_COLABOR C '+
               '	 WHERE C.CO_FUN = ' + IntToStr(codFuncao[cbFuncao.ItemIndex]) +
               '	) TOTALSALARIO '+ }
               ' from TB03_COLABOR c '+
        ...
        SqlString := SqlString + ' GROUP BY f.no_fun,c.co_fun,EMP.NO_FANTAS_EMP,EMP.SIGLA,c.CO_MAT_COL,c.NO_COL,c.NU_CPF_COL,c.DT_INIC_ATIV_COL,c.CO_TPCAL,'+
                 ...
                 'order by f.no_fun,C.NO_COL ';

      // Cria uma instância do Relatório.
      Relatorio := TFrmRelCustoFinFuncFunc.Create(Nil);
