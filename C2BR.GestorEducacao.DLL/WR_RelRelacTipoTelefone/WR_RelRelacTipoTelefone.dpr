        ...
        begin
          ...
          begin
            if strP_CO_SITU <> 'I' then
              Relatorio.QRLParam.Caption := '( Tipo Telefone: ' + Relatorio.QryRelatorio.FieldByName('NM_TIPO_TELEFONE').AsString + ' - Situação: Ativo )'
            else
              Relatorio.QRLParam.Caption := '( Tipo Telefone: ' + Relatorio.QryRelatorio.FieldByName('NM_TIPO_TELEFONE').AsString + ' - Situação: Inativo )';
          end
          else
            Relatorio.QRLParam.Caption := '( Tipo Telefone: ' + Relatorio.QryRelatorio.FieldByName('NM_TIPO_TELEFONE').AsString + ' - Situação: Todas )';
        end
        else
        begin
          if strP_CO_SITU <> 'T' then
          begin
            if strP_CO_SITU <> 'I' then
              Relatorio.QRLParam.Caption := '( Tipo Telefone: Todas - Situação: Ativo )'
            else
              Relatorio.QRLParam.Caption := '( Tipo Telefone: Todas - Situação: Inativo )';
          end
          else
            Relatorio.QRLParam.Caption := '( Tipo Telefone: Todas - Situação: Todas )';
        end;
