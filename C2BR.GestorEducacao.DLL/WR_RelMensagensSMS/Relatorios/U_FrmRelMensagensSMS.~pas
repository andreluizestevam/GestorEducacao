...
type
  TFrmRelMensagensSMS = class(TFrmRelTemplate)
    QRGroup1: TQRGroup;
    QRGroup2: TQRGroup;
    QRShape6: TQRShape;
    QRBand1: TQRBand;
...
var
  FrmRelMensagensSMS: TFrmRelMensagensSMS;
...
procedure TFrmRelMensagensSMS.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
  ...
  if not QryRelatorio.FieldByName('NU_TELE_CELU_COL').IsNull then
    QRLTelEmissor.Caption := FormatMaskText('(00) 0000-0000;0',QryRelatorio.FieldByName('NU_TELE_CELU_COL').AsString)
  else
    QRLTelEmissor.Caption := '-';
...
procedure TFrmRelMensagensSMS.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  inherited;
  totalEnv := 0;
...
procedure TFrmRelMensagensSMS.SummaryBand1BeforePrint(
...
initialization
  ...
  RegisterClasses([TFrmRelMensagensSMS]);
