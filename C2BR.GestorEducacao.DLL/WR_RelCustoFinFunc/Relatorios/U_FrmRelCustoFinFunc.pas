...
type
  TFrmRelCustoFinFunc = class(TFrmRelTemplate)
    DetailBand1: TQRBand;
...
var
  FrmRelCustoFinFunc: TFrmRelCustoFinFunc;
...
{procedure TFrmRelCustoFinFunc.FormCreate(Sender: TObject);
begin
  ShowMessage('Construtor do relatório');
end;}

procedure TFrmRelCustoFinFunc.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  diasnoano : real;
begin
...
procedure TFrmRelCustoFinFunc.QuickRep1BeforePrint(Sender: TCustomQuickRep;
...
procedure TFrmRelCustoFinFunc.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFrmRelCustoFinFunc.QRBand1AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  inherited;
  salTotal := 0;
...
procedure TFrmRelCustoFinFunc.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
  if not QryRelatorioNO_FANTAS_EMP.IsNull then
    QRLNoEmpresa.Caption := UpperCase(QryRelatorioNO_FANTAS_EMP.AsString);
end;
