...
namespace BoletoNet
{
    ...
    internal class Banco_Caixa : AbstractBanco, IBanco
    {
        ...
        public override void FormataCodigoBarra(Boleto boleto)
        {
            ...
            // Início Campo livre
            int dvCodigoCedente = -1;

            //ESSA IMPLEMENTAÇÃO FOI FEITA PARA CARTEIAS SIGCB "SR" COM NOSSO NUMERO DE 14 e 17 POSIÇÕES
            if (boleto.Carteira.Equals("SR") && (boleto.NossoNumero.Length == 14 || boleto.NossoNumero.Length == 17))
            {
                //14 POSIÇOES
                ...
                if (boleto.NossoNumero.Length == 17)
                {
                    ...
                    // Posição 26
                    dvCodigoCedente = Mod11Base9(codigoCedente);
            ...
            else
            {
                ...
                // Posição 31 - 33
                string segundaParteNossoNumero = boleto.NossoNumero.Substring(3, 3);//(0, 3); 

                // Posição 24
                string segundaConstante = EMISSAO_CEDENTE.ToString();

                //Posição 35 - 43
                string terceiraParteNossoNumero = boleto.NossoNumero.Substring(6, 9) + segundaConstante +
                                                  segundaConstante; //(3, 7);
            ...
            // Posição 5
            _dacBoleto = dvGeral;
            if (boleto.Carteira.ToString() == "SR")
                boleto.Cedente.ContaBancaria.OperacaConta = "2";
            boleto.Cedente.DigitoCedente = dvCodigoCedente;
        ...
        public override void FormataNossoNumero(Boleto boleto)
        {
            ...
            if (boleto.Carteira.Equals("SR"))
            {
                ...
                modalidade = "1";
            ///Josemberg - Verifiquei o código da caixa e constei que está no padrão diferente ao informado pelo o cliente
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            if (boleto.Carteira.Equals("SR"))
            {
                if ((boleto.NossoNumero.Length != 14) && (boleto.NossoNumero.Length != 15) && (boleto.NossoNumero.Length != 17))
                {
                    throw new Exception("Nosso Número inválido, Para Caixa Econômica - Carteira SR o Nosso Número deve conter 14, 15 ou 17 posições.");
                }
            }
            else
            {
            ...
            boleto.LocalPagamento = "CASAS LOTÉRICAS, AGÊNCIAS DA CAIXA E REDE BANCÁRIA";
        ...
        {
            return GerarDetalheSegmentoPRemessaCNAB240(boleto, numeroRegistro, numeroConvenio, boleto.Cedente);
        } 
        ...
        public string GerarHeaderRemessaCNAB240(Cedente cedente)
        {
            try
            {
                ...
                // Coloquei para pegar apenas o código, porque o digito estava retornando -1
                //header += Utils.FormatCode(cedente.Codigo.ToString() + cedente.DigitoCedente.ToString(), "0", 16); // Código do Convênio no Banco 
                header += Utils.FormatCode(cedente.Codigo.ToString(), "0", 16);
                ...
                //Estava populando caracteres a mais
        ...
        private string GerarHeaderLoteRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                header += Utils.FormatCode(cedente.ContaBancaria.Agencia, "0", 5);                      // Agência Mantenedora da Conta 
                header += Utils.FormatCode(cedente.ContaBancaria.DigitoAgencia, "0", 1);                // Dígito Verificador da Agência 
        ...
        public string GerarDetalheSegmentoPRemessaCNAB240(Boleto boleto, int numeroRegistro, string numeroConvenio, Cedente cedente)
        {
            try
            {
                ...
                // Estava preenchendo os zeros a direita e não a esquerda
                //header += Utils.FormatCode(numeroRegistro.ToString(), "0", 5);                          // Nº Sequencial do Registro no Lote 
                header += Utils.FitStringLength(numeroRegistro.ToString(), 5, 5, '0', 0, true, true, true);
                ...
                //Alterada as linhas acima para:
                if (boleto.JurosMora > 0)
                {
                    header += "1";
                    header += Utils.FitStringLength(boleto.DataMulta.ToString("ddMMyyyy"), 8, 8, '0', 0, true, true, false);
                    header += Utils.FitStringLength(boleto.JurosMora.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true);
                }
                else
                {
                    header += "3";
                ...
                // Alterada as linhas anteriores para:
                if (boleto.ValorDesconto > 0)
                {
                    header += "1";
                    header += Utils.FitStringLength(boleto.DataDesconto.ToString("ddMMyyyy"), 8, 8, '0', 0, true, true, false);
                    header += Utils.FitStringLength(boleto.ValorDesconto.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true);
                }
                else
                ...
                header += Utils.FitStringLength(boleto.IOF.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true);
                // Estava pegando o numero de casas errado 
                //header += Utils.FormatCode(boleto.Abatimento.ToString().Replace(",", "").Replace(".", ""), "0", 13); // Valor do Abatimento 
                header += Utils.FitStringLength(boleto.Abatimento.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true);
                // Coloquei o numero do documento da tabela de título
                //header += Utils.FormatCode("", " ", 25);                                                // Identificação do Título na Empresa
                header += Utils.FitStringLength(boleto.NumeroDocumento, 25, 25, ' ', 0, true, true, false);                
                ...
                // Alterei que estava jogando apenas 2 posições e são 3
                //header += diasDevolucao.ToString("00");                                                 // Número de Dias para Baixa/Devolução 3 posi
                header += diasDevolucao.ToString("000");
        ...
        public string GerarDetalheSegmentoQRemessaCNAB240(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                //header += Utils.FormatCode(boleto.Sacado.Nome, " ", 40);                                
                header += Utils.FitStringLength(boleto.Sacado.Nome, 40, 40, ' ', 0, true, true, false); // Nome
                //header += Utils.FormatCode(boleto.Sacado.Endereco.End, " ", 40);                        // Endereço
                header += Utils.FitStringLength(boleto.Sacado.Endereco.End, 40, 40, ' ', 0, true, true, false);
                //header += Utils.FormatCode(boleto.Sacado.Endereco.Bairro, " ", 15);                     // Bairro 
                header += Utils.FitStringLength(boleto.Sacado.Endereco.Bairro, 15, 15, ' ', 0, true, true, false);
                header += boleto.Sacado.Endereco.CEP;                                                   // CEP + Sufixo do CEP
                //header += Utils.FormatCode(boleto.Sacado.Endereco.Cidade, " ", 15);                     // Cidade 
                header += Utils.FitStringLength(boleto.Sacado.Endereco.Cidade, 15, 15, ' ', 0, true, true, false);
        ...
        public string GerarDetalheSegmentoRRemessaCNAB240(Boleto boleto, int numeroRegistroDetalhe, TipoArquivo CNAB240)
        {
            try
            {
                ...
                header += Utils.FitStringLength(numeroRegistroDetalhe.ToString(), 5, 5, '0', 0, true, true, true);
                header += "R";                                                                          // Cód. Segmento do Registro Detalhe
                header += " ";                                                                          // Uso Exclusivo FEBRABAN/CNAB
                header += "01";                                                                         // Código de Movimento Remessa
                header += Utils.FormatCode("", " ", 48);                                                // Uso Exclusivo FEBRABAN/CNAB 
                header += "2";                                          // Código da Multa '1' = Valor Fixo,'2' = Percentual,'0' = Sem Multa 
                header += Utils.FitStringLength(boleto.DataMulta.ToString("ddMMyyyy"), 8, 8, '0', 0, true, true, false); // Data da Multa
                //Estava pegando apenas 13 posições e são 15
                //header += Utils.FormatCode(boleto.ValorMulta.ToString().Replace(",", "").Replace(".", ""), "0", 13); // Valor/Percentual a Ser Aplicado
                header += Utils.FitStringLength(boleto.ValorMulta.ToString("0.00").Replace(",", ""), 15, 15, '0', 0, true, true, true);
        ...
        public override DetalheSegmentoTRetornoCNAB240 LerDetalheSegmentoTRetornoCNAB240(string registro)
        {
            ...
            string _outNumeroDocumento = registro.Substring(58, 11); //16
            ...
            try
            {
                ...
                segmentoT.NumeroDocumento = registro.Substring(58, 11); //16
        ...
        public override DetalheSegmentoURetornoCNAB240 LerDetalheSegmentoURetornoCNAB240(string registro)
        {
            ...
            try
            {
                ...
                segmentoU.DataOcorrencia = DateTime.ParseExact(registro.Substring(137, 8), "ddMMyyyy", CultureInfo.InvariantCulture);
