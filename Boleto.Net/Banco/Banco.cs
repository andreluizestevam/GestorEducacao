...
namespace BoletoNet
{
    public class Banco : AbstractBanco, IBanco
    {
        ...
        public override string GerarTrailerRemessa(int numeroRegistro, TipoArquivo tipoArquivo, decimal vltitulostotal)
        {
            try
            {
                return _IBanco.GerarTrailerRemessa(numeroRegistro, tipoArquivo, vltitulostotal);
            }
            catch (Exception ex)
            {
        ...
        public override string GerarHeaderRemessa(Cedente cedente, TipoArquivo tipoArquivo, int numeroArquivoRemessa, int numeroRegistros)
        {
            try
            {
                return _IBanco.GerarHeaderRemessa(cedente, tipoArquivo, numeroArquivoRemessa, numeroRegistros);
            }
            catch (Exception ex)
            {
                throw new Exception("Erro durante a geração do registro HEADER do arquivo de REMESSA.", ex);
