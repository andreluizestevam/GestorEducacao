...
namespace BoletoNet
{
    sealed class Utils
    {
        ...
        */
        ...
        internal static string FormataAgenciaConta(string agencia, string digitoAgencia, string conta, string digitoConta, int banco, int cedente)
        {
            string agenciaConta = string.Empty;
            try
            {
                agenciaConta = agencia;
                if (digitoAgencia.Trim() != string.Empty)
                    agenciaConta += "-" + digitoAgencia;

                /*12/03/2015 - MAXWELL PEREIRA ALMEIDA: Inserida essa regra, pois para o banco itaú, é requerido que a
                 conta tenha 5 dígitos, então foi feito um padleft para completar com 0 à esquerda até 5 caracteres*/
                agenciaConta += "/" + (banco == 341 ? conta.PadLeft(5, '0') : conta);

                if (digitoConta != string.Empty)
                {
                    /*27/03/2015 - MAXWELL PEREIRA ALMEIDA: Alterado o dígito do DAC para o real dígito da conta, 
                     a pedido do Gerente do Banco Itaú agência 6214 Geraldo Protásio*/
                    //if (banco == 341)
                    //    agenciaConta += "-" + Strings.Right(cedente.ToString(), 1);
                    //else
