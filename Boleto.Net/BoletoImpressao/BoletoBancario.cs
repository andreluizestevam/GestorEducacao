...
namespace BoletoNet
{
    ...
    public class BoletoBancario : System.Web.UI.Control
    {
        ...
        /// <summary>
        /// Texto de observação do canhoto do carnê
        /// </summary>
        [Browsable(true), Description("Observação no canhoto do carnê")]
        public string ObsCarne { get; set; }

        /// <summary>
        /// Cnpj do cedente do canhoto do carnê
        /// </summary>
        [Browsable(true), Description("Cnpj do cedente do canhoto do carnê")]
        ...
        private string GeraHtmlCarne(string cnpj, string obs, string htmlBoleto)
        {
            StringBuilder html = new StringBuilder();
        ...
        /// <summary>
        /// 12/03/2015 - MAXWELL PEREIRA ALMEIDA: De acordo com normativa do banco central do brasil em vigor
        /// desde 28/06/2013, cedente passa a ser beneficiário e Sacado passa a ser Pagador, daí se faz necessária 
        ///  a substituição que é realizada aqui.
        /// </summary>
        /// <param name="html"></param>
        public void AlteraNomeclaturas(StringBuilder html)
        {
            html.Replace("Cedente", "Beneficiário");
            html.Replace("cedente", "Beneficiário");
            html.Replace("Cendente", "Beneficiário");
            html.Replace("cendente", "Beneficiário");

            html.Replace("Sacado", "Pagador");
            html.Replace("sacado", "Pagador");
        }
        ...
        public string GeraHtmlReciboCedente()
        {
            try
            {
                ...
                {
                    /*12/03/2015 - MAXWELL PEREIRA ALMEIDA: O texto à ser incluso na parte de instruções foi alterado hoje
                     devido à uma solicitação da equipe de homologação do Boleto Itaú.*/
                    ...
                    html.Replace("(Texto de responsabilidade do cedente)", "(Instruções de responsabilidade do beneficiário. Qualquer dúvida sobre este boleto, contate o beneficiário.)");
                }
                ...
                else
                {
                    ...
                    if (Boleto.Banco.Codigo == 33 || Boleto.Banco.Codigo == 104)
                        html.Replace("Carteira /", "");
                }
        ...
        private string MontaHtml(string urlImagemLogo, string urlImagemBarra, string imagemCodigoBarras)
        {
            ...
            string agenciaConta = Utils.FormataAgenciaConta(Cedente.ContaBancaria.Agencia, Cedente.ContaBancaria.DigitoAgencia, Cedente.ContaBancaria.Conta, Cedente.ContaBancaria.DigitoConta, Boleto.Banco.Codigo, Boleto.Cedente.Convenio);
            ...
            {
                if (!Cedente.ContaBancaria.OperacaConta.Equals(""))
                {
                    if(Boleto.Banco.Codigo.Equals(104))
                        agenciaCodigoCedente = string.Format("{0}{1}/{2}-{3}", Cedente.ContaBancaria.Agencia, Cedente.ContaBancaria.OperacaConta, Utils.FormatCode(Cedente.Codigo.ToString(), 6), Boleto.Cedente.DigitoCedente);
                    else
            ...
            else
            {
                ...
                else if (Boleto.Banco.Codigo == 104) // Para Banco Caixa mostra o número da agência + o código do tipo de registro + o código do cedente + o código do beneficiário - Por B2 
                {
                    int numeroTipo = 0;
                    if (Boleto.Carteira.ToString() == "SR")
                    {
                        ...
                        numeroTipo = 1;
                    agenciaCodigoCedente = string.Format("{0}{1}/{2}-{3}", Cedente.ContaBancaria.Agencia, numeroTipo, Utils.FormatCode(Cedente.Codigo.ToString(), 6), Boleto.Cedente.DigitoCedente);
                }
                else
            ...
            {
                html.Append(GeraHtmlCarne(CnpjCedente, ObsCarne, GeraHtmlReciboCedente())); 
            }
            ...
            if (FormatoCarne)
                html.Append("<div style=\"height:120px;\"></div>");
                //html.Append("</br></br></br></br></br></br></br>");

            if (Boleto.Banco.Codigo != 237)
                html.Replace("@DESCONTOS", (Boleto.ValorDesconto == 0 ? "" : Boleto.ValorDesconto.ToString("R$ ##,##0.00")));
            else
                html.Replace("@DESCONTOS", "");
