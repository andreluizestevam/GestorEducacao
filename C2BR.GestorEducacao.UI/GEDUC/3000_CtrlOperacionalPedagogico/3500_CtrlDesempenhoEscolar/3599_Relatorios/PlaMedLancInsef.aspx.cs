...
{
    public partial class PlaMedLancInsef : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    liUnidadeCont.Visible = false;
                    liAnoRefer.Visible = false;
                    liTurma.Visible = false;
                    liAgrupar.Visible = false;
                    chkForm.Visible = false;
                    liTipoPlan.Visible = false;
                    chkComLanc.Visible = false;
        ...
        private void imprime()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var resultado = TB07_ALUNO.RetornaTodosRegistros().Where(x => x.CO_ALU == CO_ALU).FirstOrDefault();

                    strP_CO_EMP_REF = LoginAuxili.CO_EMP;
                    strP_CO_ANO_MES_MAT = DateTime.Now.Year.ToString();
                    strP_CO_CUR = Convert.ToInt32(resultado.CO_CUR);
                    strP_CO_MODU_CUR = Convert.ToInt32(resultado.CO_MODU_CUR);
                    strP_CO_TUR = Convert.ToInt32(resultado.CO_TUR);
                    strP_CO_MAT = 0;
                }
                ...
                string NomeFuncionalidadeCadastrada = AuxiliGeral.RetornaNomeFuncionalidadeCadastrada("GEDUC/3000_CtrlOperacionalPedagogico/3500_CtrlDesempenhoEscolar/3599_Relatorios/PlaMedLancInsef.aspx");
                RptPlaMedLancSupremoInsef rpt = new RptPlaMedLancSupremoInsef();
                lRetorno = rpt.InitReport(parametros, infos, strP_CO_EMP_REF, strP_CO_ANO_MES_MAT, strP_CO_CUR, strP_CO_MODU_CUR, strP_CO_TUR, strP_CO_BIMESTRE, strP_CO_MAT, LoginAuxili.CO_EMP, formulario, ddlAgrupPor.SelectedValue, CO_ALU, NomeFuncionalidadeCadastrada, isResponsavel);
                Session["Report"] = rpt;
            ...
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Não foi possível prosseguir com o reatório, por favor tente novamente ou entre em contato com o suporte.");
                return;
            }
        ...
        private void CarregaAlunos()
        {
            ...
            {
                AuxiliCarregamentos.CarregaAlunoComGradeXResponsavel(ddlAluno, LoginAuxili.CO_RESP, true);
            }
