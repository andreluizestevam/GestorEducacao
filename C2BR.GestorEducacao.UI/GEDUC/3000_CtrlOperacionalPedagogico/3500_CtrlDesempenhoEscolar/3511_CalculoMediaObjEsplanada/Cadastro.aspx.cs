...
namespace C2BR.GestorEducacao.UI.GEDUC._3000_CtrlOperacionalPedagogico._3500_CtrlDesempenhoEscolar._3511_CalculoMediaObjEsplanada
{
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        private void CarregaGrid()
        {
            ...
            foreach (var res in resultado)
            {
                ...
                if (lstNotas.Count() > 0)
                {
                    #region Calcula a nota da prova mensal (PM)
                    var lstN1 = (from n1 in lstNotas
                                 where n1.CO_SIGLA_ATIV == "PM"
                                 select new { n1.VL_NOTA }).OrderByDescending(p => p.VL_NOTA);

                    var lstPartPM = (from ppn in lstNotas
                                     where ppn.CO_SIGLA_ATIV == "PRTPM"
                                     select new { ppn.VL_NOTA }).OrderByDescending(p => p.VL_NOTA);

                    if (lstN1.Count() >= 1)
                    {
                        //dcmN1 = lstN1.Count() >= 3 ? lstN1.Take(3).Sum(p => p.VL_NOTA) : lstN1.Sum(p => p.VL_NOTA);
                        dcmN1 = lstN1.Sum(s => s.VL_NOTA);
                        dcmN1 = dcmN1 > 9 ? 9 : dcmN1;

                        res.NOTA1 = dcmN1.Value.ToString("N2");

                        decimal ppm = 0;
                        if (lstPartPM.Count() >= 1)
                        {
                            ppm = lstPartPM.Sum(s => s.VL_NOTA);
                            ppm = ppm > 1 ? 1 : ppm;
                            res.PARTPM = ppm.ToString("N2");
                        }
                        else
                        {
                            res.PARTPM = "-";
                    ...
                    #region Calcula a nota da prova bimestral (PB)
                    var lstN2 = (from n2 in lstNotas
                                 where n2.CO_SIGLA_ATIV == "PB"
                                 select new { n2.VL_NOTA }).OrderByDescending(p => p.VL_NOTA);

                    var lstPartPB = (from ppn in lstNotas
                                     where ppn.CO_SIGLA_ATIV == "PRTPB"
                                     select new { ppn.VL_NOTA }).OrderByDescending(p => p.VL_NOTA);
                    ...
                    if (lstN2.Count() >= 1)
                    {
                        ...
                        dcmN2 = dcmN2 > 9 ? 9 : dcmN2;

                        res.NOTA2 = dcmN2.Value.ToString("N2");

                        decimal ppb = 0;
                        if (lstPartPB.Count() >= 1)
                        {
                            ppb = lstPartPB.Sum(s => s.VL_NOTA);
                            ppb = ppb > 1 ? 1 : ppb;
                            res.PARTPB = ppb.ToString("N2");
                        }
                        else
                        {
                            res.PARTPB = "-";
                    ...
                    #region Calcula a nota do testão (TT)
                    var lstN3 = (from n3 in lstNotas
                                 where n3.CO_SIGLA_ATIV == "TT"
                    ...
                    #region Calcula a média parcial (MP)
                    decimal mp = 0;
                    c = 0;
                    so = 0;
                    ...
                    if (dcmN3 != null) { c++; so += dcmN3.Value; }
                    ...
                    #region Calcula a nota da recuperação
                    var lstNS = (from ns in lstNotas
                                 where ns.CO_SIGLA_ATIV == "REC"
                                 select new { ns.VL_NOTA }).OrderByDescending(p => p.VL_NOTA);

                    if (lstNS.Count() >= 1)
                    {
                        dcmNS = lstNS.Sum(p => p.VL_NOTA);
                    ...
                    #region Calcula a média (MF)
                    decimal mf = 0;
                    c = 0;
                    so = 0;

                    if (mp != 0) { c++; so += mp; }
                    if (dcmNS != null) { c++; so += dcmNS.Value; }
                    ...
                    if (c != 0)
                    {
                        ...
                        res.MEDIA = mf.ToString("N2");

                        switch (ddlBimestre.SelectedValue) 
                        {
                            case "B1":
                                res.TP = (mf * 2).ToString("N2");
                                break;
                            case "B2":
                                res.TP = (mf * 2).ToString("N2");
                                break;
                            case "B3":
                                res.TP = (mf * 3).ToString("N2");
                                break;
                            case "B4":
                                res.TP = (mf * 3).ToString("N2");
                                break;
        ...
        public class NotasAluno
        {
            ...
            public string PARTPM { get; set; }
            public string NOTA2 { get; set; }
            public string PARTPB { get; set; }
