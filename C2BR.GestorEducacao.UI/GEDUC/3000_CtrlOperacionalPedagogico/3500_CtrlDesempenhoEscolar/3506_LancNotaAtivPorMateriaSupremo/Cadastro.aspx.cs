...
namespace C2BR.GestorEducacao.UI.GEDUC._3000_CtrlOperacionalPedagogico._3500_CtrlDesempenhoEscolar._3506_LancNotaAtivPorMateriaSupremo
{
    public partial class Cadastro : System.Web.UI.Page
    {
            ...
            foreach (GridViewRow linha in grdBusca.Rows)
            {
                ...
                if (((TextBox)linha.Cells[2].FindControl("txtNota")).Text != "")
                {
                    if (decimal.TryParse(((TextBox)linha.Cells[2].FindControl("txtNota")).Text, out dcmMedia))
                    {
                        ...
                        decimal vlNota = decimal.Parse(((TextBox)linha.Cells[2].FindControl("txtNota")).Text);
                        ...
                        if (ddlLanctoTM.SelectedValue == "S") //Se for lançamento para todas as disciplinas
                            SalvaNotaTodasDisciplinas(modalidade, serie, anoRef, LoginAuxili.CO_EMP, coAlu, dataAtiv
                                , coTipoAtiv, turma, vlNota, noAluno);
                        else //Se não for lançamento para todas as disciplinas
                        {
                            #region individual
                ...
                {
                    //Apenas se o lançamento para todas as disciplinas estiver configurado como NÃO
                    if (ddlLanctoTM.SelectedValue == "N")
                    {
                        //------------> Recebe o código do aluno
                        int coAlu = Convert.ToInt32(grdBusca.DataKeys[linha.RowIndex].Values[0]);
        ...
        private void SalvaNotaTodasDisciplinas(int modalidade, int serie, string anog, int coem, int coAlu,
            DateTime dataAtiv, int coTipoAtiv, int turma, decimal nota, string noAluno)
        {
        ...
        protected void btnPesqGride_Click(object sender, EventArgs e)
        {
            ...
            if (ddlLanctoTM.SelectedValue == "N")
                CarregaGrid();
            else //Se for lançamento múltiplo
        ...
        protected void ddlLanctoTM_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlLanctoTM.SelectedValue == "S")
            {
