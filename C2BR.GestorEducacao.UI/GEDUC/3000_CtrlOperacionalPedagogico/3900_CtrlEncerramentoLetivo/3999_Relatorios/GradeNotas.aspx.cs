...
// OBJETIVO: GRADE DE NOTAS DO ANO LETIVO
// DATA DE CRIAÇÃO: 
//-----------------------------------------------------------------------------
...
// ----------+----------------------------+-------------------------------------
// 02/05/2013| André Nobre Vinagre        |Adicionado relatório delphi para o cliente Barão
//           |                            |
...
namespace C2BR.GestorEducacao.UI.GEDUC.F3000_CtrlOperacionalPedagogico.F3900_CtrlEncerramentoLetivo.F3999_Relatorios
{
    public partial class GradeNotas : System.Web.UI.Page
    {
...
//--------> Variáveis de parâmetro do Relatório
            ...
            strNomeRelatorio = WRAuxiliares.GeraNomeRelatorio("RelGradeNotasAluno");
...
//--------> Lança nas variáveis dos parâmetros os valores escolhidos no formulário
            ...
            strP_CO_ALU = ddlAlunos.SelectedValue != "T" ? ddlAlunos.SelectedValue : "0";
            infos = AuxiliRelatorioTemporario.GeraIdentFuncionarioRelatorio(Request.UserHostAddress);

            strParametrosRelatorio = "(Ano referência: " + ddlAnoRefer.SelectedItem.ToString() + " - Módulo: " + ddlModalidade.SelectedItem.ToString() + " - Série: " + ddlSerieCurso.SelectedItem.ToString() +
                          " - Turma: " + ddlTurma.SelectedItem.ToString() + ")";

            lIRelatorioWeb = varRelatorioWeb.CreateChannel();

            RptGradeNotas rpt = new RptGradeNotas();

            lRetorno = rpt.InitReport(strParametrosRelatorio, LoginAuxili.CO_EMP, infos, int.Parse(ddlUnidade.SelectedValue), strP_CO_ANO_REF, int.Parse(strP_CO_MODU_CUR), int.Parse(strP_CO_CUR), int.Parse(strP_CO_TUR), int.Parse(strP_CO_ALU));
            Session["Report"] = rpt;
        ...
        private void CarregaAluno()
        {
            ...
            if (turma != 0)
            {
                ddlAlunos.DataSource = (from tb48 in TB48_GRADE_ALUNO.RetornaTodosRegistros()
                                        where tb48.TB44_MODULO.CO_MODU_CUR == modalidade && tb48.CO_CUR == serie && tb48.CO_ANO_MES_MAT == anoMesMat
                                        && tb48.CO_TUR == turma
                                        select new { tb48.TB07_ALUNO.CO_ALU, tb48.TB07_ALUNO.NO_ALU }).Distinct().OrderBy( a => a.NO_ALU );
