...
{
    public partial class AtaResultadosFinais : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                CarregaSerieCurso();
                CarregaTurmas();
                txtDtEmiss.Text = DateTime.Now.ToString();
            }
        }
        ...
        void PadraoRelatoriosCorrente_OnAcaoGeraRelatorio()
        {
            ...
            var res = (from adm in ADMMODULO.RetornaTodosRegistros()
                       where adm.nomURLModulo == "GEDUC/3000_CtrlOperacionalPedagogico/3900_CtrlEncerramentoLetivo/3999_Relatorios/AtaResultadosFinais.aspx"
                       select new { adm.nomItemMenu }).FirstOrDefault();
            NO_RELATORIO = (res != null ? res.nomItemMenu : "");
            #endregion

            RptAtaResultadosFinais rpt = new RptAtaResultadosFinais();
            lRetorno = rpt.InitReport(parametros, infos, LoginAuxili.CO_EMP, ddlAno.SelectedValue, codEmp, coModuCur, CoCur, CoTur, DateTime.Parse(txtDtEmiss.Text), NO_RELATORIO.ToUpper());
            Session["Report"] = rpt;
