...
//
//05/03/2015 | Maxwell Pereira Almeida    | Criação da funcionalidade para emissão do diário
//           |                            | com a segunda página com o modelo de notas do Específico
//
//
...
{
    public partial class DiarioBimestralEspecifico : System.Web.UI.Page
    {
        ...
        void PadraoRelatoriosCorrente_OnAcaoGeraRelatorio()
        {
            ...
            else
            {
                ...
                strMateria = ddlMateria.SelectedItem.ToString().ToUpper();
                ...
                if (tbResponMat != null)
                {
                    strProfessorCod = tbResponMat.CO_MAT_COL.ToString().ToUpper();
                    strProfessor = tbResponMat.NO_COL.ToUpper();
                }
                else
                {
                    strProfessor = ddlProfessor.SelectedItem.Text.ToUpper();
                    strProfessorCod = ddlProfessor.SelectedItem.Value.ToString().ToUpper();
                }
            }
            DateTime dtIniBim, dtFimBim;

            DateTime.TryParse(hidDtIniRef.Value, out dtIniBim);
            DateTime.TryParse(hidDtFimRef.Value, out dtFimBim);
            ...
            #region Trata o Tipo do Diário

            ETipoDiario tpDiario;
            switch (ddlTipoDiario.SelectedValue)
            {
                case "2":
                    tpDiario = ETipoDiario.SemFolhaResumo;
                    break;
                case "3":
                    tpDiario = ETipoDiario.ApenasFolhaResumo;
                    break;
                case "1":
                default:
                    tpDiario = ETipoDiario.Completo;
                    break;
            }
        ...
        private void CarregaMaterias(bool Relatorio = false)
        {
            ...
            if ((serie != 0) && (modalidade != 0) && (turma != 0) && (coEmp != 0))
            {
                ...
                if (tipoFreq == "M")
                {
                    ...
                    {
                        if (LoginAuxili.FLA_PROFESSOR != "S")
                            AuxiliCarregamentos.CarregaMateriasGradeCurso(ddlMateria, coEmp, modalidade, serie, anoGrade, Relatorio, true, false,true);
                        else
                            AuxiliCarregamentos.CarregaMateriasProfeRespon(ddlMateria, LoginAuxili.CO_COL, modalidade, serie, anoInt, Relatorio, true, false, true);
