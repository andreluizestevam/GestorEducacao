...
namespace C2BR.GestorEducacao.UI.GEDUC._5000_CtrlFinanceira._5200_CtrlReceitas._5205_BaixaTituloBoleto
{
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        protected void btnPesquisar_Click(object sender, EventArgs e)
        {
            ...
            var resultado = (from r in TB321_ARQ_RET_BOLETO.RetornaTodosRegistros()
                                    ...
                                    && (flag != "T" ? r.FL_STATUS == flag : (r.FL_STATUS == "P" || r.FL_STATUS == "C" || r.FL_STATUS == "I"))
                                    select new
                                    {
                                        ...
                                        Status = r.FL_STATUS == "P" ? "Pendente de Baixa" : r.FL_STATUS == "C" ? "Cancelado" : "Inconsistente",
        ...
        protected void btnGerar_Click(object sender, EventArgs e)
        {
            ...
            //             join c in ctx.TB47_CTA_RECEB on new { b.CO_EMP, b.NU_DOC, b.NU_PAR, b.DT_CAD_DOC } equals new { c.CO_EMP, c.NU_DOC, c.NU_PAR, c.DT_CAD_DOC }
            //             where lstNossoNumero.Contains(b.CO_NOS_NUM)
            //             //&& b.IC_SIT_DOC == "A"
            //             select c;

            var lstTit = from c in ctx.TB47_CTA_RECEB
                         where lstNossoNumero.Contains(c.CO_NOS_NUM)
                         //&& b.IC_SIT_DOC == "A"
                         select c;
            ...
            foreach (var tit in lstTit)
            {
                if (tit.IC_SIT_DOC == "A")
                {                 
                    var boleto = ctx.TB321_ARQ_RET_BOLETO.FirstOrDefault(x => x.NU_NOSSO_NUMERO == tit.CO_NOS_NUM);
                    if (boleto == null)
                        continue;
                    ...
                    tit.VR_PAG = boleto.VL_PAGO;
                    tit.CO_COL_BAIXA = LoginAuxili.CO_COL;
                    tit.VR_JUR_PAG = boleto.VL_JUROS;
                    tit.VR_MUL_PAG = boleto.VL_MULTA;
                    tit.VR_DES_PAG = boleto.VL_DESCTO;
                    tit.FL_ORIGEM_PGTO = "B";
                    tit.FL_TIPO_RECEB = "B";

                    var nosNum = ctx.TB045_NOS_NUM.FirstOrDefault(x => x.CO_EMP == tit.CO_EMP && x.NU_DOC == tit.NU_DOC && x.NU_PAR == tit.NU_PAR && x.DT_CAD_DOC == tit.DT_CAD_DOC && lstNossoNumero.Contains(x.CO_NOS_NUM));
