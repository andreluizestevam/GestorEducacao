    ...
    {        
        ...
        protected void CurrentPadraoBuscas_OnAcaoBuscaDefineGridView()
        {
            ...
            var resultado = (from tb64 in TB64_SOLIC_ATEND.RetornaTodosRegistros()
                            join tb07 in TB07_ALUNO.RetornaTodosRegistros() on tb64.CO_ALU equals tb07.CO_ALU into alunos
                             from tb07 in alunos.DefaultIfEmpty()
                            where tb07 != null
                            && tb64.CO_EMP == LoginAuxili.CO_EMP 
                            && (modalidade != 0 ? tb64.CO_MODU_CUR == modalidade : modalidade == 0)
                            && (serie != 0 ? tb64.CO_CUR == serie : serie == 0)                            
                            && (turma != 0 ? tb64.CO_TUR == turma : turma == 0)                            
                            select new
                            {
                                tb07.NU_NIRE, 
                                tb64.CO_CUR, 
                                ...
                                tb07.NO_ALU, 
                                tb64.CO_EMP,
                                CO_TELE_CONT = (tb64.CO_TELE_CONT.Length == 10 ? tb64.CO_TELE_CONT.Insert(6, "-").Insert(2, " ").Insert(2, ")").Insert(0, "(") : tb64.CO_TELE_CONT),
                                tb64.DT_SOLI_ATEN, 
                                tb64.DT_PREV_ENTR, 
                                tb64.CO_SOLI_ATEN, 
                                tb64.NU_DCTO_SOLIC,
                                CO_SIT = (tb64.CO_SIT == "A" ? "Em Aberto" : (tb64.CO_SIT == "F" ? "Finalizada" : "Cancelada"))
                            }).OrderByDescending(s => s.DT_SOLI_ATEN).ThenBy(d => d.CO_ALU).Distinct();
            

            if (resultado.Count() > 0)
            {
                var res = resultado.Where(p => !resultado.Any(r => (p != r && p.CO_SOLI_ATEN == r.CO_SOLI_ATEN)));
                if (res.Count() > 0)
