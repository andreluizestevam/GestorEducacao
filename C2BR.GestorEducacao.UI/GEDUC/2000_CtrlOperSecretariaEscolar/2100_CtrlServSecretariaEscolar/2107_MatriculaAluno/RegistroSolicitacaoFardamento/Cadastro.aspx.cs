...
// OBJETIVO: REGISTRO DE SOLICITAÇÃO DE UNIFORME/FARDAMENTO
// DATA DE CRIAÇÃO: 
//-----------------------------------------------------------------------------
...
namespace C2BR.GestorEducacao.UI.GEDUC.F2000_CtrlOperSecretariaEscolar.F2100_CtrlServSecretariaEscolar.F2107_MatriculaAluno.RegistroSolicitacaoFardamento
{
...
////--------> Varre toda a grid de Uniforme
//            foreach (GridViewRow linha in grdUniforme.Rows)
//            {
...
//            GestorEntities.CurrentContext.SaveChanges();
        ...
        /// <summary>
        /// Método que carrega o grid Uniformes
        /// </summary>
        private void CarregaGridDocumentos()
        {
            grdUniforme.DataSource = (from tb90 in TB90_PRODUTO.RetornaTodosRegistros()
                                      where tb90.TB124_TIPO_PRODUTO.CO_TIP_PROD == 2 && tb90.CO_SITU_PROD == "A" && tb90.TB25_EMPRESA.CO_EMP == LoginAuxili.CO_EMP
                                      select new
                                      {
                                          tb90.NO_PROD_RED, tb90.CO_PROD, tb90.TB98_TAMANHO.DES_TAMANHO, tb90.VL_UNIT_PROD, tb90.CO_REFE_PROD
                                      }).OrderBy( p => p.NO_PROD_RED );
            grdUniforme.DataBind();
        }
...
//--------> Varre toda a grid de Uniforme
            foreach (GridViewRow linha in grdUniforme.Rows)
            {
                HiddenField hdVlrAtiv = ((HiddenField)linha.Cells[2].FindControl("hdValor"));
