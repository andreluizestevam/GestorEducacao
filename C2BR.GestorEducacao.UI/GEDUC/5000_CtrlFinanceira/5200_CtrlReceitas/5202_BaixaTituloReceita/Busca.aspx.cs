        ...
        void CurrentPadraoBuscas_OnAcaoBuscaDefineGridView()
        {
            ...
            if (strTipoFonte == "A" && ddlNomeFonte.SelectedValue != "")
            {
                var resultado = (from tb47 in VW47_CTA_RECEB.RetornaTodosRegistros()
                                 ...
                                 where 
                                 //(tb47.NU_DOC == txtNumeroDocumento.Text || string.IsNullOrEmpty(txtNumeroDocumento.Text))
                                 (!string.IsNullOrEmpty(txtNumeroDocumento.Text) ? tb47.NU_DOC.Contains(txtNumeroDocumento.Text) : 0 == 0)
                                 ...
                                 && (coNomeFonte != 0 ? tb47.CO_ALU == coNomeFonte : 0 == 0)
                                 && tb47.CO_EMP == LoginAuxili.CO_EMP && tb47.IC_SIT_DOC != "C"
                                 && tb47.IC_SIT_DOC != "Q"
                                 select new
                                 {
            ...
            else if (strTipoFonte == "O" && ddlNomeFonte.SelectedValue != "")
            {
                var resultado = (from tb47 in VW47_CTA_RECEB.RetornaTodosRegistros()
                                 ...
                                 from x in sr.DefaultIfEmpty()
                                 where 
                                 (txtNumeroDocumento.Text != "" ? tb47.NU_DOC.Contains(txtNumeroDocumento.Text) : 0 == 0)
                                && (dataVerifIni == null || (strTipoPeriodo == "V" && tb47.DT_VEN_DOC >= dataVerifIni) ||
                                (strTipoPeriodo == "C" && tb47.DT_CAD_DOC >= dataVerifIni) || (strTipoPeriodo == "E" && tb47.DT_EMISS_DOCTO >= dataVerifIni))
                                && (dataVerifFim == null || (strTipoPeriodo == "V" && tb47.DT_VEN_DOC <= dataVerifFim) ||
                                (strTipoPeriodo == "C" && tb47.DT_CAD_DOC <= dataVerifFim) || (strTipoPeriodo == "E" && tb47.DT_EMISS_DOCTO <= dataVerifFim))
                                && (strSituacao != "" ? tb47.IC_SIT_DOC == strSituacao : 0 == 0) && tb47.IC_SIT_DOC != "Q"
                                && tb47.TP_CLIENTE_DOC == "O" 
                                && (coNomeFonte != 0 ? tb47.CO_CLIENTE == coNomeFonte : 0 == 0)
        ...
        private void CarregaTipoTFR()
        {
            switch (LoginAuxili.CO_TIPO_UNID)
            {
                case "PGS":
                    ...
                    ddlTipoFonte.Items.Insert(0, new ListItem("Todos", "T"));
                    break;
...
//====> Método que carrega o DropDown Nome da Fonte com os Alunos
        private void CarregaAluno()
        {
            ...
            ddlNomeFonte.Items.Insert(0, new ListItem("Todos", ""));
        }
