...
namespace C2BR.GestorEducacao.UI.GEDUC.F5000_CtrlFinanceira.F5200_CtrlReceitas.F5203_BoletoBancarioTitulo
{
    public partial class Busca : System.Web.UI.Page
    {
        ...
        void CurrentPadraoBuscas_OnAcaoBuscaDefineGridView()
        {
            ...
            if (strTpCliente == "A" && LoginAuxili.CO_RESP == 0)
            {
            ...
            {
                var coAlu = int.Parse(ddlAluno.SelectedValue);

                var resultado = (from tb47 in TB47_CTA_RECEB.RetornaTodosRegistros()
                                 join tb07 in TB07_ALUNO.RetornaTodosRegistros() on tb47.CO_ALU equals tb07.CO_ALU
                                 join tb08 in TB08_MATRCUR.RetornaTodosRegistros() on tb07.CO_ALU equals tb08.TB07_ALUNO.CO_ALU
                                 join tb108 in TB108_RESPONSAVEL.RetornaTodosRegistros() on tb08.TB108_RESPONSAVEL.CO_RESP equals tb108.CO_RESP
                                 where (coAlu != 0 ? tb07.CO_ALU == coAlu : coAlu == 0)
                                 && tb47.CO_EMP == coEmp
                                 && tb108.CO_RESP == LoginAuxili.CO_RESP
                                 && (tb47.IC_SIT_DOC == "A" || tb47.IC_SIT_DOC == "R") && (tb47.TP_CLIENTE_DOC == "A" || tb47.TP_CLIENTE_DOC == "R")
                                 select new
                                 {
                                     tb07.TB25_EMPRESA1.CO_EMP,
        ...
        private void CarregaTipoTFR()
        {
            switch (LoginAuxili.CO_TIPO_UNID)
            {
                case "PGS":
                    ddlTipo.Items.Clear();
                    ddlTipo.Items.Insert(0, new ListItem("Não Paciente", "O"));
                    ddlTipo.Items.Insert(0, new ListItem("Paciente", "A"));
                    break;
                ...
                default:
                    ...
                    {
                        ddlTipo.Items.Clear();
                        ddlTipo.Items.Insert(0, new ListItem("Não Aluno", "O"));
                        ddlTipo.Items.Insert(0, new ListItem("Aluno", "A"));
                    }
                    else
                    {
                        ddlTipo.Items.Clear();
        ...
        private void CarregaAluno() {
            int resp = LoginAuxili.CO_RESP;

            ddlAluno.DataSource = from tb08 in TB08_MATRCUR.RetornaTodosRegistros()
                            where tb08.TB108_RESPONSAVEL.CO_RESP == resp
                            select new { 
            ...
            liUnidadeContrato.Visible = false;
