    ...
    public partial class Cadastro : System.Web.UI.Page
    {
            ...
            foreach (GridViewRow linha in grdFonte.Rows)
            {
                ...
                if (((CheckBox)linha.FindControl("chkSelect")).Checked)
                {
                    ...
                    if (rbNovo.Checked)
                    {
                        ...
                        var t47ob = (from lsttb47 in TB47_CTA_RECEB.RetornaTodosRegistros()
                                     where lsttb47.NU_DOC == strNudoc
                                     select new { lsttb47.CO_NOS_NUM }).FirstOrDefault();

                        //Verifica se o título em questão já existe um Nosso Número associado.
                        if (t47ob == null)
                        {
                    ...
                    decimal desconto = ((!tb47.VR_DES_DOC.HasValue ? 0
                    ...
                    boleto.Valor = tb47.VR_PAR_DOC - desconto;
                    ...
                    if (tb227.FL_DESC_DIA_UTIL == "S" && tb227.VL_DESC_DIA_UTIL.HasValue)
                    {
                        var desc = boleto.Valor - tb227.VL_DESC_DIA_UTIL.Value;
                    ...
                    {
                        boleto.Instrucoes += "<br> Valor " + tpdoc + ":" + " R$" + (tb47.VR_PAR_DOC - desconto).ToString("N2");
                    }
                    else
                    {
                        boleto.Instrucoes += "<br> Valor " + tpdoc + ":" + " R$" + (tb47.VR_PAR_DOC - desconto).ToString("N2") + " + " + " R$" + tb227.VR_TX_BOL_CLI.Value.ToString("N2") + "(Taxa de Emissão do Boleto) ";
                    }
                    #endregion
        ...
        private void CarregaFormulario()
        {
            ...
            if (tipo == "A")
            {
                ...
                txtCodigo.Text = aluno.NU_CPF_RESP.Length != 11 ? aluno.NU_CPF_RESP : aluno.NU_CPF_RESP.Insert(9, "-").Insert(6, ".").Insert(3, ".");
            ...
            if (tipo != "A")
            {
                ...
                if (rbTT.Checked)
                {
                    ...
                    grdFonte.DataSource = (from c in TB47_CTA_RECEB.RetornaTodosRegistros()
                                           ...
                                           from x in sr.DefaultIfEmpty()
                                           where (c.TB103_CLIENTE.CO_CLIENTE == id) &&
                                            c.CO_EMP == coemp && c.FL_EMITE_BOLETO == "S"
                                           select new
                                           {
                                               c.CO_EMP,
                                               c.NU_DOC,
