...
//----------------> Retorna a lista de Contas lançadas para a Despesa fixa selecionada
                    List<TB38_CTA_PAGAR> lstTb38 = (from lTb38 in TB38_CTA_PAGAR.RetornaTodosRegistros()
                                                    where lTb38.CO_EMP == tb37.CO_EMP
                                                    && lTb38.NU_DOC == tb37.CO_CON_RECDES
                                                    select lTb38).ToList();
...
//----------------> Se todas as contas estiverem com o status "Em aberto", exclui as mesmas e depois exclui a Despesa fixa
            ...
            if (Page.IsValid)
            {
                ...
                if (tb37 == null)
                {
                    tb37 = new TB37_RECDES_FIXA();
                    tb37.CO_CON_RECDES = txtNumContrato.Text;
                    tb37.TB25_EMPRESA = TB25_EMPRESA.RetornaPelaChavePrimaria(LoginAuxili.CO_EMP);
                    tb37.TB000_INSTITUICAO = TB000_INSTITUICAO.RetornaPelaChavePrimaria(LoginAuxili.ORG_CODIGO_ORGAO);
                    tb37.TB41_FORNEC = TB41_FORNEC.RetornaPelaChavePrimaria(int.Parse(ddlNomeFantasia.SelectedValue));
...
//------------> Despesa Fixa (Dédito = "D")
                ...
                if (TB37_RECDES_FIXA.SaveOrUpdate(tb37, true) > 0)
                {
                    var lstTb38 = (from lTb38 in TB38_CTA_PAGAR.RetornaTodosRegistros()
                                                    where lTb38.CO_EMP.Equals(tb37.CO_EMP) && lTb38.NU_DOC.Equals(tb37.CO_DOC_RECDES)
                                                    select lTb38).ToList();
...
//----------------> Lança cada parcela no Contas a Pagar
                    for (int i = 0; i < tb37.QT_PARC_RECDES; i++)
                    {
                        ...
                        int diasIntervalo;
                        int.TryParse(tb37.NU_DIA_INTER_RECDES.ToString(), out diasIntervalo);

                        if (QueryStringAuxili.OperacaoCorrenteQueryString.Equals(QueryStrings.OperacaoInsercao))
                        {
                            lstTb38[i].TB25_EMPRESA = TB25_EMPRESA.RetornaPelaChavePrimaria(tb37.TB25_EMPRESA.CO_EMP);
                            lstTb38[i].CO_EMP = tb37.CO_EMP;
                            lstTb38[i].NU_PAR = i + 1;
                            lstTb38[i].QT_PAR = tb37.QT_PARC_RECDES;
                            lstTb38[i].TB000_INSTITUICAO = TB000_INSTITUICAO.RetornaPelaChavePrimaria(tb37.TB000_INSTITUICAO.ORG_CODIGO_ORGAO);
                            lstTb38[i].NU_DOC = tb37.CO_DOC_RECDES;
                            lstTb38[i].DT_CAD_DOC = tb37.DT_CAD_RECDES;
                        }
                        ...
                        lstTb38[i].VR_TOT_DOC = tb37.VR_RECDES;
                        lstTb38[i].CO_CON_DESFIX = tb37.CO_CON_RECDES;
                        lstTb38[i].VR_PAR_DOC = tb37.VR_RECDES / tb37.QT_PARC_RECDES;
                        lstTb38[i].DT_VEN_DOC = i.Equals(0) ? tb37.DT_VENC_PRIM_PARC : tb37.DT_VENC_PRIM_PARC.AddDays(diasIntervalo * i);
                        lstTb38[i].DT_EMISS_DOCTO = tb37.DT_INI_CON_RECDES;
                        lstTb38[i].TB39_HISTORICO = tb37.TB39_HISTORICO;
                        lstTb38[i].TB086_TIPO_DOC = tb37.TB086_TIPO_DOC;
                        lstTb38[i].TB099_CENTRO_CUSTO = tb37.TB099_CENTRO_CUSTO;
                        lstTb38[i].VR_MUL_DOC = tb37.VR_MUL_RECDES;
                        lstTb38[i].VR_JUR_DOC = tb37.VR_JUR_RECDES;
                        lstTb38[i].VR_DES_DOC = tb37.VR_DES_RECDES;
                        lstTb38[i].CO_FLAG_TP_VALOR_MUL = tb37.CO_FLAG_TP_VALOR_MUL;
                        lstTb38[i].CO_FLAG_TP_VALOR_JUR = tb37.CO_FLAG_TP_VALOR_JUR;
                        lstTb38[i].CO_FLAG_TP_VALOR_DES = tb37.CO_FLAG_TP_VALOR_DES;
                        lstTb38[i].CO_FLAG_TP_VALOR_DES_ANTEC = "V";
                        lstTb38[i].IC_SIT_DOC = tb37.IC_SIT_RECDES;
                        lstTb38[i].TB41_FORNEC = tb37.TB41_FORNEC;
                        lstTb38[i].TB56_PLANOCTA = tb37.TB56_PLANOCTA;
        ...
        private void CarregaFormulario()
        {
            ...
            if (tb37 != null)
            {
                ...
                ddlDepartamento.SelectedValue = tb37.TB099_CENTRO_CUSTO != null && tb37.TB099_CENTRO_CUSTO.TB14_DEPTO != null ? tb37.TB099_CENTRO_CUSTO.TB14_DEPTO.CO_DEPTO.ToString() : string.Empty;
                CarregaCentroCusto();
                ...
                txtNumAditivo.Text = tb37.CO_ADITI_RECDES.ToString();
                txtNumPublicacao.Text = tb37.NU_PUBLI_RECDES;
                ddlTipoDocumento.SelectedValue = tb37.TB086_TIPO_DOC != null ? tb37.TB086_TIPO_DOC.SIG_TIPO_DOC.ToString() : "";
                txtNumDoc.Text = tb37.CO_DOC_RECDES;
                txtDtInicioContrato.Text = tb37.DT_INI_CON_RECDES.ToString("dd/MM/yyyy");
                txtDataPublicacao.Text = tb37.DT_PUBLI_RECDES != null ? tb37.DT_PUBLI_RECDES.Value.ToString("dd/MM/yyyy") : "";
                txtDtFimContrato.Text = tb37.DT_FIM_CON_RECDES.ToString("dd/MM/yyyy");
                txtQtParcelas.Text = tb37.QT_PARC_RECDES.ToString();
                txtDtPrimeiroVencto.Text = tb37.DT_VENC_PRIM_PARC.ToString("dd/MM/yyyy");
                txtDiasIntervalo.Text = tb37.NU_DIA_INTER_RECDES.ToString();
                txtValorDocumento.Text = tb37.VR_RECDES.ToString();
                txtDtCancelamento.Text = tb37.DT_CAN_RECDES != null ? tb37.DT_CAN_RECDES.Value.ToString("dd/MM/yyyy") : "";
                txtMulta.Text = tb37.VR_MUL_RECDES.ToString();
                txtJuros.Text = tb37.VR_JUR_RECDES.ToString();
                txtDesconto.Text = tb37.VR_DES_RECDES.ToString();
                cbFlagPercentualMulta.Checked = tb37.CO_FLAG_TP_VALOR_MUL.Equals("P");
                cbFlagPercentualJuros.Checked = tb37.CO_FLAG_TP_VALOR_JUR.Equals("P");
                cbFlagPercentualDesconto.Checked = tb37.CO_FLAG_TP_VALOR_DES.Equals("P");
                txtDtCadastro.Text = tb37.DT_CAD_RECDES.ToString("dd/MM/yyyy");
                ddlStatus.SelectedValue = tb37.IC_SIT_RECDES;
                txtDtStatus.Text = tb37.DT_ALT_REGISTRO != null ? tb37.DT_ALT_REGISTRO.Value.ToString("dd/MM/yyyy") : "";
            }
        }
        ...
        private void CarregaTipoReceita()
        {
            ...
            ddlTipoReceita.SelectByText("Despesa");
        ...
        /// <summary>
        /// Método que carrega o dropdown de Tipos de Documentos
        /// </summary>
