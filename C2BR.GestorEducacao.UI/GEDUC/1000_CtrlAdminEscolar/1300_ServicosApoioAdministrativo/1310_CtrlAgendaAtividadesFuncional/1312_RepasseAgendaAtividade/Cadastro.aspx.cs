...
// OBJETIVO: REPASSE DE ATIVIDADES FUNCIONAIS AGENDADAS
// DATA DE CRIAÇÃO: 
//-----------------------------------------------------------------------------
...
namespace C2BR.GestorEducacao.UI.GEDUC.F1000_CtrlAdminEscolar.F1300_ServicosApoioAdministrativo.F1310_CtrlAgendaAtividadesFuncional.F1312_RepasseAgendaAtividade
{
...
//====> Processo de Inclusão, Alteração e Exclusão de Registros na Entidade do BD, após a ação de salvar
        void CurrentPadraoCadastros_OnAcaoBarraCadastro()
        {
            ...
            int coCol = ddlNomeResponsavelRepasse.SelectedValue != "" ? int.Parse(ddlNomeResponsavelRepasse.SelectedValue) : 0;
...
//------------> Informaçoes do funcionário que solicitou a tarefa
                ...
                tb137.DT_COMPR_TAREF_AGEND = DateTime.Parse(txtDtRepasse.Text);
                tb137.DT_LIMIT_TAREF_AGEND = DateTime.Parse(txtDtLimiteRepasse.Text);
                tb137.TB140_PRIOR_TAREF_AGEND = TB140_PRIOR_TAREF_AGEND.RetornaPelaChavePrimaria(ddlPrioridadeRepasse.SelectedValue);
...
//------------> Atualiza os dados do colaborador que repassou a tarefa no registro da tarefa original
                ...
                tarefaOriginal.DE_MOTIV_REPAS_TAREF_AGEND = txtMotivoRepasse.Text;
                tarefaOriginal.DT_REPAS_TAREF_AGEND = dataAtual;
                tarefaOriginal.TB139_SITU_TAREF_AGEND = TB139_SITU_TAREF_AGEND.TR;
...
//---------> TB03_COLABOR1 = EMISSOR DA TAREFA
        ...
        {
            if (ddlUnidade.SelectedValue.Equals(LoginAuxili.CO_EMP.ToString()) && ddlNomeResponsavelRepasse.SelectedValue.Equals(LoginAuxili.CO_COL.ToString()))
            {
        ...
        protected void ddlUnidade_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ddlNomeResponsavelRepasse.DataSource = (from tb03 in TB03_COLABOR.RetornaPeloCoUnid(coEmp)
                                                    select new { tb03.CO_COL, tb03.NO_COL }).OrderBy( c => c.NO_COL );

            ddlNomeResponsavelRepasse.DataValueField = "CO_COL";
            ddlNomeResponsavelRepasse.DataTextField = "NO_COL";
            ddlNomeResponsavelRepasse.DataBind();
        ...
        protected void cvDataRepasse_ServerValidate(object source, ServerValidateEventArgs e)
        {
            if (DateTime.Parse(txtDtCompromisso.Text) > DateTime.Parse(txtDtRepasse.Text))
            {
                e.IsValid = false;
        ...
        protected void cvDataLimiteRepasse_ServerValidate(object source, ServerValidateEventArgs e)
        {
            if (DateTime.Parse(txtDtLimiteRepasse.Text).Date < DateTime.Parse(txtDtLimite.Text).Date)
            {
                e.IsValid = false;
                AuxiliPagina.EnvioMensagemErro(this, "");
            }
        ...
        protected void cvCelularResponsavel_ServerValidate(object source, ServerValidateEventArgs e)
        {
            ...
            {
                string strCelular = TB03_COLABOR.RetornaPeloCoCol(int.Parse(ddlNomeResponsavelRepasse.SelectedValue)).NU_TELE_CELU_COL;
