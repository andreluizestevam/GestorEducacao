...
//--------> Criação das instâncias utilizadas
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (QueryStringAuxili.OperacaoCorrenteQueryString.Equals(QueryStrings.OperacaoAlteracao))
                ddlTipoConta.Enabled = ddlGrupo.Enabled = false;
        }
...
//====> Processo de Inclusão, Alteração e Exclusão de Registros na Entidade do BD, após a ação de salvar
        void CurrentPadraoCadastros_OnAcaoBarraCadastro()
        {
            ...
            {
                int nuSGrp = int.Parse(txtNumSubGrupo.Text);
                int nuGrp = int.Parse(ddlGrupo.SelectedValue);

                int ocorrSGrp = (from lTb54 in TB54_SGRP_CTA.RetornaTodosRegistros()
                                where lTb54.TB53_GRP_CTA.TP_GRUP_CTA.Equals(ddlTipoConta.SelectedValue)
                                && lTb54.TB53_GRP_CTA.CO_GRUP_CTA == nuGrp
                                && lTb54.NR_SGRUP_CTA == nuSGrp
                                 select new { lTb54.CO_SGRUP_CTA }).Count();

                if (ocorrSGrp > 0)
                {
                    AuxiliPagina.EnvioMensagemErro(this.Page, "Número de subgrupo já cadastrado para tipo e grupo de conta informado.");
                    return;
                }
            }
            else if (QueryStringAuxili.OperacaoCorrenteQueryString.Equals(QueryStrings.OperacaoAlteracao))
            {
                int nuSGrp = int.Parse(txtNumSubGrupo.Text);
                int nuGrp = int.Parse(ddlGrupo.SelectedValue);
                int idSGrp = QueryStringAuxili.RetornaQueryStringComoIntPelaChave(QueryStrings.Id);

                int ocorrSGrp = (from lTb54 in TB54_SGRP_CTA.RetornaTodosRegistros()
                                 where lTb54.TB53_GRP_CTA.TP_GRUP_CTA.Equals(ddlTipoConta.SelectedValue)
                                 && lTb54.TB53_GRP_CTA.CO_GRUP_CTA == nuGrp
                                 && lTb54.NR_SGRUP_CTA == nuSGrp && lTb54.CO_SGRUP_CTA != idSGrp
                                 select new { lTb54.CO_SGRUP_CTA }).Count();
            ...
            TB54_SGRP_CTA tb54 = RetornaEntidade();

            if (tb54 == null)
                tb54 = new TB54_SGRP_CTA();

            if (QueryStringAuxili.OperacaoCorrenteQueryString.Equals(QueryStrings.OperacaoInsercao))
            {
                tb54.CO_GRUP_CTA = int.Parse(ddlGrupo.SelectedValue);
                tb54.TB53_GRP_CTA = TB53_GRP_CTA.RetornaPelaChavePrimaria(tb54.CO_GRUP_CTA);
            }

            tb54.DE_SGRUP_CTA = txtDescricaoSubGrupo.Text;
            tb54.NR_SGRUP_CTA = int.Parse(txtNumSubGrupo.Text);
            tb54.DT_ALT_REGISTRO = DateTime.Now;

            CurrentPadraoCadastros.CurrentEntity = tb54;
        }
        #endregion
        ...
        {            
            ...
            if (tb54 != null)
            {
                ...
                CarregaGrupo();
                ddlGrupo.SelectedValue = tb54.CO_GRUP_CTA.ToString();
                txtDescricaoSubGrupo.Text = tb54.DE_SGRUP_CTA;
                txtNumSubGrupo.Text = tb54.NR_SGRUP_CTA != null ? tb54.NR_SGRUP_CTA.ToString().PadLeft(3,'0') : "";
            }
        }
        ...
        private TB54_SGRP_CTA RetornaEntidade()
        {
            return TB54_SGRP_CTA.RetornaPelaChavePrimaria(QueryStringAuxili.RetornaQueryStringComoIntPelaChave(QueryStrings.Id));
        }
        #endregion   
