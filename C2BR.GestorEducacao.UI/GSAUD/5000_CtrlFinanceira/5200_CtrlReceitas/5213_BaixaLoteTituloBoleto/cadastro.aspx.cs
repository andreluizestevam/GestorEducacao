...
namespace C2BR.GestorEducacao.UI.GSAUD._5000_CtrlFinanceira._5200_CtrlReceitas._5213_BaixaLoteTituloBoleto
{
    public partial class cadastro : System.Web.UI.Page
    {
        ...
        void CurrentPadraoCadastros_OnAcaoBarraCadastro()
        {
            ...
            foreach (GridViewRow linha in grdTitulos.Rows)
            {
                ...
                {
                    decimal multa, juros, desc, outros, total;
                    int numParc;
                    ...
                    decimal.TryParse(((TextBox)linha.Cells[9].FindControl("tbOutros")).Text, out outros);
                    decimal.TryParse(linha.Cells[11].Text, out total);
                    int.TryParse(linha.Cells[2].Text, out numParc);
                    numDoc = linha.Cells[1].Text;
                    DateTime.TryParse(((TextBox)linha.Cells[11].FindControl("tbData")).Text, out dataRecebimento);
                    tipoRec = ((DropDownList)linha.Cells[12].FindControl("ddlTipos")).SelectedValue;
                    ...
                    TBS47_CTA_RECEB conta = TBS47_CTA_RECEB.RetornaPrimeiroRegistroPeloCoEmpNuDocNuPar(LoginAuxili.CO_EMP, numDoc, numParc);
                    ...
                    else
                    {
                        ...
                        conta.VL_MUL_PAG = multa;
                        conta.VL_JUR_PAG = juros;
                        conta.VL_DES_PAG = desc;
                        conta.VL_OUT_PAG = outros;
                        conta.VL_PAG = total;

                        TBS47_CTA_RECEB.SaveOrUpdate(conta, false);
                        alterado = true;
                    }
                }
            }
        ...
        protected void btnGerar_Click(object sender, EventArgs e)
        {
            ...
            var lst = (from tbs47 in TBS47_CTA_RECEB.RetornaTodosRegistros()
                      where (tbs47.IC_SIT_DOC == "A" || tbs47.IC_SIT_DOC == "R") 
                      && tbs47.CO_ALU == coAlu
                      && (agrupador != -1 ? tbs47.CO_AGRUP_RECDESP == agrupador : 0 == 0)
                      && (undContrato == 0 ? 0==0 : tbs47.CO_EMP_UNID_CONT == undContrato)
                      && (dataIni != DateTime.MinValue ? (tbs47.DT_VEN_DOC >= dataIni) : 0 == 0)
                      && (dataFim != DateTime.MinValue ? (tbs47.DT_VEN_DOC <= dataFim) : 0 == 0)
                      orderby tbs47.DT_CAD_DOC
                      select tbs47).DefaultIfEmpty().ToList();
            ...
            foreach(TBS47_CTA_RECEB linha in lst)
            {
                ...
                novaLinha.agrupador = linha.CO_AGRUP_RECDESP.HasValue ? TB315_AGRUP_RECDESP.RetornaPelaChavePrimaria(linha.CO_AGRUP_RECDESP.Value).DE_SITU_AGRUP_RECDESP : "";
        ...
        protected void cbMarcar_CheckedChanged(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow linha in grdTitulos.Rows)
            {
                ...
                if (marcado.ClientID == marcador.ClientID)
                {
                    ...
                    ((TextBox)linha.Cells[9].FindControl("tbOutros")).Enabled = habilitar;

                    ((TextBox)linha.Cells[11].FindControl("tbData")).Enabled = habilitar;
                    ((DropDownList)linha.Cells[12].FindControl("ddlTipos")).Enabled = habilitar;
                }
            }
        }
        ...
        private void somarValores()
        {
            foreach (GridViewRow linha in grdTitulos.Rows)
            {
                ...
                if (marcador.Checked)
                {
                    decimal multa, juros, desc, outros, parcela;
                    decimal.TryParse(((TextBox)linha.Cells[6].FindControl("tbMulta")).Text, out multa);
                    decimal.TryParse(((TextBox)linha.Cells[7].FindControl("tbJuros")).Text, out juros);
                    decimal.TryParse(((TextBox)linha.Cells[8].FindControl("tbDesc")).Text, out desc);
                    decimal.TryParse(((TextBox)linha.Cells[9].FindControl("tbOutros")).Text, out outros);
                    decimal.TryParse(linha.Cells[5].Text, out parcela);
                    linha.Cells[11].Text = ((parcela - desc) + (multa + juros + outros)).toReal();
                }
            }
        }
