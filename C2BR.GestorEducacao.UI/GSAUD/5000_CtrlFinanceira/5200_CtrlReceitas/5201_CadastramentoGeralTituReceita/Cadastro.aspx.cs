...
//--------> Se o tipo de documento for Boleto: é obrigatório informar os Dados Bancários (Banco, Agência, Conta, Cedente)
            ...
            if (QueryStringAuxili.OperacaoCorrenteQueryString == QueryStrings.OperacaoInsercao)
            {
                ...
                var varOcoTit = (from itbs47 in TBS47_CTA_RECEB.RetornaTodosRegistros()
                                 where itbs47.NU_DOC == txtNumeroDocumento.Text && itbs47.TB25_EMPRESA.CO_EMP == LoginAuxili.CO_EMP &&
                                 itbs47.DT_CAD_DOC.Year == dtCadastro.Year && itbs47.DT_CAD_DOC.Month == dtCadastro.Month
                                 && itbs47.DT_CAD_DOC.Day == dtCadastro.Day && itbs47.NU_PAR == nuPar
                ...
                {
                    var varOcoTitTp = (from itbs47 in TBS47_CTA_RECEB.RetornaTodosRegistros()
                                       where itbs47.NU_DOC == txtNumeroDocumento.Text && itbs47.TB25_EMPRESA.CO_EMP == LoginAuxili.CO_EMP
                                       && itbs47.NU_PAR == nuPar && itbs47.TB086_TIPO_DOC.CO_TIPO_DOC == tpDocto
                                       select itbs47).FirstOrDefault();
            ...
            TBS47_CTA_RECEB tbs47 = RetornaEntidade();
            var tb25 = TB25_EMPRESA.RetornaPelaChavePrimaria(LoginAuxili.CO_EMP);

            if (tbs47 == null)
            {
                tbs47 = new TBS47_CTA_RECEB();

                var numContAux = int.Parse(tb25.NU_ULTIMO_CONTRATO);
                numContAux++;
                var numContrt = numContAux.ToString("D6");
                
                tbs47.NU_CONTRATO = numContrt;
                tbs47.TB25_EMPRESA = tb25;
                tbs47.NU_DOC = txtNumeroDocumento.Text;
                tbs47.NU_PAR = int.Parse(txtNumeroParcela.Text);
                tbs47.DT_CAD_DOC = DateTime.Now;
            ...
            tbs47.TP_CLIENTE_DOC = ddlTipoFonte.SelectedValue;
            tbs47.CO_ALU = ddlTipoFonte.SelectedValue == "A" ? int.Parse(ddlNomeFonte.SelectedValue) : (int?)null;

//--------> Faz a associação como a tabela de responsável
            if (tbs47.CO_ALU != null && (QueryStringAuxili.OperacaoCorrenteQueryString == QueryStrings.OperacaoInsercao || tbs47.TB108_RESPONSAVEL == null))
            {
                TB07_ALUNO tb07 = TB07_ALUNO.RetornaPeloCoAlu(tbs47.CO_ALU.Value);
                tb07.TB108_RESPONSAVELReference.Load();
                tbs47.TB108_RESPONSAVEL = tb07.TB108_RESPONSAVEL;
            }

            tbs47.TB103_CLIENTE = ddlTipoFonte.SelectedValue == "A" ? null : TB103_CLIENTE.RetornaPelaChavePrimaria(int.Parse(ddlNomeFonte.SelectedValue));
            tbs47.QT_PAR = int.Parse(txtQuantidadeParcelas.Text);
            tbs47.TB086_TIPO_DOC = TB086_TIPO_DOC.RetornaPeloCoTipoDoc(int.Parse(ddlTipoDocumento.SelectedValue));
            tbs47.CO_REF_DOCU = txtCodRefDocumento.Text;
            //tbs47.CO_CON_RECFIX = ddlContrato.SelectedValue != "" ? ddlContrato.SelectedValue : null;
            //tbs47.CO_ADITI_RECFIX = ddlAditivo.SelectedValue != "" ? (int?)int.Parse(ddlAditivo.SelectedValue) : null;
            tbs47.DT_EMISS_DOCTO = DateTime.Parse(txtDataDocumento.Text);
            tbs47.DT_VEN_DOC = DateTime.Parse(txtDataVencimento.Text);
            tbs47.FL_TIPO_COB = ddlTipoLocalCobranca.SelectedValue;
            tbs47.FL_TIPO_PREV_RECEB = ddlTpPrevReceb.SelectedValue;
            if (ddlTipoLocalCobranca.SelectedValue == "B")
            {
                tbs47.TB101_LOCALCOBRANCA = null;
                tbs47.IDEBANCO = ddlBanco.SelectedValue != "" ? ddlBanco.SelectedValue : null;
                tbs47.CO_AGENCIA = ddlAgencia.SelectedValue != "" ? (int?)int.Parse(ddlAgencia.SelectedValue) : null;
                tbs47.NU_CTA_DOC = ddlConta.SelectedValue != "" ? ddlConta.SelectedValue : null;
            }
            else
            {
                tbs47.TB101_LOCALCOBRANCA = ddlLocalCobranca.SelectedValue != "" ? TB101_LOCALCOBRANCA.RetornaPelaChavePrimaria(int.Parse(ddlLocalCobranca.SelectedValue)) : null;                
                tbs47.IDEBANCO = null;
                tbs47.CO_AGENCIA = null;
                tbs47.NU_CTA_DOC = null;
            }            

            tbs47.FL_EMITE_BOLETO = ddlFlagBoleto.SelectedValue;
            int idBoleto;
            TB227_DADOS_BOLETO_BANCARIO boletoEscolhido = new TB227_DADOS_BOLETO_BANCARIO();
            if (int.TryParse(ddlBoleto.SelectedValue, out idBoleto))
            {
                boletoEscolhido = TB227_DADOS_BOLETO_BANCARIO.RetornaPelaChavePrimaria(idBoleto);
                if (tbs47 != null && tbs47.TB227_DADOS_BOLETO_BANCARIO != null)
                {
                    if (!tbs47.TB227_DADOS_BOLETO_BANCARIO.Equals(boletoEscolhido))
                        gerarNovoBoleto = true;
                }
                tbs47.TB227_DADOS_BOLETO_BANCARIO = boletoEscolhido;
            }            

            tbs47.TB39_HISTORICO = ddlHistorico.SelectedValue != "" ? TB39_HISTORICO.RetornaPelaChavePrimaria(int.Parse(ddlHistorico.SelectedValue)) : null;            
            tbs47.DE_COM_HIST = txtComplementoHistorico.Text;
            tbs47.TB099_CENTRO_CUSTO = TB099_CENTRO_CUSTO.RetornaPelaChavePrimaria(int.Parse(ddlCentroCusto.SelectedValue));
            tbs47.TB56_PLANOCTA = TB56_PLANOCTA.RetornaPelaChavePrimaria(int.Parse(ddlContaContabilA.SelectedValue));
            tbs47.CO_SEQU_PC_CAIXA = ddlContaContabilC.SelectedValue != "" ? (int?)int.Parse(ddlContaContabilC.SelectedValue) : null;
            tbs47.CO_SEQU_PC_BANCO = ddlContaContabilB.SelectedValue != "" ? (int?)int.Parse(ddlContaContabilB.SelectedValue) : null;
            tbs47.VL_TOT_DOC = decimal.Parse(txtValorTotal.Text);
            tbs47.VL_PAR_DOC = decimal.Parse(txtValorParcela.Text);
            tbs47.VL_MUL_DOC = txtValorMulta.Text != "" ? (decimal?)decimal.Parse(txtValorMulta.Text) : null;
            tbs47.CO_FLAG_TP_VALOR_MUL = chkValorMultaPercentual.Checked ? "P" : "V";
            ...
            tbs47.VL_JUR_DOC = convertido ? (decimal?)jurosDec : null;
            tbs47.CO_FLAG_TP_VALOR_JUR = chkValorJurosPercentual.Checked ? "P" : "V";
            tbs47.VL_DES_DOC = txtValorDesconto.Text != "" ? (decimal?)decimal.Parse(txtValorDesconto.Text) : null;
            tbs47.CO_FLAG_TP_VALOR_DES = chkValorDescontoPercentual.Checked ? "P" : "V";
            tbs47.VL_OUT_DOC = txtOutroValor.Text != "" ? (decimal?)decimal.Parse(txtOutroValor.Text) : null;
            tbs47.CO_FLAG_TP_VALOR_OUT = chkOutroValorPercentual.Checked ? "P" : "V";
            tbs47.CO_AGRUP_RECDESP = (ddlAgrupador.SelectedValue != "") ? (int?)int.Parse(ddlAgrupador.SelectedValue) : null;
            if (ddlSituacao.SelectedValue == statusF[statusFinanceiro.A])
            {
                tbs47.VL_PAG = null;
                tbs47.VL_MUL_PAG = null;
                tbs47.VL_JUR_PAG = null;
                tbs47.VL_DES_PAG = null;
                tbs47.VL_OUT_PAG = null;
                tbs47.DT_REC_DOC = null;
                tbs47.CO_CAIXA = null;
                tbs47.CO_COL_BAIXA = null;
                tbs47.DT_MOV_CAIXA = null;
                tbs47.FL_ORIGEM_PGTO = null;
            }
            //tbs47.VL_PAG = txtValorRecebido.Text != "" ? (decimal?)decimal.Parse(txtValorRecebido.Text) : null;
            //tbs47.VL_MUL_PAG = txtValorMultaRecebido.Text != "" ? (decimal?)decimal.Parse(txtValorMultaRecebido.Text) : null;
            //tbs47.VL_JUR_PAG = txtValorJurosRecebido.Text != "" ? (decimal?)decimal.Parse(txtValorJurosRecebido.Text) : null;
            //tbs47.VL_DES_PAG = txtValorDescontoRecebido.Text != "" ? (decimal?)decimal.Parse(txtValorDescontoRecebido.Text) : null;
            //tbs47.VL_OUT_PAG = txtValorOutroRecebido.Text != "" ? (decimal?)decimal.Parse(txtValorOutroRecebido.Text) : null;
            tbs47.DE_OBS = txtObservacao.Text;
            //tbs47.CO_BARRA_DOC = txtCodigoBarras.Text;
            tbs47.CO_EMP_UNID_CONT = int.Parse(ddlUnidadeContrato.SelectedValue);
            tbs47.IC_SIT_DOC = ddlSituacao.SelectedValue;
            if ((ddlSituacao.SelectedValue == statusF[statusFinanceiro.C]) && (QueryStringAuxili.OperacaoCorrenteQueryString == QueryStrings.OperacaoAlteracao))
            {
                var ocoTb296 = (from iTb296 in TB296_CAIXA_MOVIMENTO.RetornaTodosRegistros()
                                where iTb296.CO_EMP_CAIXA == tbs47.CO_EMP && iTb296.NU_DOC_CAIXA == tbs47.NU_DOC && iTb296.NU_PAR_DOC_CAIXA.Value == tbs47.NU_PAR
                                    && iTb296.DT_DOC_CAIXA.Value.Year == tbs47.DT_CAD_DOC.Year && iTb296.DT_DOC_CAIXA.Value.Month == tbs47.DT_CAD_DOC.Month
                                    && iTb296.DT_DOC_CAIXA.Value.Day == tbs47.DT_CAD_DOC.Day
            ...
            CurrentPadraoCadastros.CurrentEntity = tbs47;
        ...
        void BarraCadastro_OnDelete(System.Data.Objects.DataClasses.EntityObject entity)
        {
            if (QueryStringAuxili.OperacaoCorrenteQueryString.Equals(QueryStrings.OperacaoAlteracao))
            {
                ...
                if (tbs47 != null)
                {
                    if (GestorEntities.Delete(tbs47) <= 0)
        ...
        private void CarregaFormulario()
        {
            ...
            if (tbs47 != null)
            {
                tbs47.TB086_TIPO_DOCReference.Load();
                tbs47.TB099_CENTRO_CUSTOReference.Load();
                tbs47.TB101_LOCALCOBRANCAReference.Load();
                tbs47.TB103_CLIENTEReference.Load();
                tbs47.TB116_TIPO_DEV_DOCReference.Load();
                tbs47.TB25_EMPRESAReference.Load();
                tbs47.TB39_HISTORICOReference.Load();
                tbs47.TB56_PLANOCTAReference.Load();
                tbs47.TB227_DADOS_BOLETO_BANCARIOReference.Load();

                txtCodRefDocumento.Text = tbs47.CO_REF_DOCU;
                txtCodigoBarras.Text = tbs47.CO_BARRA_DOC;
                txtComplementoHistorico.Text = tbs47.DE_COM_HIST;
                txtDataCadastro.Text = tbs47.DT_CAD_DOC.ToString("dd/MM/yyyy");
                txtDataRecebimento.Text = tbs47.DT_REC_DOC != null ? tbs47.DT_REC_DOC.Value.ToString("dd/MM/yyyy") : "";
                txtDataVencimento.Text = tbs47.DT_VEN_DOC.ToString("dd/MM/yyyy");
                txtDataDocumento.Text = tbs47.DT_EMISS_DOCTO.ToString("dd/MM/yyyy");
                txtDataSituacao.Text = tbs47.DT_SITU_DOC != null ? tbs47.DT_SITU_DOC.Value.ToString("dd/MM/yyyy") : DateTime.Now.ToString("dd/MM/yyyy");
                txtNumeroDocumento.Text = tbs47.NU_DOC;
                txtNumeroParcela.Text = tbs47.NU_PAR.ToString();
                txtObservacao.Text = tbs47.DE_OBS;
                txtQuantidadeParcelas.Text = tbs47.QT_PAR.ToString();
                txtValorDesconto.Text = String.Format("{0:N}", tbs47.VL_DES_DOC);
                txtValorDescontoRecebido.Text = String.Format("{0:N}", tbs47.VL_DES_PAG);
                txtOutroValor.Text = String.Format("{0:N}", tbs47.VL_OUT_DOC);
                txtValorOutroRecebido.Text = String.Format("{0:N}", tbs47.VL_OUT_PAG);
                txtValorJuros.Text = String.Format("{0:0.0000}", tbs47.VL_JUR_DOC);
                txtValorJurosRecebido.Text = String.Format("{0:N}", tbs47.VL_JUR_PAG);
                txtValorMulta.Text = String.Format("{0:N}", tbs47.VL_MUL_DOC);
                txtValorMultaRecebido.Text = String.Format("{0:N}", tbs47.VL_MUL_PAG);
                txtValorParcela.Text = String.Format("{0:N}", tbs47.VL_PAR_DOC);
                txtValorRecebido.Text = String.Format("{0:N}", tbs47.VL_PAG);
                txtValorTotal.Text = String.Format("{0:N}", tbs47.VL_TOT_DOC);
                txtNossoNum.Text =    tbs47.CO_NOS_NUM != null ? tbs47.CO_NOS_NUM.ToString() : "";
                CarregaCentrosCusto();
                ddlCentroCusto.SelectedValue = tbs47.TB099_CENTRO_CUSTO != null ? tbs47.TB099_CENTRO_CUSTO.CO_CENT_CUSTO.ToString() : "";

                // Conta Contábil Ativa
                if (tbs47.TB56_PLANOCTA != null)
                {
                    tbs47.TB56_PLANOCTA.TB54_SGRP_CTAReference.Load();
                    tbs47.TB56_PLANOCTA.TB54_SGRP_CTA.TB53_GRP_CTAReference.Load();
                    tbs47.TB56_PLANOCTA.TB055_SGRP2_CTAReference.Load();

                    ddlTipoContaA.SelectedValue = tbs47.TB56_PLANOCTA.TB54_SGRP_CTA.TB53_GRP_CTA.TP_GRUP_CTA;
                    CarregaGrupoContasContabeis(ddlTipoContaA, ddlGrupoContaA);
                    ddlGrupoContaA.SelectedValue = tbs47.TB56_PLANOCTA.TB54_SGRP_CTA.TB53_GRP_CTA.CO_GRUP_CTA.ToString();
                    CarregaSubgrupo(ddlGrupoContaA, ddlSubGrupoContaA);
                    ddlSubGrupoContaA.SelectedValue = tbs47.TB56_PLANOCTA.TB54_SGRP_CTA.CO_SGRUP_CTA.ToString();
                    CarregaSubgrupo2(ddlGrupoContaA, ddlSubGrupoContaA, ddlSubGrupo2ContaA);
                    ddlSubGrupo2ContaA.SelectedValue = tbs47.TB56_PLANOCTA.TB055_SGRP2_CTA != null ? tbs47.TB56_PLANOCTA.TB055_SGRP2_CTA.CO_SGRUP2_CTA.ToString() : "";
                    CarregaContasContabeis(ddlGrupoContaA, ddlSubGrupoContaA, ddlSubGrupo2ContaA, ddlContaContabilA);
                    ddlContaContabilA.SelectedValue = tbs47.TB56_PLANOCTA.CO_SEQU_PC.ToString();
                    CarregaCodigoContaContabil(ddlContaContabilA, txtCodigoContaContabilA);
                }

                // Conta Contábil Caixa
                if (tbs47.CO_SEQU_PC_CAIXA != null)
                {
                    TB56_PLANOCTA tb56 = TB56_PLANOCTA.RetornaPelaChavePrimaria((int)tbs47.CO_SEQU_PC_CAIXA);
                ...
                if (tbs47.CO_SEQU_PC_BANCO != null)
                {
                    TB56_PLANOCTA tb56 = TB56_PLANOCTA.RetornaPelaChavePrimaria((int)tbs47.CO_SEQU_PC_BANCO);
                    tb56.TB54_SGRP_CTAReference.Load();
                    tb56.TB54_SGRP_CTA.TB53_GRP_CTAReference.Load();
                    tb56.TB055_SGRP2_CTAReference.Load();
                ...
                ddlHistorico.SelectedValue = tbs47.TB39_HISTORICO != null ? tbs47.TB39_HISTORICO.CO_HISTORICO.ToString() : "";
                CarregaLocaisCobranca();                
                ddlSituacao.SelectedValue = tbs47.IC_SIT_DOC;
                ddlTipoDocumento.SelectedValue = tbs47.TB086_TIPO_DOC != null ? tbs47.TB086_TIPO_DOC.CO_TIPO_DOC.ToString() : "";
                ddlTipoFonte.SelectedValue = tbs47.TP_CLIENTE_DOC;
                ...
                ddlFlagBoleto.SelectedValue = tbs47.FL_EMITE_BOLETO;
                ...
                ddlUnidadeContrato.SelectedValue = tbs47.CO_EMP_UNID_CONT.ToString();
                if ((tbs47.TB227_DADOS_BOLETO_BANCARIO != null) && (ddlFlagBoleto.SelectedValue != "N"))
                {
                    ddlTipoTaxaBoleto.SelectedValue = tbs47.TB227_DADOS_BOLETO_BANCARIO.TP_TAXA_BOLETO;
                    CarregaBoletos();
                    ddlBoleto.SelectedValue = tbs47.TB227_DADOS_BOLETO_BANCARIO.ID_BOLETO.ToString();
                }

                ddlTipoLocalCobranca.SelectedValue = tbs47.FL_TIPO_COB;

                if (ddlTipoLocalCobranca.SelectedValue == "B")
                {
                    ddlBanco.Enabled = ddlAgencia.Enabled = ddlConta.Enabled = ddlLocalCobranca.Enabled = true;

                    ddlBanco.SelectedValue = tbs47.IDEBANCO != null ? tbs47.IDEBANCO : "";

                    CarregaAgencias();
                    ddlAgencia.SelectedValue = tbs47.CO_AGENCIA != null ? tbs47.CO_AGENCIA.ToString() : "";

                    CarregaContas();
                    ddlConta.SelectedValue = tbs47.NU_CTA_DOC != null ? tbs47.NU_CTA_DOC : "";
                    ddlLocalCobranca.Enabled = false;
                }
                else
                {
                    ddlLocalCobranca.Enabled = true;
                    ddlLocalCobranca.SelectedValue = tbs47.TB101_LOCALCOBRANCA != null ? tbs47.TB101_LOCALCOBRANCA.CO_LOC_COB.ToString() : "";
                }

                chkValorDescontoPercentual.Checked = tbs47.CO_FLAG_TP_VALOR_DES == "P";
                chkValorJurosPercentual.Checked = tbs47.CO_FLAG_TP_VALOR_JUR == "P";
                chkValorMultaPercentual.Checked = tbs47.CO_FLAG_TP_VALOR_MUL == "P";
                chkOutroValorPercentual.Checked = tbs47.CO_FLAG_TP_VALOR_OUT == "P";
                ddlTpPrevReceb.SelectedValue = tbs47.FL_TIPO_PREV_RECEB != null ? tbs47.FL_TIPO_PREV_RECEB : "B";
                ddlTipoReceb.SelectedValue = tbs47.FL_TIPO_RECEB != null ? tbs47.FL_TIPO_RECEB : "";

                ddlTpPrevReceb.Enabled = tbs47.IC_SIT_DOC != "Q";
...
//------------> Campos que devem ser desabilitados na alteração
                ...
                if (tbs47.TP_CLIENTE_DOC == "A")
                {
                    CarregaAluno();
                    ddlNomeFonte.SelectedValue = tbs47.CO_ALU.ToString();
                    ...
                    {
                        txtNomeResponsavel.Text = tbs47.TB108_RESPONSAVEL.NO_RESP;
                        txtCpfResponsavel.Text = tbs47.TB108_RESPONSAVEL.NU_CPF_RESP.Length == 11 ? tbs47.TB108_RESPONSAVEL.NU_CPF_RESP.Insert(3, ".").Insert(7, ".").Insert(11, "-") : tb07.TB108_RESPONSAVEL.NU_CPF_RESP;
                    }
                }
                else
                {
                    CarregaFontesReceita();
                    ddlNomeFonte.SelectedValue = tbs47.TB103_CLIENTE.CO_CLIENTE.ToString();
                    CarregaCodigoFonte(false);
                }

                ddlAgrupador.SelectedValue = (tbs47.CO_AGRUP_RECDESP.HasValue) ? tbs47.CO_AGRUP_RECDESP.ToString() : "";

                txtLocalPagto.Text = tbs47.IC_SIT_DOC == "Q" && tbs47.FL_ORIGEM_PGTO != null ? tbs47.FL_ORIGEM_PGTO == "B" ? "Rede Bancária" : "Caixa Unidade" : "";
        ...
        /// </summary>
        /// <returns>Entidaed TBS47_CTA_RECEB</returns>
        private TBS47_CTA_RECEB RetornaEntidade()
        {
            return TBS47_CTA_RECEB.RetornaPrimeiroRegistroPeloCoEmpNuDocNuPar(LoginAuxili.CO_EMP, QueryStringAuxili.RetornaQueryStringPelaChave("doc"),
...
//--------> Recebe as chaves primáris do Título
            ...
            TBS47_CTA_RECEB novotbs47 = TBS47_CTA_RECEB.RetornaPrimeiroRegistroPeloCoEmpNuDocNuPar(LoginAuxili.CO_EMP, strNudoc, intNuPar);            

            novotbs47.TB227_DADOS_BOLETO_BANCARIOReference.Load();

            if (novotbs47.TB227_DADOS_BOLETO_BANCARIO != null)
            {
                if (ddlBoleto.SelectedValue != novotbs47.TB227_DADOS_BOLETO_BANCARIO.ID_BOLETO.ToString())
                {
                    novotbs47.TB227_DADOS_BOLETO_BANCARIO = TB227_DADOS_BOLETO_BANCARIO.RetornaPelaChavePrimaria(int.Parse(ddlBoleto.SelectedValue));

//----------------> Salva o novo boleto no título                    
                    GestorEntities.SaveOrUpdate(novotbs47);
...
//--------> Recebe o Título de Contas a Receber
            TBS47_CTA_RECEB tbs47 = TBS47_CTA_RECEB.RetornaPrimeiroRegistroPeloCoEmpNuDocNuPar(LoginAuxili.CO_EMP, strNudoc, intNuPar);
            ...
            int coResp = tbs47.TB108_RESPONSAVEL != null ? tbs47.TB108_RESPONSAVEL.CO_RESP : 0;
...
//--------> Informações do Boleto
            ...
            if (String.IsNullOrEmpty(tbs47.CO_NOS_NUM) || boletoNovo)
            {
                #region Gera um novo nosso número
                //===> Atribui um novo nosso número ao título
                tbs47.CO_NOS_NUM = tbs47.TB227_DADOS_BOLETO_BANCARIO.TB224_CONTA_CORRENTE.TB30_AGENCIA.TB29_BANCO.CO_PROX_NOS_NUM.Trim();
                ...
                TB29_BANCO u = TB29_BANCO.RetornaPelaChavePrimaria(tbs47.TB227_DADOS_BOLETO_BANCARIO.TB224_CONTA_CORRENTE.TB30_AGENCIA.IDEBANCO);
            ...
            if (tbs47.TB227_DADOS_BOLETO_BANCARIO == null)
                tbs47.TB227_DADOS_BOLETO_BANCARIOReference.Load();
            TB227_DADOS_BOLETO_BANCARIO tb227 = tbs47.TB227_DADOS_BOLETO_BANCARIO;

            if (tb227.FL_TX_BOL_CLI != "S")
                boleto.Valor = tbs47.VL_PAR_DOC;
            else
                boleto.Valor = tbs47.VL_PAR_DOC + tb227.VR_TX_BOL_CLI.Value;
...
//--------> Informações do Cedente
            ...
            boleto.Agencia = tbs47.TB227_DADOS_BOLETO_BANCARIO.TB224_CONTA_CORRENTE.TB30_AGENCIA.CO_AGENCIA + (tbs47.TB227_DADOS_BOLETO_BANCARIO.TB224_CONTA_CORRENTE.TB30_AGENCIA.DI_AGENCIA != "" ? ("-" +
                tbs47.TB227_DADOS_BOLETO_BANCARIO.TB224_CONTA_CORRENTE.TB30_AGENCIA.DI_AGENCIA) : "");

            boleto.CodigoCedente = tbs47.TB227_DADOS_BOLETO_BANCARIO.CO_CEDENTE.Trim();
            boleto.Conta = tbs47.TB227_DADOS_BOLETO_BANCARIO.TB224_CONTA_CORRENTE.CO_CONTA.Trim() + '-' + tbs47.TB227_DADOS_BOLETO_BANCARIO.TB224_CONTA_CORRENTE.CO_DIG_CONTA.Trim();
            boleto.CpfCnpjCedente = AuxiliFormatoExibicao.preparaCPFCNPJ(tb25.CO_CPFCGC_EMP);
            boleto.NomeCedente = tb25.NO_RAZSOC_EMP + " " + boleto.CpfCnpjCedente;

            boleto.Desconto = (!tbs47.VL_DES_DOC.HasValue ? 0
                                : (tbs47.CO_FLAG_TP_VALOR_DES == "P"
                                ? (boleto.Valor * tbs47.VL_DES_DOC.Value / 100)
                                : tbs47.VL_DES_DOC.Value));
...
//            strMultaMoraDesc += tbs47.VL_MUL_DOC != null && tbs47.VL_MUL_DOC.Value != 0 ?
//                (tbs47.CO_FLAG_TP_VALOR_MUL == "P" ? "Multa: " + tbs47.VL_MUL_DOC.Value.ToString("0.00") + "% (R$ " +
//                (boleto.Valor * (decimal)tbs47.VL_MUL_DOC.Value / 100).ToString("0.00") + ")" : "Multa: R$ " + tbs47.VL_MUL_DOC.Value.ToString("0.00")) : "Multa: XX";

////--------> Informações da Mora
//            strMultaMoraDesc += tbs47.VL_JUR_DOC != null && tbs47.VL_JUR_DOC.Value != 0 ?
//                 (tbs47.CO_FLAG_TP_VALOR_JUR == "P" ? " - Juros Diário: " + tbs47.VL_JUR_DOC.Value.ToString() + "% (R$ " +
//                 (boleto.Valor * (decimal)tbs47.VL_JUR_DOC.Value / 100).ToString("0.00") + ")" : " - Juros Diário: R$ " +
//                    tbs47.VL_JUR_DOC.Value.ToString("0.00")) : " - Juros Diário: XX";

////--------> Informações do desconto
//            strMultaMoraDesc += tbs47.VL_DES_DOC != null && tbs47.VL_DES_DOC.Value != 0 ?
//                 (tbs47.CO_FLAG_TP_VALOR_DES == "P" ? " - Descto: " + tbs47.VL_DES_DOC.Value.ToString() + "% (R$ " +
//                 (boleto.Valor * (decimal)tbs47.VL_DES_DOC.Value / 100).ToString("0.00") + ")" : " - Descto: R$ " +
//                    tbs47.VL_DES_DOC.Value.ToString("0.00")) : " - Descto: XX";

//            strMultaMoraDesc += tbs47.VL_DES_BOLSA_ALUNO != null && tbs47.VL_DES_BOLSA_ALUNO.Value != 0 ?
//                         (tbs47.CO_FLAG_TP_VALOR_DES_BOLSA_ALUNO == "P" ? " - Descto Bolsa: " + tbs47.VL_DES_BOLSA_ALUNO.Value.ToString("0.00") + "% (R$ " +
//                         (boleto.Valor * (decimal)tbs47.VL_DES_BOLSA_ALUNO.Value / 100).ToString("0.00") + ")" : " - Descto Bolsa: R$ " +
//                            tbs47.VL_DES_BOLSA_ALUNO.Value.ToString("0.00")) : "";
...
//--------> Faz a adição de Instruções de Informações do Responsável do Aluno ou Informações do Cliente, de acordo com o tipo                    
            if (strTipoFonte == "A")
            {
                if (tbs47.CO_ALU.HasValue)
                {
                    var tb07 = TB07_ALUNO.RetornaPeloCoAlu(tbs47.CO_ALU.Value);
                    strCnpjCPF += "Paciente: " + tb07.NU_NIRE.ToString().PadLeft(7, '0') + " - " + tb07.NO_ALU;

                    tbs47.TBS390_ATEND_AGENDReference.Load();

                    if (tbs47.TBS390_ATEND_AGEND != null)
                    {
                        strCnpjCPF += "<br> Unidade Atendimento: " + TB25_EMPRESA.RetornaPelaChavePrimaria(tbs47.TBS390_ATEND_AGEND.CO_EMP_ATEND).NO_FANTAS_EMP +
                ...
                if (tbs47.TB39_HISTORICO != null)
                    strCnpjCPF += "<br>*** Referente: " + tbs47.TB39_HISTORICO.DE_HISTORICO + " ***";

                boleto.Instrucoes += strCnpjCPF;
            }
            else
            {
                boleto.Instrucoes += "</br>" + "(" + tbs47.TB103_CLIENTE.NO_FAN_CLI + ")";

                boleto.Instrucoes += "</br>" + "(Contrato: " + (tbs47.CO_CON_RECFIX != null ? tbs47.CO_CON_RECFIX : "XXXXX") +
                    " - Aditivo: " + (tbs47.CO_ADITI_RECFIX != null ? tbs47.CO_ADITI_RECFIX.Value.ToString("00") : "XX") +
                    " - Parcela: " + tbs47.NU_PAR.ToString("00") + ")";
            }
            #region Informa o valor do documento eo tipo de documento
            //string tpdoc = "";
            //if (strTipoFonte == "A")
            //{
            //    switch (tbs47.NU_DOC.Substring(0, 2))
            //    {
            //        case "MN":
            ...
            //if (tb227.FL_TX_BOL_CLI != "S")
            //{
            //    boleto.Instrucoes += "<br> Valor " + tpdoc + ":" + " R$" + tbs47.VL_PAR_DOC.ToString("N2");
            //}
            //else
            //{
            //    boleto.Instrucoes += "<br> Valor " + tpdoc + ":" + " R$" + tbs47.VL_PAR_DOC.ToString("N2") + " + " + " R$" + tb227.VR_TX_BOL_CLI.Value.ToString("N2") + "(Taxa de Emissão do Boleto) ";
            //}
            #endregion
