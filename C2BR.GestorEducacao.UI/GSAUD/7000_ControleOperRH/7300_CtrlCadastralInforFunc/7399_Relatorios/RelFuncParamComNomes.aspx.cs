...
// ---------+----------------------------+-------------------------------------
//15/05/2014| Maxwell Almeida            | Criação da página e Filtro para que possam ser conseguidas as informações de quais colaboradores são plantonistas/atividade domiciliar, etc 
//          |                            | existem em cada Unidade/função/cidade, etc.
...
using C2BR.GestorEducacao.Reports.GSAUD._7000_ControleOperRH._7100_CtrlCadastralInfosFuncionais._7120_DemonColaborComplet;

namespace C2BR.GestorEducacao.UI.GSAUD._7000_ControleOperRH._7300_CtrlCadastralInforFunc._7399_Relatorios
{
    public partial class RelFuncParamComNomes : System.Web.UI.Page
    {
        ...
        {
            if (ddlTipoPesq.SelectedValue == "U")
                porUnidade();
        ...
        void porUnidade()
        {
            ...
            string tipoRelatorio, noUnidCadastro, noUnidContrato, noRegiao, noArea, noSubarea, noUF, noCidade, noBairro, noClassFunc, noCategoria, noEspecializa, uf,paramPla;
            ...
            //{
                parametros = "( Unidade Cadastro: " + noUnidCadastro.ToUpper() + " - Unidade Contrato: " + noUnidContrato.ToUpper() + " - Região/Área: " + noRegiao.ToUpper()
                    ...
                    + " - Apenas Plantonistas: " + paramPla.ToUpper() + " - Emite Gráficos: NÃO )";

                RptRelFuncParamComNPorUnid fpcb = new RptRelFuncParamComNPorUnid();
                lRetorno = fpcb.InitReport(parametros, infos, coEmp, UnidCadastro, UnidContrato, regiao, area, subarea, uf, Cidade, Bairro, classFunc, categoria, especializa, pla);
                Session["Report"] = fpcb;
                Session["URLRelatorio"] = "/GeducReportViewer.aspx";
        ...
        /// <summary>
        /// Verifica o tipo de sistema abeto
        /// </summary>
        private void VerificaUnidadeLogado()
        {
            switch (LoginAuxili.CO_TIPO_UNID)
            {
                case "PGE":
                    LabelchkPlant.Text = "Monitoria";

                    break;
                case "PGS":

                    LabelchkPlant.Text = "Plantonista";
