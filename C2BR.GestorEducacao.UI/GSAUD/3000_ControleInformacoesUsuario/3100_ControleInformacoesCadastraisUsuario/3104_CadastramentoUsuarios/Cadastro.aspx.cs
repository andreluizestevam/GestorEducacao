...
// ----------+----------------------------+-------------------------------------
// 07/03/2014| Julio Gleisson Rodrigues   | Pagina do Cadastro copiado 
//           |                            | \GEDUC\3000_CtrlOperacionalPedagogico
//           |                            | \3600_CtrlInformacoesAlunos\3640_CtrlOcorrenciasAlunos\3641_CadastramentoOcorrDiscAluno
    ...
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    txtDataSituacaoAlu.Text = DateTime.Now.ToString("dd/MM/yyyy");
                    // dados do registro
        ...
        void CurrentPadraoCadastros_OnAcaoBarraCadastro()
        {
            ...
            string cpfPac = txtCpfAlu.Text.Replace(".", "").Replace("-", "").Trim();

            if (tb07.CO_ALU == 0 && !String.IsNullOrEmpty(cpfPac))
            {
            ...
            //-------------------------------------------------------------------------------------------
            string IdPlano = ddlPlano.SelectedValue == "" ? null : ddlPlano.SelectedValue;
            tb07.NU_PLANO_SAUDE = IdPlano;

            int idOperadora = ddlOperadora.SelectedValue == "" ? 0 : Convert.ToInt32(ddlOperadora.SelectedValue);
            var Opera = TB250_OPERA.RetornaPelaChavePrimaria(Convert.ToInt32(idOperadora));
            tb07.TB250_OPERA = Opera == null ? null : Opera;


            int idCateg = ddlCategoria.SelectedValue == "" ? 0 : Convert.ToInt32(ddlCategoria.SelectedValue);
            var Categoria = TB367_CATEG_PLANO_SAUDE.RetornaPelaChavePrimaria(idCateg);
            tb07.TB367_CATEG_PLANO_SAUDE = Categoria == null ? null : Categoria;

            tb07.NU_PLANO_SAUDE = txtNumero.Text;
            tb07.DE_PASTA_CONTR = !string.IsNullOrEmpty(txtPastaControl.Text) ? tb07.NU_NIRE.toNire() : txtPastaControl.Text;
            ...
            #endregion
            #region Bloco 6
            tb07.NO_MAE_ALU = txtNomeMaeAlu.Text.ToUpper();
            ...
            tb07.NO_PAI_ALU = txtNomePaiAlu.Text.ToUpper();
        ...
        private bool ValidaCamposUsuario()
        {
            if (QueryStringAuxili.OperacaoCorrenteQueryString.Equals(QueryStrings.OperacaoInsercao))
            {
                int nire = 0;
                if (txtNireAlu.Text != "")
                {
                    nire = int.Parse(txtNireAlu.Text);
                }



                ///Faz a verificação de ocorrência de aluno pelo nome, sexo e data de nascimento ( quando inclusão )
                var ocorrUsuario = from lTb07 in TB07_ALUNO.RetornaTodosRegistros()
                                   where lTb07.NO_ALU == txtNomeAlu.Text && lTb07.NU_NIRE == nire
                                   select new { lTb07.NU_NIRE };

                if (ocorrUsuario.Count() > 0)
                {
                    AuxiliPagina.EnvioMensagemErro(this.Page, "Usuario já cadastrado no sistema.");
                    return false;
                }
            }

            if (string.IsNullOrEmpty(txtNomeAlu.Text))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Campo Nome é obrigatório.");
                txtNomeAlu.Focus();
                return false;
            }
            if (string.IsNullOrEmpty(txtCepAlu.Text))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Campo CEP é obrigatório.");
                txtCepAlu.Focus();
                return false;
            }
            if (string.IsNullOrEmpty(ddlUfAlu.SelectedValue))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Campo UF é obrigatório.");
                ddlUfAlu.Focus();
                return false;
            }
            //if (string.IsNullOrEmpty(txtNaturalidadeAlu.Text))
            //{
            //    AuxiliPagina.EnvioMensagemErro(this.Page, "Campo Cidade é obrigatório.");
            //    txtNaturalidadeAlu.Focus();
            //    return false;
            //}
            if (string.IsNullOrEmpty(ddlBairroAlu.SelectedValue))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Campo Bairro é obrigatório.");
                ddlBairroAlu.Focus();
                return false;
            }
            if (string.IsNullOrEmpty(txtLogradouroAlu.Text))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Campo Logradouro é obrigatório.");
                txtLogradouroAlu.Focus();
                return false;
            }
            if (string.IsNullOrEmpty(txtApelidoAlu.Text))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Campo Apelido é obrigatório.");
                txtApelidoAlu.Focus();
                return false;
            }
            var tb25 = TB25_EMPRESA.RetornaPelaChavePrimaria(LoginAuxili.CO_EMP);

            if (!String.IsNullOrEmpty(txtCpfAlu.Text))
            {
                if (!AuxiliValidacao.ValidaCpf(txtCpfAlu.Text))
                {
        ...
        private void CarregaFormulario()
        {
            ...
            if (tb07 != null)
            {
                ...
                txtApelidoAlu.Text = !String.IsNullOrEmpty(tb07.NO_APE_ALU) ? tb07.NO_APE_ALU.ToUpper() : "";
                txtNomeMaeAlu.Text = !String.IsNullOrEmpty(tb07.NO_MAE_ALU) ? tb07.NO_MAE_ALU.ToUpper() : "";
                ...
                txtNomePaiAlu.Text = !String.IsNullOrEmpty(tb07.NO_PAI_ALU) ? tb07.NO_PAI_ALU.ToUpper() : ""; ;
                ...
                {
                    string Plono = tb07.TB251_PLANO_OPERA.ID_PLAN.ToString();
                    if (Plono != null || Plono != "")
                    {
                        ddlPlano.SelectedValue = Plono;
                        AuxiliCarregamentos.CarregaCategoriaPlanoSaude(ddlCategoria, ddlPlano, false);

                        if (tb07.TB367_CATEG_PLANO_SAUDE != null)
                        {
                            string Categoria = tb07.TB367_CATEG_PLANO_SAUDE.ID_CATEG_PLANO_SAUDE.ToString();
                ...
                txtNumero.Text = tb07.NU_PLANO_SAUDE;
