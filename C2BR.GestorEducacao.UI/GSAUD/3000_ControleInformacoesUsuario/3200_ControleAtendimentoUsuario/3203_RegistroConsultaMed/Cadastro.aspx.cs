...
// 23/08/2017 | Diogo Costa                       | Criação do evento que atualiza a agenda através da modal
// 04/11/2017 | Warllison Lima                    | Alteração da Pesquisa Fonética do campo Paciente Linha 412 - 438
...
namespace C2BR.GestorEducacao.UI.GSAUD._3000_ControleInformacoesUsuario._3200_ControleAtendimentoUsuario._3203_RegistroConsultaMed
{
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        /// <summary>
        /// Abre o modal de procedimentos do agendamento
        /// </summary>
        protected void lnkSelecProcedimento_OnClick(object sender, EventArgs e)
        {
            //txtDtInicioModal.Text = DateTime.Now.ToShortDateString();
            //txtDtFimModal.Text = DateTime.Now.ToShortDateString();

            //DateTime dtIni = DateTime.Parse(txtDtInicioModal.Text);
            //DateTime dtFim = DateTime.Parse(txtDtFimModal.Text);
            var verificaHorario = false;
            var verificaProfi = false;
            int idHorario = 0;
            int idOperador = 0;
            int idPaciente = 0;
            ...
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Você deve selecionar um plano.");
                return;
            }

            //percorreo grid de horário para verificar se existe horário selecionado
            foreach (GridViewRow linha in grdHorario.Rows)
            {
                CheckBox chk = ((CheckBox)linha.Cells[0].FindControl("ckSelectHr"));

                if (chk.Checked)
                {
                    var item = ((HiddenField)linha.FindControl("hidCoAgenda")).Value;

                    int.TryParse(item, out idHorario);
            ...
            //percorre o grid de profissionais para verificar se exite algum item selecionado
            ...
            if (!verificaHorario || !verificaProfi)
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Você deve selecionar um profissional e horário.");
                return;
            }

            if (string.IsNullOrEmpty(ddlNomeUsu.SelectedValue))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Você deve selecionar um paciente.");
                return;
            }


            int.TryParse(ddlOperPlano.SelectedValue, out idOperador);
            int.TryParse(ddlNomeUsu.SelectedValue, out idPaciente);

            abreModalSelecProcedimentos();
            PopularGrdProcedimentos();

            var lstCoProcedimento = ViewState["lstCoProcedimento"] as List<string>;

            if (idOperador != 0 && idPaciente != 0 && (lstCoProcedimento == null || !(lstCoProcedimento.Count > 0)))
            {
                PopularProcedimentoHorario(idHorario, idOperador, idPaciente);
            }


            VerificarGrdProcedimentoModal();
            //PopularGrdModalHorarios(dtIni, dtFim);
        }

        protected void lnkAddProcedimento_OnClick(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow l in GrdProcedimentosPesquisa.Rows)
            {
                CheckBox chk = ((CheckBox)l.Cells[0].FindControl("ckSelectProcedimento"));

                if (chk.Checked)
                {
                    var id = ((HiddenField)l.FindControl("hidCoAgenda")).Value;
                    lstIds.Add(id);
                    lst.Add(new
                    {
                        ID_PROC_MEDI_PROCE = id,
                        CO_PROC_MEDI = ((HiddenField)l.FindControl("hidCoAlu")).Value
                    });
            ...
            ViewState["lstCoProcedimento"] = lstIds;
            GrdProcedimentosModal.DataSource = lst;
            GrdProcedimentosModal.DataBind();

        }

        protected void imgExcProcedimentoModal_OnClick(object sender, EventArgs e)
        {
            ...
            if (GrdProcedimentosModal.Rows.Count != 0)
            {
                foreach (GridViewRow linha in GrdProcedimentosModal.Rows)
                {
                    var img = (ImageButton)linha.FindControl("imgExcProcedimentoModal");
                    var id = ((HiddenField)linha.FindControl("hidCoAgenda")).Value;
                    if (img.ClientID != atual.ClientID)
                    {
                        ...
                        {
                            ID_PROC_MEDI_PROCE = id,
                            CO_PROC_MEDI = ((HiddenField)linha.FindControl("hidCoAlu")).Value
                        });

                    }
                    else
                    {
                        HabilitarItemProcedimentoModal(id, true);
                    }
        ...
        private void PopularProcedimentoHorario(int idHorario, int idOper, int idPaciente)
        {

            var res = TBS389_ASSOC_ITENS_PLANE_AGEND.RetornaTodosRegistros()
                .Where(p => p.TBS174_AGEND_HORAR.ID_AGEND_HORAR == idHorario
                    && p.TBS174_AGEND_HORAR.TB250_OPERA.ID_OPER == idOper
                    && p.TBS174_AGEND_HORAR.CO_ALU == idPaciente
                    && p.TBS174_AGEND_HORAR.TBS356_PROC_MEDIC_PROCE.CO_SITU_PROC_MEDI == "A")
                    .Select(p => new { p.TBS386_ITENS_PLANE_AVALI.TBS356_PROC_MEDIC_PROCE.ID_PROC_MEDI_PROCE, CO_PROC_MEDI = p.TBS386_ITENS_PLANE_AVALI.TBS356_PROC_MEDIC_PROCE.CO_PROC_MEDI + " - " + p.TBS386_ITENS_PLANE_AVALI.TBS356_PROC_MEDIC_PROCE.NM_PROC_MEDI }).OrderBy(w => w.CO_PROC_MEDI)
                    .ToList();

            ViewState["lstCoProcedimento"] = res.Select(p => p.ID_PROC_MEDI_PROCE.ToString()).ToList();

            GrdProcedimentosModal.DataSource = res;
            GrdProcedimentosModal.DataBind();
        }

        /// <summary>
        /// Retorna uma string com os horários selecionados
        /// </summary>
        private string ObterHorarioSelecionado()
        {

            var idHorario = ViewState["idChkHorario"];
            var lstHorarios = new List<string>();
            ...
            foreach (GridViewRow l in grdHorario.Rows)
            {
                ...
                if (chk.Checked)
                {
                    lstHorarios.Add(((HiddenField)l.FindControl("hora")).Value);
                }
            }

            for (int i = 0; i < lstHorarios.Count; i++)
            {
                var h = lstHorarios[i];

                horarios += h;

                if (i + 1 < lstHorarios.Count)
                {
                    horarios += " | ";
                }
            }
        ...
        /// <summary>
        /// Verifica o grid de procedimentos para desabilitar os que já estão selecionados
        /// </summary>
        private void VerificarGrdProcedimentoModal()
        {
            int contador = 0;
            var lstCoProcedimento = ViewState["lstCoProcedimento"] as List<string>;

            foreach (GridViewRow linhaPesq in GrdProcedimentosPesquisa.Rows)
            {

                if (contador > GrdProcedimentosModal.Rows.Count)
                {
                    break;
                }
                var id = ((HiddenField)linhaPesq.FindControl("hidCoAgenda")).Value;
                if (lstCoProcedimento.Any(p => p == id))
                {
                    contador++;

                    HabilitarItemProcedimentoModal(id, false);
                }
            }
        }

        /// <summary>
        /// Habilita/Desabilita o checkbox do procedimento passado como parametro
        /// </summary>
        private void HabilitarItemProcedimentoModal(string id, bool habilitar)
        {
            foreach (GridViewRow linha in GrdProcedimentosPesquisa.Rows)
            {
                if (((HiddenField)linha.FindControl("hidCoAgenda")).Value == id)
                {
                    linha.Enabled = habilitar;
                    CheckBox chk = ((CheckBox)linha.Cells[0].FindControl("ckSelectProcedimento"));
                    chk.Checked = !habilitar;
                    break;
                }
            }
        }
        ...
        /// <summary>
        /// Popula o grid de procedimentos de acordo com o plano selecionado
        /// </summary>
        private void PopularGrdProcedimentos()
        {

            int idOp = 0;
            int.TryParse(ddlOperPlano.SelectedValue, out idOp);

            var res = ObterProcedimentosConsulta(idOp);

            GrdProcedimentosPesquisa.DataSource = res;
            GrdProcedimentosPesquisa.DataBind();
        }

        private void PopularGrdModalHorarios(DateTime dtInicio, DateTime dtTermino)
        {
            ...
            foreach (GridViewRow l in grdProfi.Rows)
            {
                ...
                if (chk.Checked == true)
                {
                    ...
                    coColI = (!string.IsNullOrEmpty(coCol) ? int.Parse(coCol) : 0);
                    break;
                }
            }


            var res = ObterHorariosProfissional(coColI, chkHorDispResCons.Checked, dtInicio, dtTermino);
        ...
        //INÍCIO - Pesquisa por nome 
        protected void imgbPesqPacNome_OnClick(object sender, EventArgs e)
        {
            var result = (from tb07 in TB07_ALUNO.RetornaTodosRegistros()
                          where tb07.CO_SITU_ALU != "O" && tb07.CO_SITU_ALU != "H"
                          && (tb07.CO_EMP == LoginAuxili.CO_EMP)
        ...
        {
            ddlNomeUsu.SelectedValue = string.Empty;
        ...
        /// <summary>
        /// Evento que atualiza o horário e as informações do plano de saúde por um Modal
        /// </summary>
        protected void ButtonSalvarHorario_Click(object sender, EventArgs e)
        {
            int idHorarioSelecionado = 0;

            //obtem o id do item selecionado que foi armazenado no viewstate
            int.TryParse(ViewState["idChkHorario"].ToString(), out idHorarioSelecionado);

            if (idHorarioSelecionado != 0)
            {
                TBS174_AGEND_HORAR novaAgenda = null;
                int operadora = 0;
                int plano = 0;
                int procedimento = 0;
                string numeroPlano = string.Empty;
                var update = true;


                //obtem o item atual pela PK e carrega seus relacionamentos
                var res = TBS174_AGEND_HORAR.RetornaPelaChavePrimaria(idHorarioSelecionado);
                res.TB250_OPERAReference.Load();
                res.TB251_PLANO_OPERAReference.Load();
                res.TBS356_PROC_MEDIC_PROCEReference.Load();
                res.TBS370_PLANE_AVALIReference.Load();

                numeroPlano = txtNumeroCartPlaModal.Text;

                int.TryParse(ddlOperadoraModal.SelectedValue, out operadora);
                int.TryParse(ddlPlanoModal.SelectedValue, out plano);
                //int.TryParse(ddlProcedimentoModal.SelectedValue, out procedimento);

                //valida os campos de plano de saúde
                //if (operadora == 0)
                //{
                //    AuxiliPagina.EnvioMensagemErro(this.Page, "Selecione uma Operadora.");
                //    LimparHorariosAgendaSelecionados();
                //    return;
                //}
                //if (plano == 0)
                //{
                //    AuxiliPagina.EnvioMensagemErro(this.Page, "Selecione um Plano.");
                //    LimparHorariosAgendaSelecionados();
                //    return;
                //}
                //if (procedimento == 0)
                //{
                //    AuxiliPagina.EnvioMensagemErro(this.Page, "Selecione um Procedimento.");
                //    LimparHorariosAgendaSelecionados();
                //    return;
                //}

                var resIdOperadora = res.TB250_OPERA != null ? res.TB250_OPERA.ID_OPER : 0;
                var resIdPlano = res.TB251_PLANO_OPERA != null ? res.TB251_PLANO_OPERA.ID_PLAN : 0;
                var resIdProcedimento = res.TBS356_PROC_MEDIC_PROCE != null ? res.TBS356_PROC_MEDIC_PROCE.ID_PROC_MEDI_PROCE : 0;

                //Verifica se usuário alterou algum dado do plano de saúde
                if (numeroPlano != res.NU_PLAN_SAUDE || operadora != resIdOperadora || plano != resIdPlano || procedimento != resIdProcedimento)
                {
                    res.TB250_OPERA = TB250_OPERA.RetornaPelaChavePrimaria(operadora) ?? null;
                    res.TB251_PLANO_OPERA = TB251_PLANO_OPERA.RetornaPelaChavePrimaria(plano) ?? null;
                    res.TBS356_PROC_MEDIC_PROCE = TBS356_PROC_MEDIC_PROCE.RetornaPelaChavePrimaria(procedimento) ?? null;
                    res.HR_AGEND_HORAR = res.HR_AGEND_HORAR;
                    res.NU_PLAN_SAUDE = numeroPlano;
                }

                //Verifica se usuário mudou o horário
                if (TextBoxNovaHora.Text != res.HR_AGEND_HORAR)
                {
                    //obtem o novo horário selecionado pelo usuário
                    novaAgenda = TBS174_AGEND_HORAR.RetornaTodosRegistros()
                                .FirstOrDefault(p => p.CO_COL == res.CO_COL && p.DT_AGEND_HORAR == res.DT_AGEND_HORAR && p.HR_AGEND_HORAR == TextBoxNovaHora.Text);


                    //se não possui registro para aquele horário é feita a criação deste
                    if (novaAgenda == null)
                    {
                        novaAgenda = MapearNovaAgenda(res);
                        novaAgenda.HR_AGEND_HORAR = TextBoxNovaHora.Text;

                        //limpa os objeto do horário antigo
                        LimparDadosAgenda(res);
                        TBS174_AGEND_HORAR.SaveOrUpdate(res, true);
                        update = false;//verifica se salva os dados no final do método

                        var tbs386 = AssociarPlanejamento(novaAgenda, (int)novaAgenda.CO_ALU, operadora, plano.ToString(), procedimento.ToString());

                        //Associa o item criado ao agendamento em contexto na tabela TBS389_ASSOC_ITENS_PLANE_AGEND
                        AssociarItemCriadoPlanejamento(tbs386, novaAgenda);

                    }
                    else
                    {
                        //atribui os dados do item atual ao novo horário da agenda
                        novaAgenda.CO_EMP_ALU = res.CO_EMP_ALU;
                        novaAgenda.CO_ALU = res.CO_ALU;
                        novaAgenda.TP_CONSU = res.TP_CONSU;
                        novaAgenda.NU_REGIS_CONSUL = res.NU_REGIS_CONSUL;
                        novaAgenda.FL_CONFIR_CONSUL_SMS = res.FL_CONFIR_CONSUL_SMS;
                        novaAgenda.NU_PLAN_SAUDE = res.NU_PLAN_SAUDE;
                        novaAgenda.TB250_OPERA = res.TB250_OPERA;
                        novaAgenda.TB251_PLANO_OPERA = res.TB251_PLANO_OPERA;
                        novaAgenda.TBS356_PROC_MEDIC_PROCE = res.TBS356_PROC_MEDIC_PROCE;
                        novaAgenda.TBS370_PLANE_AVALI = res.TBS370_PLANE_AVALI;
                ...
                //Atualiza o Grid da Agenda
                CarregaGridHorario(res.CO_COL.GetValueOrDefault(), chkHorDispResCons.Checked);

                //Atualiza as informações do plano de saúde da página
                AuxiliCarregamentos.CarregaOperadorasPlanSaude(ddlOperPlano, false);
                ddlOperPlano.SelectedValue = operadora.ToString();

                AuxiliCarregamentos.CarregaPlanosSaude(ddlPlano, ddlOperPlano, false);
                ddlPlano.SelectedValue = plano.ToString();

                //CarregaProcedimentosConsulta(ddlProcConsul, ddlOperPlano);
                // ddlProcConsul.SelectedValue = procedimento.ToString();

                txtNumeroCartPla.Text = numeroPlano;
        ...
        void CurrentPadraoCadastros_OnAcaoBarraCadastro()
        {
            ...
            string hAgendamento = string.Empty;
            ...
            try
            {
                ...
                foreach (GridViewRow lis in grdHorario.Rows)
                {
                    ...
                    if (((CheckBox)lis.Cells[0].FindControl("ckSelectHr")).Checked)
                    {
                        ...
                        hAgendamento = tbs174.HR_AGEND_HORAR;
                        ...
                        var lstCoProcedimento = ViewState["lstCoProcedimento"] != null ? ViewState["lstCoProcedimento"] as List<string> : new List<string>();
                        ...
                        tbs174.TBS356_PROC_MEDIC_PROCE = TBS356_PROC_MEDIC_PROCE.RetornaPelaChavePrimaria(int.Parse(lstCoProcedimento.FirstOrDefault()));
                        ...
                        var tbs386 = AssociarPlanejamento(tbs174, coAlu, coOper, ddlPlano.SelectedValue, lstCoProcedimento);

                        //Associa o item criado ao agendamento em contexto na tabela TBS389_ASSOC_ITENS_PLANE_AGEND
                        #region Associa o Item ao Agendamento

                        foreach (var item in tbs386)
                        {
                            AssociarItemCriadoPlanejamento(item, tbs174);
                        }
                        ...
                        int idOper = tbs174.TB250_OPERA != null ? tbs174.TB250_OPERA.ID_OPER : 0;
                        int idPlan = tbs174.TB251_PLANO_OPERA != null ? tbs174.TB251_PLANO_OPERA.ID_PLAN : 0;

                        if (GrdProcedimentosModal.Rows.Count > 0)
                        {
                            foreach (var item in lstCoProcedimento)
                            {
                                GravaFinanceiroProcedimentos(TBS356_PROC_MEDIC_PROCE.RetornaPelaChavePrimaria(int.Parse(item)),
                        ...
                        ddlPlano.SelectedValue = idPlan.ToString();
                        ddlOperPlano.SelectedValue = idOper.ToString();
                    }
                }
                ...
                if (ddlSelecAgendamentos.SelectedValue != "2")
                {
            ...
            {
                var msg = (!string.IsNullOrEmpty(hAgendamento)) ? string.Format(" | Não foi possível salvar o agendamento {0}", hAgendamento) : string.Empty;
                AuxiliPagina.EnvioAvisoGeralSistema(this.Page, ex.Message + msg);
            }

            if (ddlSelecAgendamentos.SelectedValue == "2")
            {
                AuxiliPagina.EnvioAvisoGeralSistema(this.Page, "Agendamentos realizados com Sucesso!");
            }
        ...
        /// <summary>
        /// Percorre o Grid da agenda de horário e limpa a seleção dos checkboxs
        /// </summary>
        private void LimparHorariosAgendaSelecionados()
        {
            foreach (GridViewRow li in grdHorario.Rows)
            {
                CheckBox chk = ((CheckBox)li.Cells[0].FindControl("ckSelectHr"));
        ...
        private TBS386_ITENS_PLANE_AVALI AssociarPlanejamento(TBS174_AGEND_HORAR tbs174, int coAlu, int coOper, string coPlano, string coProcedimento)
        {
            LimparAssociacoesPlanejamento(tbs174);

            return CriarItemPlanejamento(tbs174, coAlu, coOper, coPlano, coProcedimento);
        }

        private List<TBS386_ITENS_PLANE_AVALI> AssociarPlanejamento(TBS174_AGEND_HORAR tbs174, int coAlu, int coOper, string coPlano, List<string> lstCoProcedimento)
        {
            //limpa associaçoes referene ao planejamento
            LimparAssociacoesPlanejamento(tbs174);
            var lst386 = new List<TBS386_ITENS_PLANE_AVALI>();

            //percorreo grid de associação dos procedimentos e para cada item cria uma tbs386
            foreach (var coProcedimento in lstCoProcedimento)
            {
                lst386.Add(CriarItemPlanejamento(tbs174, coAlu, coOper, coPlano, coProcedimento));
            }
        ...
        private void LimparAssociacoesPlanejamento(TBS174_AGEND_HORAR tbs174)
        {
            ...
            {
                throw new Exception("Não foi possível fazer a associação ao planejamento.");
            }

        }

        private TBS386_ITENS_PLANE_AVALI CriarItemPlanejamento(TBS174_AGEND_HORAR tbs174, int coAlu, int coOper, string coPlano, string coProcedimento)
        {
            ...
            try
            {
                ...
                tbs386.TBS356_PROC_MEDIC_PROCE = (!string.IsNullOrEmpty(coProcedimento) ? TBS356_PROC_MEDIC_PROCE.RetornaPelaChavePrimaria(int.Parse(coProcedimento)) : null);
                ...
                tbs386.ID_PLAN = String.IsNullOrEmpty(coPlano) ? (int?)null : (int?)int.Parse(coPlano);
            ...
            catch (Exception)
            {
                ...
                throw new Exception("Não foi possível cadastrar os itens de planejamento."); ;
            }

            return tbs386;
        }

        private void AssociarItemCriadoPlanejamento(TBS386_ITENS_PLANE_AVALI tbs386, TBS174_AGEND_HORAR tbs174)
        {

            try
            {
                //Associa o item criado ao agendamento em contexto na tabela TBS389_ASSOC_ITENS_PLANE_AGEND
                TBS389_ASSOC_ITENS_PLANE_AGEND tbs389 = TBS389_ASSOC_ITENS_PLANE_AGEND.RetornaTodosRegistros().FirstOrDefault(x => x.TBS386_ITENS_PLANE_AVALI.ID_ITENS_PLANE_AVALI == tbs386.ID_ITENS_PLANE_AVALI);
                if (tbs389 == null)
                {
            ...
            catch (Exception)
            {
                ...
                throw new Exception("Não foi possível associar um item ao planejamento.");
            }
        }
        ...
        public void CarregaLocalRecep(DropDownList ddlLocalRecep)
        {
        ...
        /// <summary>
        /// Carrega os departamentos de acordo com a unidade logada
        /// </summary>
        /// 

        public void CarregarDepartamentosUnidades()
        {
        ...
        private void CarregarDepartamentosUnidade()
        {

            var res = TB14_DEPTO.RetornaTodosRegistros().Where(p => p.TB25_EMPRESA.CO_EMP == LoginAuxili.CO_EMP);

            ddlLocalAtend.DataTextField = "NO_DEPTO";
            ddlLocalAtend.DataValueField = "CO_DEPTO";
            ddlLocalAtend.DataSource = res;
            ddlLocalAtend.DataBind();

            var dept = res.FirstOrDefault();

            if (dept != null)
            {
                ddlDept.SelectedValue = dept.CO_DEPTO.ToString();
            }
            ddlLocalAtend.Items.Insert(0, new ListItem("Todos", ""));
        ...
        private void CarregaInfosAgenda(int CO_ALU, string TP_CONSUL)
        {
            if (CO_ALU != 0)
            {
                if (ddlNomeUsu.Items.FindByValue(CO_ALU.ToString()) == null)
                {
                    var result = (from tb07 in TB07_ALUNO.RetornaTodosRegistros()
        ...
        private void LimparDadosAgenda(TBS174_AGEND_HORAR tbs)
        {
            //Limpa os dados do agendamento no item atual
            tbs.CO_EMP_ALU = null;
            tbs.CO_ALU = null;
            tbs.TP_CONSU = null;
            tbs.NU_REGIS_CONSUL = null;
            tbs.FL_CONFIR_CONSUL_SMS = null;
            tbs.NU_PLAN_SAUDE = null;
            tbs.TB250_OPERA = null;
            tbs.TB251_PLANO_OPERA = null;
            tbs.TBS356_PROC_MEDIC_PROCE = null;
            tbs.TBS370_PLANE_AVALI = null;

        }

        /// <summary>
        /// Copia os dados criando um novo objeto para poder alterar o horário da agenda
        /// </summary>
        private TBS174_AGEND_HORAR MapearNovaAgenda(TBS174_AGEND_HORAR tbs)
        {
            var novaAgenda = new TBS174_AGEND_HORAR();
            novaAgenda.DT_AGEND_HORAR = tbs.DT_AGEND_HORAR;
            novaAgenda.HR_AGEND_HORAR = tbs.HR_AGEND_HORAR;
            novaAgenda.CO_EMP_ALU = tbs.CO_EMP_ALU;
            novaAgenda.CO_ALU = tbs.CO_ALU;
            novaAgenda.TP_CONSU = tbs.TP_CONSU;
            novaAgenda.CO_SITUA_AGEND_HORAR = tbs.CO_SITUA_AGEND_HORAR;
            novaAgenda.DT_SITUA_AGEND_HORAR = tbs.DT_SITUA_AGEND_HORAR;
            novaAgenda.CO_EMP_SITUA = tbs.CO_EMP_SITUA;
            novaAgenda.CO_COL_SITUA = tbs.CO_COL_SITUA;
            novaAgenda.CO_EMP = tbs.CO_EMP;
            novaAgenda.CO_COL = tbs.CO_COL;
            novaAgenda.FL_CONF_AGEND = tbs.FL_CONF_AGEND;
            novaAgenda.FL_AGEND_CONSU = tbs.FL_AGEND_CONSU;
            novaAgenda.CO_DEPT = tbs.CO_DEPT;
            novaAgenda.FL_ENCAI_AGEND = tbs.FL_ENCAI_AGEND;
            novaAgenda.NU_REGIS_CONSUL = tbs.NU_REGIS_CONSUL;
            novaAgenda.FL_CONFIR_CONSUL_SMS = tbs.FL_CONFIR_CONSUL_SMS;
            novaAgenda.TB250_OPERA = tbs.TB250_OPERA;
            novaAgenda.TB251_PLANO_OPERA = tbs.TB251_PLANO_OPERA;
            novaAgenda.NU_PLAN_SAUDE = tbs.NU_PLAN_SAUDE;
            novaAgenda.TBS356_PROC_MEDIC_PROCE = tbs.TBS356_PROC_MEDIC_PROCE;
            novaAgenda.HR_DURACAO_AGENDA = tbs.HR_DURACAO_AGENDA;
            novaAgenda.ID_DEPTO_LOCAL_RECEP = tbs.ID_DEPTO_LOCAL_RECEP;
            novaAgenda.TBS370_PLANE_AVALI = tbs.TBS370_PLANE_AVALI;
            novaAgenda.VL_CONSU_BASE = tbs.VL_CONSU_BASE;
            novaAgenda.VL_DESCT = tbs.VL_DESCT;
            novaAgenda.VL_CONSUL = tbs.VL_CONSUL;
        ...
        {
            hidCoResp.Value = string.Empty;
            txtCPFResp.Text = string.Empty;
            txtNomeResp.Text = string.Empty;
            ddlSexResp.SelectedValue = string.Empty;
            txtDtNascResp.Text = string.Empty;
            ddlGrParen.SelectedValue = string.Empty;
            txtNuIDResp.Text = string.Empty;
            txtOrgEmiss.Text = string.Empty;
            ddlUFOrgEmis.SelectedValue = string.Empty;
            txtTelFixResp.Text = string.Empty;
            txtTelCelResp.Text = string.Empty;
            txtTelComResp.Text = string.Empty;
            txtNuWhatsResp.Text = string.Empty;
            txtDeFaceResp.Text = string.Empty;
            chkPaciEhResp.Checked = false;
            chkPaciMoraCoResp.Checked = false;
            chkRespFinanc.Checked = false;
            txtCEP.Text = string.Empty;
            ddlUF.SelectedValue = string.Empty;
            ddlCidade.SelectedValue = string.Empty;
            ddlBairro.SelectedValue = string.Empty;
            txtLograEndResp.Text = string.Empty;
            txtEmailResp.Text = string.Empty;
            upImagemAluno.CarregaImagem(0);
            txtNuNisPaci.Text = string.Empty;
            txtCPFMOD.Text = string.Empty;
            txtnompac.Text = string.Empty;
            ddlSexoPaci.SelectedValue = string.Empty;
            txtDtNascPaci.Text = string.Empty;
            ddlOrigemPaci.SelectedValue = string.Empty;
            txtNuCarSaude.Text = string.Empty;
            txtTelResPaci.Text = string.Empty;
            txtTelCelPaci.Text = string.Empty;
            txtWhatsPaci.Text = string.Empty;
            txtEmailPaci.Text = string.Empty;
            txtApelido.Text = string.Empty;
            ddlIndicacao.SelectedValue = string.Empty;
            ddlOperadora.SelectedValue = string.Empty;
            ddlPlanoS.SelectedValue = string.Empty;
            ddlCategoria.SelectedValue = string.Empty;
            txtNumeroPlano.Text = string.Empty;
            txtDtVencPlan.Text = string.Empty;
            TextBoxNumeroPasta.Text = string.Empty;
        ...
        private void CarregaGridHorarioProfi(int coCol)
        {
            ...
            var res = ObterGridHorario(coCol);

            grdHorario.DataSource = res;
            grdHorario.DataBind();

            VerificarPresencaPaciente();
        }

        private dynamic ObterGridHorario(int coCol)
        {
            DateTime dataAtu = DateTime.Now;

            return (from a in TBS174_AGEND_HORAR.RetornaTodosRegistros()
        ...
        /// Executa método javascript que mostra a Modal com o Registro de Informações Financeiras
        /// </summary>
        private void abreModalSelecProcedimentos()
        {
            lblHorario.Text = ObterHorarioSelecionado();

            ScriptManager.RegisterStartupScript(
                this.Page,
                this.GetType(),
                "Acao",
                "AbreModalSelProcedimento();",
                true
            );
        ...
        private void CarregaGridHorario(int coCol, bool HorariosDisponiveis)
        {
            ...
            //Obtem o departamento selecionado pelo usuário
            int coDpto = !string.IsNullOrEmpty(ddlLocalAtend.SelectedValue) ? int.Parse(ddlLocalAtend.SelectedValue) : 0;

            //Pesquisa a agenda de consultas do colaborador filtrando as datas da funcionalidade
            var res = ObterHorariosProfissional(coCol, HorariosDisponiveis, dtIni, dtFim, coDpto);
        ...
        private void CarregaGridHorario(List<int> coCol, bool HorariosDisponiveis)
        {
            //CarregaDatasIniFim(coCol);
            if (String.IsNullOrEmpty(txtDtIniResCons.Text))
                txtDtIniResCons.Text = DateTime.Now.ToShortDateString();
        ...
        /// <summary>
        /// Verifica se a linha do grid de horário possui agendamento, se possuir habilita o chk de presença
        /// </summary>
        private void VerificarPresencaPaciente()
        {

            foreach (GridViewRow linha in grdHorario.Rows)
            {
                CheckBox chek = ((CheckBox)linha.Cells[5].FindControl("ckConf"));
                HiddenField nome = ((HiddenField)linha.Cells[0].FindControl("hidNoPac"));

                if (!string.IsNullOrEmpty(nome.Value) && nome.Value.Trim() != "-")
                {
                    chek.Enabled = true;
                }
            }
        ...
        private dynamic ObterHorariosProfissional(int coCol, bool horariosDisponiveis, DateTime dtInici, DateTime dtFimC, int? coDpto = null)
        {
            //Trata as datas para poder compará-las com as informações no banco
            string dataConver = dtInici.ToString("yyyy/MM/dd");
            DateTime dtIni = DateTime.Parse(dataConver);

            //Trata as datas para poder compará-las com as informações no banco
            string dataConverF = dtFimC.ToString("yyyy/MM/dd");
            DateTime dtFim = DateTime.Parse(dataConverF);

            var res = (from a in TBS174_AGEND_HORAR.RetornaTodosRegistros()
                       where a.CO_COL == coCol
                       && (horariosDisponiveis ? a.CO_ALU == null : true)
                       && (EntityFunctions.TruncateTime(a.DT_AGEND_HORAR) >= EntityFunctions.TruncateTime(dtIni) && EntityFunctions.TruncateTime(a.DT_AGEND_HORAR) <= EntityFunctions.TruncateTime(dtFim))
                       //&& coDpto == a.CO_DEPT
                       select new HorarioSaida
                       {
                           dt = a.DT_AGEND_HORAR,
            ...
            if (coDpto != null && coDpto != 0)
            {
                res = res.Where(p => p.CO_DEPTO.ToString() == coDpto.ToString()).ToList();
            }

            return res.OrderBy(b => b.dt).ThenBy(c => c.hr);

        }


        private dynamic ObterHorariosProfissional(List<int> coCol, bool horariosDisponiveis, DateTime dtInici, DateTime dtFimC, int? coDpto = null)
        {
            //Trata as datas para poder compará-las com as informações no banco
            string dataConver = dtInici.ToString("yyyy/MM/dd");
            DateTime dtIni = DateTime.Parse(dataConver);
            ...
            var query = (from a in TBS174_AGEND_HORAR.RetornaTodosRegistros()
                       where coCol.Contains((int)a.CO_COL)
                       &&  (horariosDisponiveis ? a.CO_ALU == null : true)
                       && (EntityFunctions.TruncateTime(a.DT_AGEND_HORAR) >= EntityFunctions.TruncateTime(dtIni) && EntityFunctions.TruncateTime(a.DT_AGEND_HORAR) <= EntityFunctions.TruncateTime(dtFim))
                      // && coDpto == a.CO_DEPT                       
                       select new HorarioSaida
        ...
        /// </summary>
        private void CalcularPreencherValoresTabelaECalculado(List<int> lstProc, int ID_OPER, int ID_PLAN, out List<Tuple<string, string>> valNomeProcedimentos)
        {
            decimal valorBase = 0;
            decimal valorConsulta = 0;

            valNomeProcedimentos = new List<Tuple<string, string>>();
            ...
            if (lstProc.Count > 0)
            {
                ...
                {
                    var valorProcedimento = AuxiliCalculos.RetornaValoresProcedimentosMedicos(proc, ID_OPER, ID_PLAN);
                    valorBase += valorProcedimento.VL_BASE;
                    valorConsulta += valorProcedimento.VL_CALCULADO;

                    valNomeProcedimentos.Add(new Tuple<string, string>(valorProcedimento.NomeProcedimento, valorProcedimento.VL_BASE.ToString("N2")));
                }

                txtVlBase.Text = (valorBase > 0) ? valorBase.ToString("N2") : string.Empty; // Insere o valor base 
                txtVlConsul.Text = txtVlConsulOriginal.Text = (valorConsulta > 0) ? valorConsulta.ToString("N2") : string.Empty;
        ...
        /// </summary>
        private void CarregaProcedimentosConsulta(DropDownList ddl = null, DropDownList ddlOper = null)
        {
            //if (ddl == null)
            //{
            //    ddl = ddlProcConsul;
        ...
        private dynamic ObterProcedimentosConsulta(int idOper)
        {
        ...
        protected void ckSelect_CheckedChangedP(object sender, EventArgs e)
        {
            ...
            List<int> listacoCol = new List<int>();
            ...
            if (listacoCol.Count > 0)
                CarregaGridHorario(listacoCol, chkHorDispResCons.Checked);
            else {
                grdHorario.DataSource = null;
                grdHorario.DataBind();
            }
        ...
        protected void ddlLocalRecepcao(object sender, EventArgs e)
        {
            //CarregaLocalRecep(null);
        ...
        {
            bool agendamentoMultiplos = (ddlSelecAgendamentos.SelectedValue == "2");

            if (agendamentoMultiplos)
            {
                //ddlOperPlano.SelectedValue = string.Empty;
                //ddlPlano.SelectedValue = string.Empty;
                hidCoConsul.Value = string.Empty;
                return;
            }
            ...
            foreach (GridViewRow l in grdHorario.Rows)
            {
                ...
                if (atual.ClientID != chk.ClientID && !agendamentoMultiplos)
                {
                    chk.Checked = false;
                }
                else if (atual.ClientID == chk.ClientID || (chk.Checked && agendamentoMultiplos))
                {
                    ...
                    string hidCoAlu = ((HiddenField)l.Cells[0].FindControl("hidCoAlu")).Value;
                    int coAlu = (!string.IsNullOrEmpty(hidCoAlu) ? int.Parse(hidCoAlu) : 0);

                    string hidCoAgenda = ((HiddenField)l.Cells[0].FindControl("hidCoAgenda")).Value;
                    int coAgen = (!string.IsNullOrEmpty(hidCoAgenda) ? int.Parse(hidCoAgenda) : 0);

                    var idOperador = 0;
                    var idPaciente = 0;

                    //verificar se são multiplos horários para carregar os dados do plano ou não

                    int.TryParse(ddlOperPlano.SelectedValue, out idOperador);
                    int.TryParse(ddlNomeUsu.SelectedValue, out idPaciente);

                    if (idOperador != 0 && idPaciente != 0)
                    {
                        PopularProcedimentoHorario(coAgen, idOperador, idPaciente);
                    }


                    //CarregaInfoPaciente(coAluI);
                    var agenda = TBS174_AGEND_HORAR.RetornaPelaChavePrimaria(coAgen);
                    if (string.IsNullOrEmpty(ddlNomeUsu.SelectedValue) && !agendamentoMultiplos)
                    {
                        if (agenda.CO_ALU != null)
                        {
                            ...
                            ddlOperPlano.SelectedValue = agenda.TB250_OPERA != null ? agenda.TB250_OPERA.ID_OPER.ToString() : string.Empty;
                            CarregaPlanoSaude();
                            ddlPlano.SelectedValue = agenda.TB251_PLANO_OPERA != null ? agenda.TB251_PLANO_OPERA.ID_PLAN.ToString() : string.Empty;
                            //CarregaProcedimentosConsulta();
                            txtNumeroCartPla.Text = agenda.NU_PLAN_SAUDE;

                            GrdProcedimentosModal.DataSource = agenda.TBS389_ASSOC_ITENS_PLANE_AGEND
                                .Where(p => p.TBS174_AGEND_HORAR.ID_AGEND_HORAR == agenda.ID_AGEND_HORAR)
                                .Select(o => o.TBS386_ITENS_PLANE_AVALI.TBS356_PROC_MEDIC_PROCE);
                        ...
                        else
                        {
                            ...
                            CarregaPlanoSaude();
                            //CarregaProcedimentosConsulta();
                            txtNumeroCartPla.Text = string.Empty;
                        }

                        CarregaInfosAgenda(coAlu, tpCon);
                    }

                    if (chk.Checked && !agendamentoMultiplos)
                    {
                        string coAgend = ((HiddenField)l.Cells[0].FindControl("hidCoAgenda")).Value;
                        hidCoConsul.Value = coAgend.ToString();

                        //Guarda o id do horario selecionado
                        ViewState["idChkHorario"] = coAgen;
                        ...
                        if (coAlu != 0)
                        {
                            ...
                            //popula os dropdowns do modal e seta com o valor que está na agenda
                            AuxiliCarregamentos.CarregaOperadorasPlanSaude(ddlOperadoraModal, false);
                            AuxiliCarregamentos.CarregaOperadorasPlanSaude(ddlOperPlano, false);

                            if (agenda.TB250_OPERA != null)
                            {
                                ddlOperadoraModal.SelectedValue = ddlOperPlano.SelectedValue = agenda.TB250_OPERA.ID_OPER.ToString();

                                AuxiliCarregamentos.CarregaPlanosSaude(ddlPlanoModal, ddlOperadoraModal, false);
                                AuxiliCarregamentos.CarregaPlanosSaude(ddlPlano, ddlOperPlano, false);
                                if (agenda.TB251_PLANO_OPERA != null)
                                {
                                    ddlPlanoModal.SelectedValue = ddlPlano.SelectedValue = agenda.TB251_PLANO_OPERA.ID_PLAN.ToString();
                                }

                                //CarregaProcedimentosConsulta(ddlProcedimentoModal, ddlOperadoraModal);

                                //if (agenda.TBS356_PROC_MEDIC_PROCE != null)
                                //{
                                //    ddlProcedimentoModal.SelectedValue = agenda.TBS356_PROC_MEDIC_PROCE.ID_PROC_MEDI_PROCE.ToString();
                                //}
                            }

                            txtNumeroCartPlaModal.Text = txtNumeroCartPla.Text = agenda.NU_PLAN_SAUDE;
        ...
        protected void lnkInfosFinanc_OnClick(object sender, EventArgs e)
        {
            ...
            var verificaVazio = lstCoProcedimento.Where(p => string.IsNullOrEmpty(p));

            foreach (var item in verificaVazio)
            {
            ...
            if (lstCoProcedimento == null || !(lstCoProcedimento.Count > 0))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Você deve selecionar pelo menos um procedimento.");
                return;
            }

            List<int> idsProcedimento = lstCoProcedimento.Select(int.Parse).ToList();

            //TODO: Listar os procedimentos com seus respectivos valores (pegar valores no viewstate)
            ...
            var valNomesProcedimentos = new List<Tuple<string, string>>();

            CalcularPreencherValoresTabelaECalculado(idsProcedimento, idOper, idPlan, out valNomesProcedimentos);

            grdDescPagamento.DataSource = valNomesProcedimentos;
            grdDescPagamento.DataBind();
        ...
        protected void lnkSalvar_OnClick(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(hidCoPac.Value))
            {
                ...
                tb07.TB251_PLANO_OPERA = !string.IsNullOrEmpty(ddlPlanoS.SelectedValue) ? TB251_PLANO_OPERA.RetornaPelaChavePrimaria(int.Parse(ddlPlanoS.SelectedValue)) : null;
            ...
            ddlNomeUsu.Items.Clear();
            var result = (from _tb07 in TB07_ALUNO.RetornaTodosRegistros()
                          where _tb07.CO_ALU == tb07.CO_ALU
                          select new { _tb07.NO_ALU, _tb07.CO_ALU }).OrderBy(w => w.NO_ALU).ToList();
            ...
            ddlPac_SelectedIndexChanged(null, null);
            OcultarPesquisa(true);
            //updTopo.Update();
            LimparCadastroPaciente();
            #endregion
        }
        ...
        /// <summary>
        /// Carrega o plano de saúde e os procedimentos no Modal de edição de horário
        /// </summary>
        protected void ddlOperPlanoModal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        protected void grdHorario_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                foreach (var item in e.Row.Controls)
                {
                    var nn = item.ToString();
                }
                
                DropDownList ddlLocalRecep = ((DropDownList)e.Row.FindControl("ddlLocalRecep"));
                CarregaLocalRecep(ddlLocalRecep);
            }
        }
