...
namespace C2BR.GestorEducacao.UI.GSAUD._3000_ControleInformacoesUsuario._3200_ControleAtendimentoUsuario._3206_RegistroHistorAtendimPaci
{
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                AuxiliCarregamentos.CarregaUnidade(ddlUnidAtend, LoginAuxili.ORG_CODIGO_ORGAO, false);
        ...
        protected void lnkIncMedicaGrid_OnClick(object sender, EventArgs e)
        {
            ...
            tbs330.DT_RECEI = DateTime.Parse(txtDataAtend.Text);
        ...
        protected void lnkIncExam_OnClick(object sender, EventArgs e)
        {
            ...
            tbs218.DT_EXAME = DateTime.Parse(txtDataAtend.Text);
        ...
        protected void lnkIncServAmbu_OnClick(object sender, EventArgs e)
        {
            ...
            tbs332.DT_SERVI_AMBUL = DateTime.Parse(txtDataAtend.Text);
        ...
        protected void lnkIncAtesMedic_OnClick(object sender, EventArgs e)
        {
            ...
            tbs333.DT_ATEST_MEDIC = DateTime.Parse(txtDataAtend.Text);
        ...
        /// <summary>
        /// Verifica se existe cadastrado o CID padrão e já preenche as informações
        /// </summary>
        private void VerificaExisteCidPadrao()
        {
            var res = (from tb117 in TB117_CODIGO_INTERNACIONAL_DOENCA.RetornaTodosRegistros()
                       where tb117.CO_CID == "999"
            ...
            if (res == null)
            {
                ...
                //tb117.CO_CID = "999";
                //tb117.NO_CID = "Não atribuído ao Paciente";
                //tb117.CO_SITUA_CID = "A";
                //tb117 = TB117_CODIGO_INTERNACIONAL_DOENCA.SaveOrUpdate(tb117);
                txtCidAtendMed.Text = "999";
                txtDescCID.Text = "Não atribuído ao Paciente";
            }

            //var recid = TB117_CODIGO_INTERNACIONAL_DOENCA.RetornaTodosRegistros().Where(w => w.CO_CID == "999").FirstOrDefault();

            //txtCidAtendMed.Text = recid.CO_CID;
            //txtDescCID.Text = recid.NO_CID;
        }
        ...
        private bool SalvaAtendimento()
        {
            ...
            if (string.IsNullOrEmpty(ddlUnidAtend.SelectedValue))
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Para efetivar o Atendimento Médico, é preciso selecionar a Unidade do Atendimento");
            ...
            tbs219.CO_EMP = int.Parse(ddlUnidAtend.SelectedValue);
            ...
            if (string.IsNullOrEmpty(hidCoPreAtend.Value))
            {
                ...
                #region Resgata informações do paciente

                int idp = int.Parse(hidCoPac.Value);
                var dadosPaci = (from t07 in TB07_ALUNO.RetornaTodosRegistros()
                                 where t07.CO_ALU == idp
                                 select new { t07.CO_SEXO_ALU, t07.DT_NASC_ALU }).FirstOrDefault();
                ...
                if (dadosPaci != null)
                {
                    tbs194.CO_SEXO_USU = dadosPaci.CO_SEXO_ALU;
                    tbs194.DT_NASC_USU = dadosPaci.DT_NASC_ALU ?? DateTime.Now;
                }
                else
                {
        ...
        private void CarregarPacientes()
        {
            ...
            ddlNomPacienteSelec.DataTextField = "NO_ALU";
            ddlNomPacienteSelec.DataValueField = "CO_ALU";
            ddlNomPacienteSelec.DataSource = res;
            ddlNomPacienteSelec.DataBind();

            ddlNomPacienteSelec.Items.Insert(0, new ListItem("Selecione", ""));
        }

        /// <summary>
        /// Executa os processos necessários ao marcar o paciente
        /// </summary>
        /// <param name="tb07"></param>
        private void AoMarcarPaciente(TB07_ALUNO tb07)
        {
            txtDtNascUsuAteMed.Text = (tb07.DT_NASC_ALU.HasValue ? tb07.DT_NASC_ALU.Value.ToString("dd/MM/yyyy") : "");
            txtIdadeUsuAteMed.Text = (tb07.DT_NASC_ALU.HasValue ? CalculaIdadeUsu(tb07.DT_NASC_ALU.Value) : "");

            lblIdadeCalc.Text = tb07.DT_NASC_ALU.HasValue ? CarregaIdadePaciente(tb07.DT_NASC_ALU.Value) : "";
            lblDtNascPaci.Text = tb07.DT_NASC_ALU.HasValue ? "Nasc " + tb07.DT_NASC_ALU.Value.ToString("dd/MM/yyyy") : "Nasc - ";
            ...
            //var tb07ob = TB07_ALUNO.RetornaTodosRegistros().Where(w => w.CO_ALU == tb07.CO_ALU).FirstOrDefault();
            ...
            CarregaInfosAbas(tb07.CO_ALU);

            txtPaciente.Text = tb07.NO_ALU;

            //Atende os padrões de selecionar o checkbox da aba seguinte e mostrar o label OK!
            if ((!string.IsNullOrEmpty(ddlProfSaude.SelectedValue)) && (!string.IsNullOrEmpty(ddlNomPacienteSelec.SelectedValue)) && (!string.IsNullOrEmpty(ddlUnidAtend.SelectedValue)))
            {
        ...
        /// <summary>
        /// Executa os processos necessiários ao desmarcar paciente
        /// </summary>
        private void AoDesmarcarPaciente()
        {
        ...
        /// <summary>
        /// Método responsável por marcar o paciente de acordo com informações recebidas
        /// </summary>
        private void MarcarPaciente()
        {
            string cpf = txtPesqCpfPaci.Text.Replace(".", "").Replace("-", "").Trim();
            decimal CartSaude = (!string.IsNullOrEmpty(txtPesqNuCartaoSaude.Text) ? decimal.Parse(txtPesqNuCartaoSaude.Text) : 0);
            decimal nis = (!string.IsNullOrEmpty(txtPesqNuNisPaci.Text) ? decimal.Parse(txtPesqNuNisPaci.Text) : 0);

            TB07_ALUNO tb07 = (from rtb07 in TB07_ALUNO.RetornaTodosRegistros()
                               where (chkPesqCPFUsu.Checked ? rtb07.NU_CPF_ALU == cpf : 0 == 0)
                               && (chkPesqNuCarPac.Checked ? rtb07.NU_CARTAO_SAUDE_ALU == CartSaude : 0 == 0)
                               && (chkPesqNuNisPac.Checked ? rtb07.NU_NIS == nis : 0 == 0)
                               select rtb07).FirstOrDefault();

            if (tb07 != null)
            {
                ListItem it = ddlNomPacienteSelec.Items.FindByValue(tb07.CO_ALU.ToString());
                if (it != null)
                {
                    ddlNomPacienteSelec.SelectedValue = hidCoPac.Value = tb07.CO_ALU.ToString();
                    tb07.TB108_RESPONSAVELReference.Load();

                    if (tb07.TB108_RESPONSAVEL != null)
                        hidCoResp.Value = tb07.TB108_RESPONSAVEL.CO_RESP.ToString();
                    else //Se não tiver responsável, salva um novo responsável com as informações dele mesmo
                    {
                        tb07.TB905_BAIRROReference.Load();

                        if (tb07.TB905_BAIRRO != null)
                            tb07.TB905_BAIRRO.TB904_CIDADEReference.Load();

                        TB108_RESPONSAVEL tb108 = (from rtb108 in TB108_RESPONSAVEL.RetornaTodosRegistros()
                                                   where rtb108.NU_CPF_RESP == tb07.NU_CPF_ALU
                                                   select rtb108).FirstOrDefault();

                        if (tb108 == null)
                        {
                            tb108 = new TB108_RESPONSAVEL();

                            tb108.NO_RESP = tb07.NO_ALU;
                            tb108.NU_CPF_RESP = tb07.NU_CPF_ALU;
                            tb108.CO_RG_RESP = tb07.CO_RG_ALU;
                            tb108.CO_ORG_RG_RESP = tb07.CO_ORG_RG_ALU;
                            tb108.CO_ESTA_RG_RESP = tb07.CO_ESTA_RG_ALU;
                            tb108.DT_NASC_RESP = tb07.DT_NASC_ALU;
                            tb108.CO_SEXO_RESP = tb07.CO_SEXO_ALU;
                            tb108.CO_CEP_RESP = tb07.CO_CEP_ALU;
                            tb108.CO_ESTA_RESP = tb07.CO_ESTA_ALU;
                            tb108.CO_CIDADE = (tb07.TB905_BAIRRO != null ? tb07.TB905_BAIRRO.TB904_CIDADE != null ? tb07.TB905_BAIRRO.TB904_CIDADE.CO_CIDADE : (int?)null : (int?)null);
                            tb108.CO_BAIRRO = (tb07.TB905_BAIRRO != null ? tb07.TB905_BAIRRO.CO_BAIRRO : (int?)null);
                            tb108.DE_ENDE_RESP = tb07.DE_ENDE_ALU;
                            tb108.DES_EMAIL_RESP = tb07.NO_WEB_ALU;
                            tb108.NU_TELE_CELU_RESP = tb07.NU_TELE_CELU_ALU;
                            tb108.NU_TELE_RESI_RESP = tb07.NU_TELE_RESI_ALU;
                            tb108.NU_TELE_WHATS_RESP = tb07.NU_TELE_WHATS_ALU;
                            tb108.NU_TELE_COME_RESP = tb07.NU_TELE_COME_ALU;
                            ...
                            TB108_RESPONSAVEL.SaveOrUpdate(tb108);
                        }

                        //Salva o paciente com a referência ao responsável
        ...
        {
            AuxiliCarregamentos.CarregaUnidade(ddlUnidPesqProfi, LoginAuxili.ORG_CODIGO_ORGAO, true);
        }

        /// <summary>
        /// Carrega os profissionais de saúde de acordo com a empresa selecionada
        /// </summary>
        private void CarregarProfissionaisSaude()
        {
            int coEmp = (!string.IsNullOrEmpty(ddlUnidPesqProfi.SelectedValue) ? int.Parse(ddlUnidPesqProfi.SelectedValue) : 0);
            AuxiliCarregamentos.CarregaMedicos(ddlProfSaude, coEmp, false, true);
        }
        ...
        {
            var resCons = (from tb03o in TB03_COLABOR.RetornaTodosRegistros()
                           select new { tb03o.CO_COL, tb03o.NO_COL }).OrderBy(w => w.NO_COL).ToList().Distinct();
        ...
        private void CarregaSessionReserMedicamentos(int coProd)
        {
            ...
            if (string.IsNullOrEmpty(hidCoReservaMedicamentos.Value))
            {
                ...
                tb092.DT_RESER_MEDIC = DateTime.Parse(txtDataAtend.Text);
        ...
        protected void chkselectEn_OnCheckedChanged(object sender, EventArgs e)
        {
            ...
            //                ddlMedico.SelectedValue = ddlMedicoEn.SelectedValue = hidCoProfSaud.Value = coCol.ToString();
        ...
        protected void chkselectCon_OnCheckedChanged(object sender, EventArgs e)
        {
            ...
            //                int coCol = int.Parse(((HiddenField)linha.Cells[0].FindControl("hidCoCol")).Value);
            //                ddlMedico.SelectedValue = ddlMedicoCo.SelectedValue = hidCoProfSaud.Value = coCol.ToString();
        ...
        protected void chkPesqNuNisPac_OnCheckedChanged(object sender, EventArgs e)
        {
            if (chkPesqNuNisPac.Checked)
                chkPesqCPFUsu.Checked = chkPesqNuCarPac.Checked = false;
        }

        protected void chkPesqNuCarPac_OnCheckedChanged(object sender, EventArgs e)
        {
            if (chkPesqNuCarPac.Checked)
                chkPesqNuNisPac.Checked = chkPesqCPFUsu.Checked = false;
        }

        protected void chkPesqCPFUsu_OnCheckedChanged(object sender, EventArgs e)
        {
            if (chkPesqCPFUsu.Checked)
                chkPesqNuNisPac.Checked = chkPesqNuCarPac.Checked = false;
        }
        ...
        protected void ddlUnidPesqProfi_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            CarregarProfissionaisSaude();
        }

        protected void ddlNomPacienteSelec_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            hidCoPac.Value = ddlNomPacienteSelec.SelectedValue;
            if (!string.IsNullOrEmpty(ddlNomPacienteSelec.SelectedValue))
            {
                int coalu = int.Parse(hidCoPac.Value);
        ...
        protected void ddlProfSaude_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            ddlMedico.SelectedValue = hidCoProfSaud.Value = ddlProfSaude.SelectedValue;
        ...
        /// <summary>
        /// Carrega as UF's no endereço do paciente
        /// </summary>
        private void CarregaUFCadasUsuar()
        {
        ...
        protected void lnkSalvar_OnClick(object sender, EventArgs e)
        {
            ...
            //{
                ...
                tb07.NU_CARTAO_SAUDE_ALU = (!string.IsNullOrEmpty(txtNuCarSaude.Text) ? decimal.Parse(txtNuCarSaude.Text.Replace(".", "").Trim()) : (decimal?)null);
