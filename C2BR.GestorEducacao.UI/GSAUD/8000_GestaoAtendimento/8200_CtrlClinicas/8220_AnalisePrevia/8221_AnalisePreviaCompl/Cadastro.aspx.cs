...
namespace C2BR.GestorEducacao.UI.GSAUD._8000_GestaoAtendimento._8200_CtrlClinicas._8220_AnalisePrevia._8221_AnalisePreviaCompl
{
                    ...
                    else
                    {
                        if (chk.Checked)
                        {
                            ...
                            var res = (from tbs367 in TBS367_RECEP_SOLIC.RetornaTodosRegistros()
                                       where tbs367.TBS372_AGEND_AVALI.ID_AGEND_AVALI == idAgendAval
                                       select new { tbs367.ID_RECEP_SOLIC, tbs367.CO_ALU }).FirstOrDefault();

                            txtNecessidade.Text = TBS372_AGEND_AVALI.RetornaPelaChavePrimaria(idAgendAval).DE_OBSER_NECES;

                            //Se houve recepção, então houve um cadastro prévio do registro de avaliação 
                            if (res != null)
                            {
                                //Dados da avaliação
                                var resAvali = (from tbs381 in TBS381_AVALI_RECEP.RetornaTodosRegistros()
                                                where tbs381.TBS367_RECEP_SOLIC.ID_RECEP_SOLIC == res.ID_RECEP_SOLIC
                                                select new
                                                {
                                                    tbs381.DE_CONSI_AVALI,
                                                    tbs381.ID_AVALI_RECEP,
                                                }).FirstOrDefault();
