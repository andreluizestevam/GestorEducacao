...
// SUBMÓDULO: RECEPÇÃO E ENCAMINHAMENTO
// OBJETIVO: RECEPÇÃO - UNIFICADA DE ATENDIMENTO DE PACIENTES / AVALIAÇÃO E PROCEDIMENTOS
// DATA DE CRIAÇÃO: 
//----------------------------------------------------------------------------------
    ...
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                FinalizarAgend.Visible = true;
                FinalizarAgend.ToolTip = "Alterar a situação do agendamento para 'ATENDIDO'.";
                if (tb03 != null)
                {
        ...
        public class saidaLog
        {
            ...
            public string DE_TIPO
            {
                get
                {
                    ...
                    switch (this.FL_TIPO)
                    {
                        //Trata quando é PRESENÇA (REGISTRO DE PRESENÇA DO PACIENTE NA CLÍNICA)
                        case "P":
                        ...
                        case "C":
                            ...
                            break;
                        //Trata quando é de ENCAMINHAMENTO PARA ATENDIMENTO CLÍNICO
                        case "E":
                            s = (this.FL_AGEND_ENCAM == "S" ? "Encaminhado para a Atendimento" : "Remoção de encaminhamento");
                            break;
                        //Trata quando é ENCAMINHAMENTO PARA TRIAGEM (AVALIAÇÃO DE RISCO)
                        case "T":
                        ...
                        case "A":
                            ...
                            break;
                        //Trata quando é ATENDIMENTO FINALIZADO
                        case "R":
                            s = "Atendimento realizado";
                            break;
                        //Trata quando é MOVIMENTAÇÃO DE AGENDA - ORIGEM
                        case "W":
                            s = "Movimentação de agendamento";
                            break;
                        //Trata quando é MOVIMENTAÇÃO DE AGENDA - DESTINO
                        case "M":
                            s = "Movimentação de agendamento";
                            break;
                        default:
        ...
        private void CarregaConsultasAgendadas()
        {
            ...
            //ddlLocal.Items.Insert(-1, new ListItem("Todos", (ddlLocal.SelectedValue)));
            int local = ddlLocal.SelectedValue.Equals("") ? -1 : int.Parse(ddlLocal.SelectedValue);
            //ddlLocal.Items.Add("Todos");
            //ddlLocal.Items.Insert(0, new ListItem("Todos", ""));
            ...
            grdAgendamentos.DataSource = res.Where(x => x.CO_SITU != "M" && x.FL_CONSU_AVALIA != "S");            //05/05/17 Where para não mostrar cancelados por movimentação na recepção;
        ...
        {
            grdAgendaAvaliacoes.DataSource = null;
            ...
            var res = (from tbs372 in TBS372_AGEND_AVALI.RetornaTodosRegistros()
                       ...
                       join tb03 in TB03_COLABOR.RetornaTodosRegistros() on tbs372.TBS460_AGEND_AVALI_PROFI.FirstOrDefault().CO_COL_AVALI equals tb03.CO_COL
        ...
        public class saidaAgendaAvaliacoes
        {
            ...
            public string NO_PAC
            {
                ...
                {
                    return (this.NU_NIRE.ToString().PadLeft(7, '0') + " - "
                        + (this.NO_PAC_RECEB.Length > 20 ? this.NO_PAC_RECEB.Substring(0, 20) + "..." : this.NO_PAC_RECEB));
                }
            }
            public int NU_NIRE { get; set; }
            public string NO_PAC_RECEB { get; set; }
                ...
                if ((CO_TIPO_ALTERACAO == "P") || (CO_TIPO_ALTERACAO == "E" || CO_TIPO_ALTERACAO == "T"))
                {
                    ...
                    try
                    {
                        ...
                        System.Data.SqlClient.SqlCommand cmd = new System.Data.SqlClient.SqlCommand(@"UPDATE [BDGC_XX_Modelo].[dbo].[TSN011_AGENDA_CREDENCIADO]
                          ...
                          WHERE ID_AGEND_HORAR = @ID_AGEND_HORAR", conn);
                        cmd.Parameters.AddWithValue("@CO_SITUACAO_AGENDA", tbs174.FL_CONF_AGEND);
                        cmd.Parameters.AddWithValue("@ID_AGEND_HORAR", tbs174.ID_AGEND_HORAR);
                        cmd.ExecuteNonQuery();
                    }
                    catch (Exception ex)
                    {
        ...
        protected void imgPesqAgendaAval_OnClick(object sender, EventArgs e)
        {
            if (!String.IsNullOrEmpty(txtDtIniAgendaAval.Text) && !String.IsNullOrEmpty(txtDtFimAgendaAval.Text))
            {
                ...
                else
                {
                    ...
                    if (t.Days <= 14 || !String.IsNullOrEmpty(txtNomePacPesqAgendAvaliacao.Text))
                        CarregaAgendamentoAvaliacoes();
                    else
        ...
        protected void lnkbAgend_OnClick(object sender, EventArgs e)
        {
            ...
            Response.Redirect("../../../8000_GestaoAtendimento/8100_CtrlAgendaMedica/8120_RegistroConsulMedMod2/Cadastro.aspx//?moduloId=6585&moduloNome=Agendamento%20de%20Atendimento%20de%20Pacientes%20(Usuários%20de%20Saúde)%20-%20Com%20Histórico/Procedimento&moduloId=6585");
        ...
        public class GrdModal
        {
            ...
            public String NO_PACI
            {
                get
                {
                    return (this.CO_PACI.HasValue ? TB07_ALUNO.RetornaPeloCoAlu(CO_PACI.Value).NO_ALU.Length > 30 ? TB07_ALUNO.RetornaPeloCoAlu((int)this.CO_PACI).NO_ALU.Substring(0, 30) + "..." : TB07_ALUNO.RetornaPeloCoAlu((int)this.CO_PACI).NO_ALU : " - ");
                }
            }
            public int CO_COL { get; set; }
