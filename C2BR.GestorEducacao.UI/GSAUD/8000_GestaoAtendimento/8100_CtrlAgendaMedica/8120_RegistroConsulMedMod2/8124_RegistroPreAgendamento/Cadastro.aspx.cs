...
namespace C2BR.GestorEducacao.UI.GSAUD._8000_GestaoAtendimento._8100_CtrlAgendaMedica._8120_RegistroConsulMedMod2._8124_RegistroPreAgendamento
{
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!base.IsPostBack)
            {
                ...
                CarregaUnidades(ddlUnidPreAgend);
                CarregaClassificacoes();
                CarregaDepartamento(ddlDept, ddlUnidResCons);
                //CarregaEspecialidades(ddlClassFunci, ddlUnidResCons);
                CarregaEspecialidades(ddlTipoPreAgend, ddlUnidPreAgend);
                txtDtIniResCons.Text = DateTime.Now.ToShortDateString();
                txtDtFimResCons.Text = DateTime.Now.ToShortDateString();
                txtDtIniPreAgend.Text = DateTime.Now.AddDays(-5).ToShortDateString();
                txtDtFimPreAgend.Text = DateTime.Now.AddDays(30).ToShortDateString();
        ...
        private void Persistencias()
        {
            ...
            foreach (GridViewRow l in grdHrsPreAgend.Rows)
            {
                var chk = (CheckBox)l.FindControl("chkSelectAgend");

                if (chk.Checked)
                {
                    var idAgend = int.Parse(((HiddenField)l.FindControl("hidIdAgenda")).Value);

                    var tbs409 = TBS409_PRE_AGEND.RetornaPelaChavePrimaria(idAgend);
        ...
        /// </summary>
        private void CarregaEspecialidades(DropDownList drpEspc, DropDownList drpUnid)
        {
            int coEmp = (drpUnid.SelectedValue != "" ? int.Parse(drpUnid.SelectedValue) : 0);
            drpEspc.Items.Clear();
            AuxiliCarregamentos.CarregaTiposAgendamento(drpEspc, true, false, false, coEmp, AuxiliCarregamentos.ETiposClassificacoes.agendamento);
        }
        ...
        /// <summary>
        /// Limpar dados pré-agendamento atual
        /// </summary>
        private void LimparDadosPreAgendamento()
        {
            hidCoColSelec.Value =
        ...
        private void CarregaGridPreAgendamentos()
        {
            int unid = (!string.IsNullOrEmpty(ddlUnidPreAgend.SelectedValue) ? int.Parse(ddlUnidPreAgend.SelectedValue) : 0);
            DateTime dtIni = (!string.IsNullOrEmpty(txtDtIniPreAgend.Text) ? DateTime.Parse(txtDtIniPreAgend.Text) : DateTime.Now);
            DateTime dtFim = (!string.IsNullOrEmpty(txtDtFimPreAgend.Text) ? DateTime.Parse(txtDtFimPreAgend.Text) : DateTime.Now);

            var res = (from tbs409 in TBS409_PRE_AGEND.RetornaTodosRegistros()
                       where tbs409.CO_SITU == "A" && (unid != 0 ? tbs409.CO_EMP == unid : 0 == 0)
                       && (ddlTipoPreAgend.SelectedValue != "0" ? tbs409.TP_AGEND == ddlTipoPreAgend.SelectedValue : 0 == 0)
                       && (EntityFunctions.TruncateTime(tbs409.DT_CADAS) >= EntityFunctions.TruncateTime(dtIni)
                       && EntityFunctions.TruncateTime(tbs409.DT_CADAS) <= EntityFunctions.TruncateTime(dtFim))
                       select new HorarioPreAgendado
                       {
                           ID_AGEND = tbs409.ID_AGEND,
                           OPER = tbs409.TB250_OPERA != null ? tbs409.TB250_OPERA.NM_SIGLA_OPER : " - ",
                           DT = tbs409.DT_AGEND,
                           HR = tbs409.HR_AGEND,
                           TP_AGENDA = tbs409.TP_AGEND != null ? tbs409.TP_AGEND : " - ",
                           NO_PAC_RECEB = tbs409.NO_PAC,
                           TELEFONE_PAC = tbs409.NU_TELE_CELU_RESP,

                           Situacao = tbs409.CO_SITU
                       }).OrderBy(w => w.DT).ToList();

            grdHrsPreAgend.DataSource = res;
            grdHrsPreAgend.DataBind();
        }
        ...
        public class HorarioPreAgendado
        {
            ...
            public string NO_PAC
            {
                get
                {
                    return this.NO_PAC_RECEB.Length > 23 ? this.NO_PAC_RECEB.Substring(0, 23) + "..." : this.NO_PAC_RECEB;
                }
            }
            ...
            public string TELEFONE
            {
                get
                {
                    return (AuxiliFormatoExibicao.PreparaTelefone(this.TELEFONE_PAC));
                }
            }
        ...
        protected void chkSelectAgend_OnCheckedChanged(object sender, EventArgs e)
        {
            LimparDadosPreAgendamento();
            CheckBox atual = (CheckBox)sender;

            foreach (GridViewRow l in grdHrsPreAgend.Rows)
            {
                CheckBox chk = (CheckBox)l.FindControl("chkSelectAgend");
                ...
                if (atual.ClientID == chk.ClientID)
                {
                    if (chk.Checked == true)
                    {
                        ...
                        if (tbs409 != null)
                        {
                            if (ddlUnidResCons.Items.FindByValue(tbs409.CO_EMP.ToString()) != null)
                                ddlUnidResCons.SelectedValue = tbs409.CO_EMP.ToString();

                            CarregaClassificacoes();
                            var tpAgend = AuxiliCarregamentos.RecuperarValorClassificacaoPorTipoAgend(tbs409.TP_AGEND);
                            if (ddlClassProfi.Items.FindByValue(tpAgend) != null)
                                ddlClassProfi.SelectedValue = tpAgend;

                            CarregaGridProfi();

                            txtDtIniResCons.Text = txtDtFimResCons.Text = tbs409.DT_AGEND.ToShortDateString();

                            if (!String.IsNullOrEmpty(tbs409.NU_CPF_PAC))
                            {
                                var res = (from tb07 in TB07_ALUNO.RetornaTodosRegistros()
                                           where tb07.NU_CPF_ALU == tbs409.NU_CPF_PAC
                                           select tb07.CO_ALU).FirstOrDefault();

                                if (res != null)
                                {
                                    CarregaPacientes();
                                    if (ddlNomeUsu.Items.FindByValue(res.ToString()) != null)
                                    {
                                        ddlNomeUsu.SelectedValue = res.ToString();
                            ...
                            if (String.IsNullOrEmpty(ddlNomeUsu.SelectedValue))
                            {
                                txtnompac.Text = tbs409.NO_PAC;
                                txtCPFMOD.Text = tbs409.NU_CPF_PAC;
                                txtDtNascPaci.Text = tbs409.DT_NASC_PAC.ToString();
                                ddlSexoPaci.SelectedValue = tbs409.CO_SEXO_PAC;
                                txtTelCelPaci.Text = tbs409.NU_TELE_CELU_PAC;
                                txtTelResPaci.Text = tbs409.NU_TELE_RESI_PAC;
                                ddlGrParen.SelectedValue = tbs409.CO_GRAU_PAREN_RESP;
                                txtEmailPaci.Text = tbs409.NO_EMAIL_PAC;
                                txtWhatsPaci.Text = tbs409.NU_TELE_WHATS_PAC;

                                txtNomeResp.Text = tbs409.NO_RESP;
                                txtCPFResp.Text = tbs409.NU_CPF_RESP;
                                txtNuIDResp.Text = tbs409.CO_RG_RESP;
                                txtOrgEmiss.Text = tbs409.CO_ORG_RG_RESP;
                                ddlUFOrgEmis.SelectedValue = tbs409.CO_ESTA_RG_RESP;
                                txtDtNascResp.Text = tbs409.DT_NASC_RESP.ToString();
                                ddlSexResp.SelectedValue = tbs409.CO_SEXO_RESP;
                                txtCEP.Text = tbs409.CO_ENDE_CEP;
                                ddlUF.SelectedValue = tbs409.CO_ENDE_ESTADO;
                                carregaCidade();
                                ddlCidade.SelectedValue = tbs409.CO_ENDE_CIDADE != null ? tbs409.CO_ENDE_CIDADE.ToString() : "";
                                carregaBairro();
                                ddlBairro.SelectedValue = tbs409.CO_ENDE_BAIRRO != null ? tbs409.CO_ENDE_BAIRRO.ToString() : "";
                                txtLograEndResp.Text = tbs409.DE_ENDE;
                                txtEmailResp.Text = tbs409.DE_EMAIL_RESP;
                                txtTelCelResp.Text = tbs409.NU_TELE_CELU_RESP;
                                txtTelFixResp.Text = tbs409.NU_TELE_RESI_RESP;
                                txtNuWhatsResp.Text = tbs409.NU_TELE_WHATS_RESP;
                                txtTelComResp.Text = tbs409.NU_TELE_COME_RESP;
                            }
                            else
                                AuxiliPagina.EnvioMensagemSucesso(this.Page, "O paciente já esta cadastrado no sistema e foi localizado através do seu CPF");
                        }
                    }
                }
                else
        ...
        protected void imgbExcluirPreAg_OnClick(object sender, EventArgs e)
        {
            ImageButton atual = (ImageButton)sender;

            foreach (GridViewRow l in grdHrsPreAgend.Rows)
            {
                var img = (ImageButton)l.FindControl("imgbExcluirPreAg");

                if (atual.ClientID == img.ClientID)
                {
                    var idAgend = int.Parse(((HiddenField)l.FindControl("hidIdAgenda")).Value);

                    var tbs409 = TBS409_PRE_AGEND.RetornaPelaChavePrimaria(idAgend);

                    TBS409_PRE_AGEND.Delete(tbs409, true);
                    ...
                    tb236.CO_ACAO_ATIVI_LOG = "D";
                    tb236.CO_TABEL_ATIVI_LOG = "TBS409_PRE_AGEND";
                    ...
                    AuxiliPagina.EnvioMensagemSucesso(this.Page, "Pré-Agendamento EXCLUIDO com sucesso!");
                }
            }
        }
        ...
        protected void ddlUnidPreAgend_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            CarregaEspecialidades(ddlTipoPreAgend, ddlUnidPreAgend);
        }
        ...
        {
            CarregaDepartamento(ddlDept, ddlUnidResCons);
            CarregaEspecialidades(ddlClassProfi, ddlUnidResCons);
            //LimparGridHorarios();
        }
        ...
        protected void imgPesqPreAgends_OnClick(object sender, EventArgs e)
        {
            CarregaGridPreAgendamentos();
        }
        ...
        protected void LinkButtonSIM_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    DadosAgenda.Visible = true;
            //    liBtnGrdFinanMater.Visible = false;
            //    li9.Visible = false;
            //    AbreModalPadrao("AbreModalAgendMulti();");
            //}
            //else
