...
namespace C2BR.GestorEducacao.UI.GSAUD._8000_GestaoAtendimento._8100_CtrlAgendaMedica._8120_RegistroConsulMedMod2._8122_ReplicacaoAgendaProf
{
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        private void Persistencias()
        {
            ...
            foreach (GridViewRow lis in grdHorario.Rows)
            {
                ...
                if (((CheckBox)lis.FindControl("ckSelectHr")).Checked)
                {
                    ...
                    var coAlu = int.Parse(((HiddenField)lis.FindControl("hidCoAlu")).Value);
        ...
        {
            txtDtIniResCons.Text = "04/01/" + DateTime.Now.Year.ToString();
            txtDtFimResCons.Text = "23/12/" + DateTime.Now.Year.ToString();
        }
        ...
        protected void imgBtnPesqHistoUsuar_OnClick(object sender, EventArgs e)
        {
            ...
            {
                AuxiliPagina.EnvioMensagemErro(this.Page, "Um profissional precisa ser selecionada para poder executar essa funcionalidade.");
            }
        }
        ...
        protected void ddlNomeUsu_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(ddlNomeUsu.SelectedValue))
            {
                ...
                foreach (GridViewRow l in grdHistorPaciente.Rows)
                {
                    ...
                    //var hrs = CarregarHorarios(CO_COL, coAlu, hora, dia);
                ...
                hrsAtl = hrsAtl.DistinctBy(h => new { h.CO_ALU, h.hora }).ToList();

                Session["Horarios"] = hrsAtl;
                grdHorario.DataSource = hrsAtl.OrderBy(h => h.dt).ThenBy(h => h.hr);
                grdHorario.DataBind();
            }
        }
        ...
        protected void grdHorario_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                var coAlu_ = int.Parse(((HiddenField)e.Row.FindControl("hidCoAlu")).Value);
                var chkH = (CheckBox)e.Row.FindControl("ckSelectHr");
                var lblPac = (Label)e.Row.FindControl("lblPaciente");
                var lblOper = (Label)e.Row.FindControl("lblOperadora");
                var lblPlan = (Label)e.Row.FindControl("lblPlano");

                chkH.Checked = true;

                var res = TB07_ALUNO.RetornaPeloCoAlu(coAlu_);

                if (res != null)
                {
                    lblPac.Text = !String.IsNullOrEmpty(res.NO_APE_ALU) ? res.NO_APE_ALU : res.NO_ALU;
