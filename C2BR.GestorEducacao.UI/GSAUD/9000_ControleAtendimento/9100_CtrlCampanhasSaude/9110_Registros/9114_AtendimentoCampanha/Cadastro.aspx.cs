...
namespace C2BR.GestorEducacao.UI.GSAUD._9000_ControleAtendimento._9100_CtrlCampanhasSaude._9110_Registros._9114_AtendimentoCampanha
{
    public partial class Cadastro : System.Web.UI.Page
    {
        ...
        private bool ExecutaRotinasPersistencia()
        {
            ...
            tbs341.CO_RESP_ALU = (!string.IsNullOrEmpty(ddlResp.SelectedValue) ? int.Parse(ddlResp.SelectedValue) : (int?)null);
        ...
        private bool VerificaExisteAtendimento(int CO_ALU, int ID_CAMPAN)
        {
            if (ID_CAMPAN != 0)
            {
                if (TBS339_CAMPSAUDE.RetornaPelaChavePrimaria(ID_CAMPAN).CO_TIPO_CAMPAN != "V")
                {
                    ...
                    if (res != null)
                    {
                        string nomeUsu = ddlNomeUsu.SelectedItem.Text;
        ...
        public class HistAtendCampa
        {
            ...
            public string dataValid
            {
                get
                {
                    return this.DT_ATEND.ToString("dd/MM/yy") + " " + this.DT_ATEND.ToString("HH:mm");
                }
            }
