...
// ---------+----------------------------+-------------------------------------
// 09/12/2014| Maxwell Almeida           | Funcionalidade para alteração da disponibilidade de procedimentos médicos associados à Unidades
            ...
            foreach (TreeNode node in nodeCollection)
            {
                ...
                foreach (TreeNode nodeAvo in node.ChildNodes)
                {
                    ...
                    foreach (TreeNode nodePai in nodeAvo.ChildNodes)
                    {
                        ...
                        int co_emp = int.Parse(ddlUnidadeTPA.SelectedValue);

                        TBS358_PROC_MEDIC_UNID tbs358 = TBS358_PROC_MEDIC_UNID.RetornaTodosRegistros().Where(w => w.TBS356_PROC_MEDIC_PROCE.ID_PROC_MEDI_PROCE == id_proc_filho && w.TB25_EMPRESA.CO_EMP == co_emp).FirstOrDefault();
                        tbs358.FL_PROC_MEDIC_DISPO = nodePai.Checked ? "S" : "N";
                    ...
                    if (CO_EMP > 0)
                    {
                        ...
                        bool temAssociado = TBS358_PROC_MEDIC_UNID.RetornaTodosRegistros().Where(
                            w => w.TBS356_PROC_MEDIC_PROCE.TBS354_PROC_MEDIC_GRUPO.ID_PROC_MEDIC_GRUPO ==
                                ...
                                && w.TBS356_PROC_MEDIC_PROCE.CO_OPER == "999" && w.FL_PROC_MEDIC_DISPO == "S").Any();
                    ...
                    if (subGruposProc.Count() > 0)
                    {
                        foreach (var resultadoPai in subGruposProc)
                        {
                            ...
                            var itemProceMedic = (from tbs356 in TBS356_PROC_MEDIC_PROCE.RetornaTodosRegistros()
                                                  ...
                                                  && tbs356.CO_OPER == "999"
                                                  && tbs358.TB25_EMPRESA.CO_EMP == CO_EMP
