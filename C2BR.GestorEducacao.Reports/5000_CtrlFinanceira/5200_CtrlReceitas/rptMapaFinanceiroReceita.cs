    ...
    public partial class rptMapaFinanceiroReceita : C2BR.GestorEducacao.Reports.RptPaisagem
    {
                              ...
                              bool incluCance)
        {
            try
            {
                ...
                var rest = (from tb47 in TB47_CTA_RECEB.RetornaTodosRegistros()
                            where (tb47.CO_EMP == strP_CO_EMP)
                            && (coUniContr != 0 ? tb47.CO_EMP_UNID_CONT == coUniContr : 0 == 0)
                            && ((tb47.DT_VEN_DOC >= dtP_DT_INI) && (tb47.DT_VEN_DOC <= dtP_DT_FIM))
                            ...
                            group tb47 by tb47.DT_VEN_DOC into grp
                            select new MapaFinaceiroReceita
                            {
                                // Previsão Financeira
                                DtVenctoPF = grp.Key,

                                VrDocumetoPF = grp.Where(x => x.FL_TIPO_PREV_RECEB == "B").Any() ? grp.Where(x => x.FL_TIPO_PREV_RECEB == "B").Sum(x => x.VR_PAR_DOC) : 0,
                                VrCartaoPF = grp.Where(x => x.FL_TIPO_PREV_RECEB == "C").Any() ? grp.Where(x => x.FL_TIPO_PREV_RECEB == "C").Sum(x => x.VR_PAR_DOC) : 0,
                                VrChequePF = grp.Where(x => x.FL_TIPO_PREV_RECEB == "H").Any() ? grp.Where(x => x.FL_TIPO_PREV_RECEB == "H").Sum(x => x.VR_PAR_DOC) : 0,
                                VrDinheiroPF = grp.Where(x => x.FL_TIPO_PREV_RECEB == "D").Any() ? grp.Where(x => x.FL_TIPO_PREV_RECEB != "D").Sum(x => x.VR_PAR_DOC) : 0,
                                VrMistaPF = grp.Where(x => x.FL_TIPO_PREV_RECEB == "M").Any() ? grp.Where(x => x.FL_TIPO_PREV_RECEB != "M").Sum(x => x.VR_PAR_DOC) : 0,

                                vrBancoRD = grp.Where(x => x.FL_TIPO_RECEB == "B" && x.IC_SIT_DOC == "Q").Sum(x => x.VR_PAG) ?? 0,
                                vrDinheiroRD = grp.Where(x => x.FL_TIPO_RECEB != "D" && x.IC_SIT_DOC == "Q").Sum(x => x.VR_PAG) ?? 0,
                                vrChequeRD = grp.Where(x => x.FL_TIPO_RECEB == "H" && x.IC_SIT_DOC == "Q").Sum(x => x.VR_PAG) ?? 0,
                                vrCartaoRD = grp.Where(x => x.FL_TIPO_RECEB == "C" && x.IC_SIT_DOC == "Q").Sum(x => x.VR_PAG) ?? 0,
                                vrMistaRD = grp.Where(x => x.FL_TIPO_RECEB == "M" && x.IC_SIT_DOC == "Q").Sum(x => x.VR_PAG) ?? 0,
                                vrJrsMultRD = (grp.Sum(x => x.VR_JUR_PAG) ?? 0) + (grp.Sum(x => x.VR_MUL_PAG) ?? 0),

                                QdtVenc = grp.Where(x => x.IC_SIT_DOC == "A" && grp.Key < DateTime.Today).Count(),
                                VencTotal = grp.Where(x => x.IC_SIT_DOC == "A" && grp.Key < DateTime.Today).Sum(x => x.VR_PAR_DOC)
