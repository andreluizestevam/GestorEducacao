...
{
    public partial class RptPosicaoFinancConsCtaPagar : C2BR.GestorEducacao.Reports.RptPaisagem
    {
        public RptPosicaoFinancConsCtaPagar()
        {
            InitializeComponent();
        }
                                ...
                                string infos)
        {
            try
            {
                ...
                if (strP_TIPO == "C")
                {
                    lst = (from tb47 in ctx.TB38_CTA_PAGAR
                ...
                else if (strP_TIPO == "M")
                {
                    lst = (from tb39 in ctx.TB39_HISTORICO
                           from tb47 in tb39.TB38_CTA_PAGAR
                           where
                           strSIT_PAG.Contains(tb47.IC_SIT_DOC) &&
                          (tb47.DT_CAD_DOC >= strP_DT_INI && tb47.DT_CAD_DOC <= strP_DT_fIM)
                           group tb47 by new
