...
{
    public partial class RptDiarioClasseFrenteBimSupremoPg2 : DevExpress.XtraReports.UI.XtraReport
    {
        public RptDiarioClasseFrenteBimSupremoPg2()
        {
            InitializeComponent();
        }
        #region Init Report
                             ...
                             )
        {
            try
            {
                ...
                //Apenas se houverem mais de 30 registros (evita um erro de exceção) é retornado como não existe mais nada
                if(!(lstDtFreq.Count >= 31))
                    return null;
                ...
                #region Retorna os dados dos aluno
                ...
                var lst = (from tb132 in ctx.TB132_FREQ_ALU
                           ...
                           select new DiarioFrente.DiarioClasseFrente
                           {
                               ...
                               VL_NOTA_BIM = (ImprimeMedias ? (strP_BIMESTRE == "B1" ? tb079.VL_MEDIA_BIM1 : strP_BIMESTRE == "B2" ? tb079.VL_MEDIA_BIM2 :
                               strP_BIMESTRE == "B3" ? tb079.VL_MEDIA_BIM3 : tb079.VL_MEDIA_BIM4) : (decimal?)null),
                               VL_RECU_BIM = (ImprimeMedias ? (strP_BIMESTRE == "B1" ? tb079.VL_RECU_BIM1 : strP_BIMESTRE == "B2" ? tb079.VL_RECU_BIM2 :
                               strP_BIMESTRE == "B3" ? tb079.VL_RECU_BIM3 : tb079.VL_RECU_BIM4) : (decimal?)null),
                               coTur = tb132.CO_TUR,
                ...
                int c = 1;
                int tf = 0;
                foreach (DiarioFrente.DiarioClasseFrente dcf in res)
                {
                    ...
                    foreach (DiarioFrente.RelDataFre rdf in lstDtFreq)
                    {
                        ...
                        if (rdf.NR_TEMPO != null)
                        {
                            ...
                            //Me interessam apenas os registros após 30º, pois os 30 primeiro estão na primeira página
                            if (auxQtDiasFreq >= 31)
                            {
                                //foreach (var v in lstFreq)
                                //{
                ...
                bsReport.Clear();
                foreach (DiarioFrente.DiarioClasseFrente at in res)
                    bsReport.Add(at);
