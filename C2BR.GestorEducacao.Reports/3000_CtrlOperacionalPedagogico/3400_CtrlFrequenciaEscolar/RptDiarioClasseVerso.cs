    ...
    public partial class RptDiarioClasseVerso : DevExpress.XtraReports.UI.XtraReport
    {
                                ...
                                )
        {
            try
            {
                ...
                if ((from resp in ctx.TB_RESPON_MATERIA where resp.CO_MODU_CUR == strP_CO_MODU_CUR 
                         && resp.CO_CUR == strP_CO_CUR
                         && resp.CO_TUR == strP_CO_TUR
                         && resp.CO_ANO_REF == intAno
                         && coFlagResp != "S" ? resp.CO_MAT == strP_CO_MAT : 0 == 0 select resp).Any())
                {
                    if (coFlagResp == "S")
                    {
