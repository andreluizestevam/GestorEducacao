...
{
    public partial class RptOrcamento2 : C2BR.GestorEducacao.Reports.RptRetratoCentro
    {
        public RptOrcamento2()
        {
            InitializeComponent();
        }
                        ...
                        )
        {
            ...
            try
            {
                ...
                //Definir lable da grid
                lblLegenda.Text = "CONTRAT (Plano de contratação) | ND (Número do Dente [TD - Todos] [NE - Não Especificado]) | QTD (Quantidade de Itens) | R$ UNIT (Valor Unitário) | R$ DESC (Valor Desconto)";
                ...
                var res = (from tbs390 in TBS390_ATEND_AGEND.RetornaTodosRegistros()
                           ...
                           join tb25 in TB25_EMPRESA.RetornaTodosRegistros() on tbs396.CO_EMP_CADAS equals tb25.CO_EMP
                           join tb904 in TB904_CIDADE.RetornaTodosRegistros() on tb25.CO_CIDADE equals tb904.CO_CIDADE
                           join tb250 in TB250_OPERA.RetornaTodosRegistros() on tbs396.CO_CONTRAT equals tb250.ID_OPER
                           join tb251 in TB251_PLANO_OPERA.RetornaTodosRegistros() on tb250.ID_OPER equals tb251.TB250_OPERA.ID_OPER
                           ...
                           select new GuiaExame
                           {
                               ...
                               Unit = tbs396.VL_PROC,
                               obs = tbs396.DE_OBSER,
                               DataValidade = tbs390.DT_VALID_ORCAM,
                               Desconto = tbs396.VL_DSCTO_PROC == null ? 0 : tbs396.VL_DSCTO_PROC,
                               ...
                               Plano = tb251.NM_SIGLA_PLAN,
                               QTD = tbs396.QT_PROC,
                               DENTE = tbs396.NU_DENTE == null ? 0 : tbs396.NU_DENTE.Value,
                           }).ToList();
        ...
        public class GuiaExame
        {
            ...
            public string DENTE_V
            {
                get
                {
                    return this.DENTE == 0 ? "TD" : this.DENTE == 99 ? "NE" : this.DENTE.ToString();
                }
            }
            ...
            public decimal TotalFinal
            {
                get
                {
                    return (this.Unit * this.QTD) - this.Desconto.Value;
                }
            }
            ...
            public string Validade
            {
                get
                {
                    ...
                    return DataValidade.HasValue ? DataValidade.Value.ToShortDateString() : " - ";
                }
            }
            ...
            public string DescPaci
            {
                get
                {
                    return "Nascimento: " + this.NascPac + " - Idade: " + this.idadePac + " - Sexo: " + this.SexoPac;
                }
            }
