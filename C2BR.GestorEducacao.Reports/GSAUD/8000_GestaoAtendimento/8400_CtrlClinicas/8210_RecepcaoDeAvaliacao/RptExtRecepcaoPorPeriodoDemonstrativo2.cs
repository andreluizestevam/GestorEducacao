...
namespace C2BR.GestorEducacao.Reports.GSAUD._8000_GestaoAtendimento._8400_CtrlClinicas._8210_RecepcaoDeAvaliacao
{
    public partial class RptExtRecepcaoPorPeriodoDemonstrativo2 : C2BR.GestorEducacao.Reports.RptPaisagem
    {
        ...
        public int InitReport(string parametros,
                              ...
                              int coEmp,
                             int CoUnidade,
                             int Paciente, int Operadora, int Plano, int Categoria, string dataIni, string dataFim, string OrdenadoPor, string Status, string Titulo
            )
        {
            try
            {
                ...
                if (Status == "AA")
                {
                    var res = (from tbs367 in TBS367_RECEP_SOLIC.RetornaTodosRegistros()
                               ...
                               select new Extrato
                               {
                                   ...
                                   TelefoneRecebe = tb07.NU_TELE_RESI_ALU == null ? "-" : tb07.NU_TELE_RESI_ALU == "" ? "-" : tb07.NU_TELE_RESI_ALU,
                ...
                else
                {
                    var res = (from tbs174 in TBS174_AGEND_HORAR.RetornaTodosRegistros()
                               ...
                               join tb03 in TB03_COLABOR.RetornaTodosRegistros() on tbs174.CO_COL_ENCAM equals tb03.CO_COL
                               where (CoUnidade != 0 ? tbs174.CO_EMP == CoUnidade : 0 == 0)
                               && (Paciente != 0 ? tb07.CO_ALU == Paciente : 0 == 0)
                               && (tbs174.FL_AGEND_ENCAM == "S")
                               && ((tbs174.DT_ENCAM >= dataIni1) && (tbs174.DT_ENCAM <= dataFim1))
                               select new Extrato
                               {
                                   IdRecep_Solic = 0,
                                   DataHoraRecebe = tbs174.DT_ENCAM.Value,
                                   Paciente = tb07.NO_ALU,
                                   Neri = tb07.NU_NIRE,
                                   Sexo = tb07.CO_SEXO_ALU,
                                   DataNascimento = tb07.DT_NASC_ALU,
        ...
        public class Extrato
        {
            ...
            public string DataHora
            {
                ...
                get
                {
                    ...
                    string data = this.DataHoraRecebe.ToString("dd/MM/yy") + " " + this.DataHoraRecebe.ToShortTimeString();
                    return data;
                }
            ...
            public decimal Total
            {
                ...
                get
                {
                    ...
                    {
                        decimal aux = (i.VL_LIQUI.HasValue ? i.VL_LIQUI.Value : 0);
                        vlTotal += aux;
                    }
