            ...
            )
        {
            try
            {
                ...
                if (impAnamnese)
                {
                    ...
                    {
                        var pls = (from ps in TBS390_ATEND_AGEND.RetornaTodosRegistros()
                                   where ps.ID_ATEND_AGEND == a.idAna
                                   && !String.IsNullOrEmpty(ps.DE_ACAO_REALI)
                                   select new PlanAcoes
                                   {
                                       data = a.data,
                                       acao = ps.DE_ACAO_REALI,
                                       idPlan = ps.ID_ATEND_AGEND,
                                       nomProf = a.nomProf
                                       //status = ps.CO_SITUA
                                   }).ToList();
