...
{
    public partial class RptMapaFreqPacientes : C2BR.GestorEducacao.Reports.RptPaisagem
    {
        public RptMapaFreqPacientes()
        {
            InitializeComponent();
        }
            ...
            )
        {
            try
            {
                ...
                var res = (from tbs174 in TBS174_AGEND_HORAR.RetornaTodosRegistros()
                           ...
                           && tbs174.DT_AGEND_HORAR.Month == Mes
                           && tbs174.DT_AGEND_HORAR.Year == Ano
                           select new Relatorio
                           {
                               ...
                               idPlan = (tb07.TB251_PLANO_OPERA != null ? tb07.TB251_PLANO_OPERA.ID_PLAN : (int?)null),
                               idOper = (tb07.TB250_OPERA != null ? tb07.TB250_OPERA.ID_OPER : (int?)null),
                               tcDt01 = 01,
                               tcDt02 = 02,
                               tcDt03 = 03,
                               tcDt04 = 04,
                               ...
                               st29 = "-",
                               st30 = "-",
                           }).DistinctBy(a => a.Paciente).OrderBy(a => a.Paciente).ToList();
                ...
                {
                    DateTime dt = new DateTime(Ano, Mes, i);
                ...
                foreach (var item in res)
                {
                    ...
                    //Coleta a lista de agendamentos do paciente para o mês e ano selecionados
                    var ress = (from tbs174 in TBS174_AGEND_HORAR.RetornaTodosRegistros()
                                ...
                                select new
                                {
                                    ...
                                    StatusAgenda = tbs174.CO_SITUA_AGEND_HORAR,
                                    Data = tbs174.DT_AGEND_HORAR,
                                    idProc = (tbs174.TBS356_PROC_MEDIC_PROCE != null ? tbs174.TBS356_PROC_MEDIC_PROCE.ID_PROC_MEDI_PROCE : (int?)null),
                                    CanceJustif = tbs174.FL_JUSTI_CANCE,
                                }).ToList();
                    ...
                    for (int i = 1; i < Dias; i++)
                    {
                        //Se tiver qualquer agendamento do paciente em contexto para o dia
                        if (ress.Where(w => w.Data.Day == i).Any())
                        {
                            //Cria o objeto do registro para facilitar
                            var obPac = new ObjetoPaciente();
                            obPac.IdAgenda = ress.Where(w => w.Data.Day == i).FirstOrDefault().IdAgenda;
                            obPac.coAlu = ress.Where(w => w.Data.Day == i).FirstOrDefault().coAlu;
                            obPac.Status = ress.Where(w => w.Data.Day == i).FirstOrDefault().Status;
                            obPac.StatusAgenda = ress.Where(w => w.Data.Day == i).FirstOrDefault().StatusAgenda;
                            obPac.Data = ress.Where(w => w.Data.Day == i).FirstOrDefault().Data;
                            obPac.CanceJustif = ress.Where(w => w.Data.Day == i).FirstOrDefault().CanceJustif;
                            string Status = "";

                            //Determina o status que será apresentado
                            #region Verifica o Status

                            //Se a agenda estiver como presente, coleta a quantidade de atendimentos que houveram
                            if (ress.Where(w => w.Data.Day == i).FirstOrDefault().Status == "S")
                            {
                                int status = TBS174_AGEND_HORAR.RetornaTodosRegistros().Where(a => 
                                    a.CO_ALU == obPac.coAlu 
                                    && (CoUnidade != 0 ? a.CO_EMP == CoUnidade : 0 == 0)
                                    && (CoProfissional != 0 ? a.CO_COL == CoProfissional : 0 == 0)
                                    && a.FL_CONF_AGEND == "S"
                                    && a.DT_AGEND_HORAR.Day == i
                                    && a.DT_AGEND_HORAR.Year == Ano
                                    && a.DT_AGEND_HORAR.Month == Mes
                                    ).Count();

                                Status = status.ToString();
                                qtdSessoes += status; // Acrescenta a quantidade total de sessões  
                                item.TotalPR = item.TotalPR + status;
                                qtdTotal += status;
                            }
                            //Se não estiver como presente, verifica se a falta foi justificada
                            else if (obPac.StatusAgenda == "C")
                            {
                                if (obPac.CanceJustif == "S") //Se está com falta justificada
                                {
                                    Status = "FJ";
                                    item.TotalFJ += 1;
                                }
                                else //Se não está com falta justificada
                                {
                                    Status = "F";
                                    item.TotalF += 1;
                                    qtdSessoes++; // Incrementa a quantidade de sessões pois falta não justificada é paga
                                    qtdTotal++;
                                }
                            }
                            //Se ainda estiver como em aberto
                            else if (obPac.StatusAgenda == "A")
                                { Status = "A"; }
                            else if (obPac.StatusAgenda == "M")
                            { Status = "M"; }
                    ...
                    item.QTD = qtdTotal; // Atribui a quantidade total calculada ao atributo correspondente

                    //Calcula o valor total de acordo com a quantidade e procedimentos
                    #region Calcula o valor total

                    //Retorna o cálculo do valor total dos procedimentos
                    if (ress.Count > 0)
                    {
                        if (ress.FirstOrDefault().idProc.HasValue)
                        {
                            decimal ax = (CalcularPreencherValoresTabelaECalculado(ress.FirstOrDefault().idProc.Value, item.idOper, item.idPlan)) * qtdSessoes;
                            item.ValorTotal = ax.ToString("N2");
                            ValorTotalGeral += ax;
                        }
                    }
                ...
                foreach (var item in res)
                {
                    formattingRule1.DataMember = this.DataMember;
                    formattingRule2.DataMember = this.DataMember;
                    bsReport.Add(item);
        ...
        public class ObjetoPaciente
        {
            ...
            public string CanceJustif { get; set; }
        }
        ...
        public class Relatorio
        {
            ...
            public string Nome
            {
                get
                {
                    return Convert.ToString(this.Neri).PadLeft(7, '0') + " - " + (this.Paciente.Length > 24 ? this.Paciente.Substring(0, 24) + "..." : this.Paciente);
                }
            }
            public int COAlu { get; set; }
