...
{
    public partial class RptDemQtdAlunosOutros : C2BR.GestorEducacao.Reports.RptPaisagem
    {
        #region ctor

        public RptDemQtdAlunosOutros()
        {
            InitializeComponent();
        } 
        ...
        public int InitReport(int codEmp, int codOrg, string ano, string situacao, string infos)
        {
            try
            {
                ...
                var res = (from m in ctx.TB08_MATRCUR
                           ...
                           select new ReportAlunosPorUnidSerieOutros
                           {
                               Unidade = grp.Key.NO_FANTAS_EMP,
                               Sigla = grp.Key.sigla,
                               TipoUnidade = grp.Key.NO_TIPOEMP,
                               Inep = grp.Key.NU_INEP,
                               Qtd1A = grp.Count(x => x.cur.CO_SERIE_REFER == "1O"),
                               Qtd2A = grp.Count(x => x.cur.CO_SERIE_REFER == "2O"),
                               Qtd3A = grp.Count(x => x.cur.CO_SERIE_REFER == "3O"),
                               Qtd4A = grp.Count(x => x.cur.CO_SERIE_REFER == "4O"),
                               Qtd5A = grp.Count(x => x.cur.CO_SERIE_REFER == "5O"),
                               Qtd6A = grp.Count(x => x.cur.CO_SERIE_REFER == "6O"),
                               Qtd7A = grp.Count(x => x.cur.CO_SERIE_REFER == "7O"),
                               Qtd8A = grp.Count(x => x.cur.CO_SERIE_REFER == "8O"),
                               Qtd9A = grp.Count(x => x.cur.CO_SERIE_REFER == "9O")
                           });
        ...
        public class ReportAlunosPorUnidSerieOutros
        {
