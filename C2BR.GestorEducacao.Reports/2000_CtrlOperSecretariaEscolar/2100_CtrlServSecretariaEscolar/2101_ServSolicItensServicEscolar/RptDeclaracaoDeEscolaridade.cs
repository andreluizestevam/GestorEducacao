...
{
    public partial class RptDeclaracaoDeEscolaridade : C2BR.GestorEducacao.Reports.Base.RptDeclaracao
    {
        public RptDeclaracaoDeEscolaridade()
        {
            InitializeComponent();
        }
        public int InitReport(int codEmp, string codAluCad, string ano, int coModCur, int coCur, int coTurma)
        {
            ...
            try
            {
                ...
                var res = from m in ctx.TB08_MATRCUR
                          ...
                          where (intCodAluCad != 0 ? m.CO_ALU == intCodAluCad : 0 == 0)
                          && (m.CO_SIT_MAT == "A")
                          && (codEmp != 0 ? m.CO_EMP == codEmp : 0 == 0)
                ...
                #region Pega o horário pelos turnos Matutino, Vespertino e Noturno
                int codCurso = dados.AlunoCodCurso;
                int codModu = dados.AlunoCodModulo;
                ...
                // Valida se o curso possui o turno matutino
                if (ctx.TB131_TEMPO_AULA.Where(r => r.CO_CUR == codCurso && r.CO_MODU_CUR == codModu && r.TP_TURNO == "M").Any())
                {
                    tM = true;
                    HRIM = ctx.TB131_TEMPO_AULA.Where(i => i.CO_CUR == codCurso && i.CO_MODU_CUR == codModu && i.TP_TURNO == "M").OrderBy(o => o.NR_TEMPO).FirstOrDefault().HR_INICIO;
                    HRTM = ctx.TB131_TEMPO_AULA.Where(i => i.CO_CUR == codCurso && i.CO_MODU_CUR == codModu && i.TP_TURNO == "M").OrderByDescending(o => o.NR_TEMPO).FirstOrDefault().HR_TERMI;
                }

                // Valida se o curso possui o turno vespertino
                if (ctx.TB131_TEMPO_AULA.Where(r => r.CO_CUR == codCurso && r.CO_MODU_CUR == codModu && r.TP_TURNO == "V").Any())
                {
                    tV = true;
                    HRIV = ctx.TB131_TEMPO_AULA.Where(i => i.CO_CUR == codCurso && i.CO_MODU_CUR == codModu && i.TP_TURNO == "V").OrderBy(o => o.NR_TEMPO).FirstOrDefault().HR_INICIO;
                    HRTV = ctx.TB131_TEMPO_AULA.Where(i => i.CO_CUR == codCurso && i.CO_MODU_CUR == codModu && i.TP_TURNO == "V").OrderByDescending(o => o.NR_TEMPO).FirstOrDefault().HR_TERMI;
                }

                // Valida se o curso possui o turno noturno
                if (ctx.TB131_TEMPO_AULA.Where(r => r.CO_CUR == codCurso && r.CO_MODU_CUR == codModu && r.TP_TURNO == "N").Any())
                {
                    tN = true;
                    HRIN = ctx.TB131_TEMPO_AULA.Where(i => i.CO_CUR == codCurso && i.CO_MODU_CUR == codModu && i.TP_TURNO == "N").OrderBy(o => o.NR_TEMPO).FirstOrDefault().HR_INICIO;
                    HRTN = ctx.TB131_TEMPO_AULA.Where(i => i.CO_CUR == codCurso && i.CO_MODU_CUR == codModu && i.TP_TURNO == "N").OrderByDescending(o => o.NR_TEMPO).FirstOrDefault().HR_TERMI;
                }
                #endregion
                ...
                if (lst != null && lst.Where(x => x.Pagina == 1).Any())
                {
                    ...
                    foreach (var Doc in lst)
                    {
                        ...
                        st.Value = st.Value.Replace("[RespCep]", dados.RespCep);
                        ...
                        st.Value = st.Value.Replace("[AlunoNatural]", (dados.AlunoNatural == " " || dados.AlunoNatural == null ? "*****" : dados.AlunoNatural));
                        ...
                        st.Value = st.Value.Replace("[Turno]", dados.AlunoTurno);
                        ...
                        #region Dados do horário da turma
                        if (tM)
                        {
                            st.Value = st.Value.Replace("[HrManha]", " no turno da manhã das " + HRIM + " às " + HRTM);
                        }
                        else
                        {
                            st.Value = st.Value.Replace("[HrManha]", "");
                        }
                        ...
                        if (tV)
                        {
                            if (tM)
                            {
                                if (tN)
                                {
                                    st.Value = st.Value.Replace("[HrTarde]", ", no turno da tarde das " + HRIV + " às " + HRTV);
                                }
                                else
                                {
                                    st.Value = st.Value.Replace("[HrTarde]", " e no turno da tarde das " + HRIV + " às " + HRTV);
                                }
                            }
                            else
                            {
                        ...
                        {
                            st.Value = st.Value.Replace("[HrTarde]", "");
                        }
                        ...
                        if (tN)
                        {
                            if (tV)
                            {
                                st.Value = st.Value.Replace("[HrNoite]", " e no turno da noite das " + HRIN + " às " + HRTN);
                            }
                            else
                            {
                                st.Value = st.Value.Replace("[HrNoite]", " no turno da noite das " + HRIN + " às " + HRTN);
                            }
                        }
                        else
                        {
                            st.Value = st.Value.Replace("[HrNoite]", "");
                        }
                        #endregion
