...
{
    public partial class RptDeclaracaoAdvertencia : C2BR.GestorEducacao.Reports.Base.RptDeclaracao
    {
        public RptDeclaracaoAdvertencia()
        {
            InitializeComponent();
        }

        public int InitReport(int codEmp, string codAluCad, int idOcorr)
        {
            try
            {
                ...
                var res = (from m in ctx.TB08_MATRCUR
                           ...
                           join bResp in ctx.TB905_BAIRRO on m.TB108_RESPONSAVEL.CO_BAIRRO equals bResp.CO_BAIRRO
                ...
                if (lst != null && lst.Where(x => x.Pagina == 1).Any())
                {
                    ...
                    foreach (var Doc in lst)
                    {
                        ...
                        var ocorr = (from tb191 in TB191_OCORR_ALUNO.RetornaTodosRegistros()
                                     join tb03 in TB03_COLABOR.RetornaTodosRegistros() on tb191.TB03_COLABOR.CO_COL equals tb03.CO_COL
                                     where tb191.IDE_OCORR_ALUNO == idOcorr
                                     select new
                                     {
                        ...
                        //Descrição da Ocorrência, se tiver sido selecionada
                        st.Value = st.Value.Replace("[MotivOcorr]", ocorr.DE_OCORR);
                        st.Value = st.Value.Replace("[AcaoOcorr]", ocorr.DE_ACAO_TOMAD);
                        st.Value = st.Value.Replace("[RespoOcorr]", ocorr.NO_COL);
                        st.Value = st.Value.Replace("[DataOcorr]", ocorr.DT_OCORR.ToShortDateString());
