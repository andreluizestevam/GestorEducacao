...
{
    public partial class RptDeclaracaoDeQuitacaoFinanceira : C2BR.GestorEducacao.Reports.Base.RptDeclaracao
    {
        public RptDeclaracaoDeQuitacaoFinanceira()
        {
            InitializeComponent();
        }
        public int InitReport(int codEmp, string codAluCad)
        {
            try
            {
               ...
               // C2BR.GestorEducacao.Reports.Base.RptDeclaracao pageHeader = new Base.RptDeclaracao();
                ...
                //var flr = ctx.TB009_RTF_DOCTOS.Where(fl => fl.co == codAluCad).Single();
                ...
                var res = from m in ctx.TB08_MATRCUR
                          ...
                          select new
                          {
                              ...
                              AlunoCursoCodigo = m.CO_CUR,
                              AlunoModuloCodigo = m.TB44_MODULO.CO_MODU_CUR,
                              ano = m.CO_ANO_MES_MAT,
                          };
                #region dell
                ...
                if (lst != null && lst.Where(x => x.Pagina == 1).Any())
                {
                    ...
                    foreach (var Doc in lst)
                    {
                        ...
                        if (dados.RespSexo == "M")
                        {
                            st.Value = st.Value.Replace("[ResponsavelNome]", "Sr. " + dados.AlunoPai);
                        }
                        else
                        {
                            st.Value = st.Value.Replace("[ResponsavelNome]", "Sra. " + dados.AlunoPai);
                        }
                        st.Value = st.Value.Replace("[ResponsavelRG]", dados.RespRG);
                        //st.Value = st.Value.Replace("[ResponsavelCPF]", String.Format(@"{0:000\.000\.000\-00}", long.Parse(dados.RespCPF)));
                        st.Value = st.Value.Replace("[ResponsavelCPF]", dados.RespCPF);
                        ...
                        //Dados Gerais
                        st.Value = st.Value.Replace("[DataPriVncto]", (dataPrimPar.HasValue ? dataPrimPar.Value.ToString("dd/MM/yyyy") : "***"));
