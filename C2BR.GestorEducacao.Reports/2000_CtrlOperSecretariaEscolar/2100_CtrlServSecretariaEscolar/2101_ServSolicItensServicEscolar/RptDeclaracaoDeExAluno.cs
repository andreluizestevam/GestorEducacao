...
{
    public partial class RptDeclaracaoDeExAluno : C2BR.GestorEducacao.Reports.Base.RptDeclaracao
    {
        public RptDeclaracaoDeExAluno()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                var par = (from m in ctx.TB47_CTA_RECEB
                           ...
                           select new Parcela
                           {
                               dtVen = m.DT_VEN_DOC,
                               icSit = m.IC_SIT_DOC,
                               dtRec = m.DT_REC_DOC.Value,
                               vrDes = m.VR_DES_PAG.Value,
                               vrPar = m.VR_PAR_DOC,
                               vrBol = m.VR_DES_BOLSA_PAG.Value,
                               vrTot = m.VR_TOT_DOC,
                               vrPag = m.VR_PAG.Value
                           }).OrderBy(o => o.dtVen);
                
                // Se todas as parcelas não estiverem quitadas, a declaração não pode ser emitida
                //if (par.Where(r => r.icSit == "Q").Count() < par.Count())
                //{
                //    return -1;
                //}
                #endregion
                ...
                var lst = (from tb009 in ctx.TB009_RTF_DOCTOS
                           ...
                           where tb009.TP_DOCUM == "DE" && tb009.CO_SIGLA_DOCUM == "DEA"
                ...
                if (lst != null && lst.Where(x => x.Pagina == 1).Any())
                {
                    foreach (var Doc in lst)
                    {
                        ...
                        string anoIniPar = par.First().dtRec.Year.ToString();
                        st.Value = st.Value.Replace("[AnoIniPar]", anoIniPar);

                        string anoFimPar = par.ToList().Last().dtRec.Year.ToString();
                        st.Value = st.Value.Replace("[AnoFimPar]", anoFimPar);
                        ...
                        st.Value = st.Value.Replace("[AlunoRGNumero]", dados.AlunoRGNumero != "" ? " RG: " + dados.AlunoRGNumero : "");
                        ...
                        #region Gera a grid de valores

                        string Tab = "";

                        string iniTab = "{\\rtf1\\ansi\\deff0\\trowd\\cellx800\\cellx2700\\cellx5000\\cellx7000\\cellx8300\\intbl ANO\\cell\\intbl R$ CONTRATO\\cell\\intbl R$ MENSALIDADE\\cell\\intbl R$ DESCONTO\\cell\\intbl R$ PAGO\\cell\\row";
                        //string iniTab = "\\deff0\\trowd\\cellx800\\cellx2700\\cellx5000\\cellx7000\\cellx8300\\intbl ANO\\cell\\intbl R$ CONTRATO\\cell\\intbl R$ MENSALIDADE\\cell\\intbl R$ DESCONTO\\cell\\intbl R$ PAGO\\cell\\row";
                        string cabTab = "\\trowd\\cellx800\\cellx2700\\cellx5000\\cellx7000\\cellx8300\\intbl ---------\\cell\\intbl -----------------------\\cell\\intbl ----------------------------\\cell\\intbl -------------------------\\cell\\intbl ------------\\cell\\row";
                        string linTab = "";

                        //int i = 0;
                        foreach (var p in par){
                            linTab += "\\trowd\\cellx800\\cellx2700\\cellx5000\\cellx7000\\cellx8300\\intbl " + p.dtRec.Year.ToString() + "\\cell\\intbl " + String.Format("{0:C}", p.vrTot) + "\\cell\\intbl " + String.Format("{0:C}", p.vrPar) + "\\cell\\intbl " + String.Format("{0:C}", p.vrDes + p.vrBol) + "\\cell\\intbl " + String.Format("{0:C}", p.vrPag) + "\\cell\\row";
                        }

                        linTab += "}";

                        Tab = iniTab + cabTab + linTab;

                        st.Value = st.Value.Replace("[TabValores]", Tab);
        ...
        public class Parcela
        {
            ...
            public string icSit { get; set; }
            public DateTime dtRec { get; set; }
            public decimal vrDes { get; set; }
            public decimal vrPar { get; set; }
            public decimal vrBol { get; set; }
            public decimal vrTot { get; set; }
