...
{
    public partial class RptDeclaracaoComparecimentoInsef : C2BR.GestorEducacao.Reports.Base.RptDeclaracao
    {
        public RptDeclaracaoComparecimentoInsef()
        {
            InitializeComponent();
        }
        public int InitReport(int codEmp, string codAluCad, string DataTemp, string PeriodoTemp, string TextoTemp)
        {
            try
            {
                ...
                int codAluno = int.Parse(codAluCad);
                var res = from m in ctx.TB08_MATRCUR
                          ...
                          select new
                          {
                              ...
                              //Dados alunos
                              AlunoNome = String.IsNullOrEmpty(m.TB07_ALUNO.NO_ALU) ? "*****" : m.TB07_ALUNO.NO_ALU,
                              ...
                              ProximaSerie = cur.CO_PREDEC_CUR,
                              ProximaModalidade = cur.CO_PREDEC_MOD,
                              ...
                              AlunoAprov = m.CO_STA_APROV,
                              //Dados responsavel
                              ResponsavelEndereco = m.TB07_ALUNO.TB108_RESPONSAVEL.NU_ENDE_RESP,
                              RespNome = m.TB07_ALUNO.TB108_RESPONSAVEL.NO_RESP,
                              RespEnd = String.IsNullOrEmpty(m.TB07_ALUNO.TB108_RESPONSAVEL.DE_ENDE_RESP) ? "*****" : m.TB07_ALUNO.TB108_RESPONSAVEL.DE_ENDE_RESP,
                              ...
                              RespCep = String.IsNullOrEmpty(m.TB07_ALUNO.TB108_RESPONSAVEL.CO_CEP_RESP) ? "*****" : m.TB07_ALUNO.TB108_RESPONSAVEL.CO_CEP_RESP,
                ...
                var lst = (from tb009 in ctx.TB009_RTF_DOCTOS
                           ...
                           //b009.ID_DOCUM && CO_SITUS_DOCUM == "A"
                ...
                if (lst != null && lst.Where(x => x.Pagina == 1).Any())
                {
                    ...
                    foreach (var Doc in lst)
                    {
                        ...
                        st.Value = st.Value.Replace("[EmpCidadeEstado]", dados.CidadeEstado);
                        ...
                        #region Proxima Série e Modalidade

                        switch(dados.AlunoAprov){
                                    
                            case "S":

                                st.Value = st.Value.Replace("[ProximaSerie]", dados.ProximaSerie.ToString());
                                st.Value = st.Value.Replace("[ProximaModalidade]", dados.ProximaModalidade.ToString());
                                break;

                            case "N":

                                st.Value = st.Value.Replace("[ProximaSerie]", dados.AlunoCurso);
                                st.Value = st.Value.Replace("[ProximaModaildade]", dados.AlunoModulo);
                                break;

                            default:
                                
                                st.Value = st.Value.Replace("[ProximaSerie]", dados.AlunoCurso);
                                st.Value = st.Value.Replace("[ProximaModalidade]", dados.AlunoModulo);
                                break;
                        }

                        #endregion

                        #region Dados Temporários

                        //Dados temporários
                        st.Value = st.Value.Replace("[DataTemp]", DataTemp);
                        
                        switch (PeriodoTemp) 
                        { 
                            case "M":
                                st.Value = st.Value.Replace("[PeriodoTemp]", "Matutino");
                                break;

                            case "V":
                                st.Value = st.Value.Replace("[PeriodoTemp]", "Vespertino");
                                break;

                            case "N":
                                st.Value = st.Value.Replace("[PeriodoTemp]", "Noturno");
                                break;
                        }

                        st.Value = st.Value.Replace("[TextoTemp]", TextoTemp);
